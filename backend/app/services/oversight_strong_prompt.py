...
پیش از و حین تغییر، تمام وابستگی‌های **دو طرفه** را به‌طور **کامل و بدون
هیچ خلاصه‌سازی** شناسایی و همگام کن:
...
**۳. وابستگی‌های جانبی (cross-cutting — حتماً همه را چک کن):**
- تست‌ها (unit، integration، e2e — همه‌ی test file های مرتبط)
- migrations و seed data
- API contracts، OpenAPI/GraphQL schemas
- frontend components که این endpoint/data را مصرف می‌کنند
- background jobs، cron tasks، schedulers
- documentation و README و changelog
- CI/CD pipelines (اگر behavior جدید نیاز به env یا step جدید دارد)
- مستندات کاربر (i18n files، help text، UI labels)
- type definitions (`.d.ts`، Pydantic models، Protobuf، Avro، GraphQL types)
- caches و serialization formats (اگر data shape تغییر کرده)
- monitoring/logging (اگر metric یا log key اضافه/حذف شده)
- feature flags و config های runtime

**قانون مطلق همگام‌سازی:**
- هر چیزی که در (۱)، (۲)، یا (۳) شناسایی شد، در **همان workflow این تسک**
  همگام و به‌روز شود. هرگز برای بعد رها نکن.
...
همگام‌شده را explicit بنویس. مثال:
```
Dependencies synced:
- upstream: User model schema, auth middleware
- downstream: 3 API endpoints, 5 frontend components, 12 tests
- side: OpenAPI spec, README API section, i18n keys for new errors
```
اگر هیچ وابستگی پیدا نکردی، صریحاً بنویس: «بررسی شد — هیچ وابستگی
upstream/downstream/side شناسایی نشد» تا مشخص باشد بررسی **انجام شده**
نه اینکه فراموش شده.
