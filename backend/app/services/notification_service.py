...
class NotificationService:
    ...
    async def _handle_codex_callback(
        ...
        if action == "build_model":
            ...
            try:
                ...
                deep_warn = "" if used_deep else "\n⚠️ بدون Deep Scan — ساختار پایه استفاده شد."
                msg = (
                    f"✅ *Codex ساخته شد*\n\n"
    ...
    def _render_codex_markdown(self, codex: Dict[str, Any], repo_name: str) -> str:
        """تبدیل Codex به markdown برای ارسال به‌عنوان فایل."""
        lines: List[str] = []
        ...
        if codex.get("user_goal"):
            ...
            lines.append("")
        if codex.get("stacks"):
            lines.append(f"**Stack**: {', '.join(codex['stacks'])}")
            lines.append("")
        if codex.get("model_used"):
            lines.append(f"**Generated by**: `{codex['model_used']}`")
        if codex.get("updated_at"):
            lines.append(f"**Updated at**: {codex['updated_at']}")
        lines.append("")
        lines.append("---")
        lines.append("")
        files = codex.get("files") or {}
