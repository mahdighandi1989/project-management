...
def _build_visual_debug_prompt_list() -> list:
    ...
    return [
        ...
        },
        {
            ...
            "icon": "📥",
            "prompt_detail": """اطلاعاتی که دریافت می‌کنید:
...
3. **کد فعلی فایل‌ها**: فرانت و بکند مرتبط با آن صفحه/endpoint
4. **ساختار پروژه**: درخت فایل‌ها
...
🚫 ممنوعیت مطلق حدس‌زنی:
- هرگز محتوای فایلی را که ندیده‌ای حدس نزن، فرض نکن
- از «فرض می‌کنیم»، «احتمالاً» استفاده نکن
- اگر فایلی نیست: بگو «فایل X خوانده نشده — نیاز به محتوای واقعی آن دارم»
- فقط فایل‌هایی را در action_plan بگذار که واقعاً محتوایشان را دیده‌ای
...
def _build_visual_debug_prompt_text(general_instructions_text: str = "") -> str:
    ...
    if general_instructions_text:
        prompt_text += f"\n\n{general_instructions_text}\n"
    prompt_text += """\n## فایل‌های بکند مرتبط:
اگر لیستی از فایل‌های بکند در زیر نشان داده شده، از آنها استفاده کنید. اگر فایل مورد نیاز در لیست نیست، صراحتاً بگویید «فایل X خوانده نشده — نیاز به محتوای واقعی آن دارم»."""
    return prompt_text
