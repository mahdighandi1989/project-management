---
task_id: 9339e026-8d34-4cc3-9bee-c6adc549ab31
title: ذخیرهٔ API keyها در فایل .env به‌صورت plaintext و لاگ احتمالی خطا
type: security
priority: high
...
created_at: '2026-06-14T09:39:52.049588+00:00'
updated_at: '2026-06-14T09:42:17.949713+00:00'
target_files:
- backend/app/api/routes/settings.py
- backend/app/api/routes/settings.py
---

# ذخیرهٔ API keyها در فایل .env به‌صورت plaintext و لاگ احتمالی خطا

## Raw Idea

endpoint های update_api_keys و update_deploy_keys در settings.py تمام کلیدهای API (OpenAI, Claude, GitHub, Render, ...) را علاوه بر دیتابیس (که value_type=encrypted است) به‌صورت plaintext در فایل .env می‌نویسند. این فایل روی فایل‌سیستم سرور با مقدار خام `KEY=value` ذخیره می‌شود و هرگونه دسترسی فایل‌سیستم (یا اشتباه در .gitignore / backup) منجر به افشای همهٔ secretها می‌شود. همچنین خود endpoint update_api_keys هیچ authentication ندارد، پس هر کسی می‌تواند کلیدها را بازنویسی کند. در صورت بروز خطا هنگام نوشتن .env، پیام با print چاپ می‌شود که می‌تواند بخشی از مسیر/داده را در لاگ افشا کند.
...
- `backend/app/api/routes/settings.py:201-219` — `update_api_keys` — نوشتن plaintext کلیدها در .env
  ```python
  ...
  ```
- `backend/app/api/routes/settings.py:165-167` — `update_api_keys`
  ```python
  @router.put("/api-keys")
...
1. نوشتن کلیدها در فایل .env را حذف کن و فقط بر ذخیرهٔ رمزشده در دیتابیس + environment runtime اتکا کن. روی endpoint های /settings/api-keys و /settings/deploy-keys یک dependency auth (admin token) اضافه کن. اطمینان حاصل کن .env در .gitignore است و در client bundle یا response هرگز مقدار خام برنمی‌گردد. print خطاها را با logger.warning بدون افشای مقدار جایگزین کن.
