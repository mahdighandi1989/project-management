---
task_id: 6aff92c2-f339-4bbd-8723-8b1716c30f01
...
created_at: '2026-05-31T09:07:45.314446+00:00'
updated_at: '2026-06-03T18:38:13.909965+00:00'
target_files:
...
در `backend/app/api/routes/analysis.py` خطوط 200-206، یک try/except عام وجود دارد که exception را لاگ می‌کند ولی `progress_queue.put` را با event 'error' صدا می‌زند. اما اگر خود `progress_queue.put` هم exception بدهد (مثلاً به دلیل بسته شدن queue)، آن exception بلعیده می‌شود و کاربر هیچ خطایی نمی‌بیند. همچنین در خطوط 251-253، یک try/except دیگر برای خطاهای SSE وجود دارد که فقط logger.error می‌زند و yield می‌کند — اگر yield هم fail شود، exception بی‌صدا می‌میرد.
...
- `backend/app/api/routes/analysis.py:200-206` — `run_analysis_task` — اگر progress_queue.put exception بدهد، بلعیده می‌شود
  ```python
  ...
  ```
- `backend/app/api/routes/analysis.py:251-253` — `generate_events` — اگر yield exception بدهد، بی‌صدا می‌میرد
  ```python
  except Exception as e:
...
FastAPI StreamingResponse با SSE، asyncio.Queue

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/deep_analysis_service.py` (سطر 120) — این سرویس توسط run_analysis_stream فراخوانی می‌شود
...
## 🌐 نقشهٔ وابستگی‌ها
این endpoint توسط frontend/src/app/analysis/page.tsx (خط 190) فراخوانی می‌شود. اگر stream بی‌صدا بمیرد، کاربر تا timeout منتظر می‌ماند.
...
- [ ] اگر deep_analysis_service exception بدهد، کاربر event 'error' دریافت می‌کند
- [ ] اگر stream بسته شود، exception بی‌صدا نمی‌میرد
- [ ] لاگ خطا شامل traceback کامل است
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. بهبود error handling در run_analysis_stream: 1) در خط 200-206، یک try/except سطح بالاتر برای progress_queue.put اضافه کن. 2) در خط 251-253، از finally برای بستن مطمئن stream استفاده کن. 3) یک fallback response برای زمانی که stream نمی‌تواند شروع شود اضافه کن.

## 💡 نمونه‌های قبل/بعد
**بهبود error handling در generate_events**
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/ -k analysis_stream -v`
- `curl -X POST http://localhost:8000/api/analysis/run-stream -H 'Content-Type: application/json' -d '{"project_id":"test","project_path":"/nonexistent"}'`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر در error handling ممکن است رفتار stream را برای clientهای موجود تغییر دهد
...
1. اگر deep_analysis_service exception بدهد، کاربر event 'error' دریافت می‌کند _(verify: backend_test)_
2. اگر stream بسته شود، exception بی‌صدا نمی‌میرد _(verify: backend_test)_
3. لاگ خطا شامل traceback کامل است _(verify: backend_test)_
