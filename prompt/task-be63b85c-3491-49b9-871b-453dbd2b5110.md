---
task_id: be63b85c-3491-49b9-871b-453dbd2b5110
title: Conflicting Model Profiling Endpoints
type: bug
priority: high
...
created_at: '2026-06-07T09:29:00.488778+00:00'
updated_at: '2026-06-07T09:33:02.199306+00:00'
target_files:
- backend/app/api/routes/models.py
- backend/app/api/routes/model_profiles.py
---

# Conflicting Model Profiling Endpoints

## Raw Idea

There are two distinct API endpoints, both mapped to `/profiles`, in `backend/app/api/routes/models.py` and `backend/app/api/routes/model_profiles.py`. This creates a direct routing conflict and ambiguity, making it unclear which endpoint will be served and leading to unpredictable behavior or errors. The `model_profiles.py` module is explicitly designed for model profiling, suggesting it should be the sole authority for this functionality.
...
- `backend/app/api/routes/models.py:321-342` — `get_model_profiles`
  ```python
  @router.get("/profiles")
  ...
  ```
- `backend/app/api/routes/model_profiles.py:142-245` — `get_all_profiles`
  ```python
  @router.get("/profiles")
  async def get_all_profiles(
      ...
      """
      # ... (truncated for brevity, contains full logic for profiles) ...
  ```
...
FastAPI routing, Python backend.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` — Registers both routers, leading to the conflict.
- `frontend/src/app/model-profiles/page.tsx` — Primary frontend consumer of model profiles, should use the dedicated model_profiles endpoint.
- `frontend/src/app/models/page.tsx` — Displays model information, but should not be the source for aggregated profiles.
...
## 🌐 نقشهٔ وابستگی‌ها
This is a direct routing conflict that needs immediate resolution to ensure predictable API behavior and maintain a clear separation of concerns between model listing and model profiling.
...
- [ ] The `/profiles` endpoint in `backend/app/api/routes/models.py` is removed.
- [ ] All API calls to `/api/models/profiles` are correctly routed to `backend/app/api/routes/model_profiles.py`.
- [ ] The `frontend/src/app/models/page.tsx` and `frontend/src/app/model-profiles/page.tsx` correctly display model data without conflicts.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Remove the `/profiles` endpoint from `backend/app/api/routes/models.py`. Ensure that all frontend components or internal services that previously called this endpoint are updated to use the `/api/models/profiles` endpoint provided by `backend/app/api/routes/model_profiles.py`.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `Run `uvicorn app.main:app --reload` and verify that no routing conflicts are reported on startup.`
- `Access `/docs` and verify that only one `/api/models/profiles` endpoint is listed.`
- `Test `frontend/src/app/model-profiles/page.tsx` to ensure it loads data correctly.`

## ⚠️ ریسک‌ها و موارد احتیاط
Minor risk of breaking frontend components if not all references are updated to the correct endpoint.
...
1. The `/profiles` endpoint in `backend/app/api/routes/models.py` is removed. _(verify: static)_
2. All API calls to `/api/models/profiles` are correctly routed to `backend/app/api/routes/model_profiles.py`. _(verify: api_response)_
3. The `frontend/src/app/models/page.tsx` and `frontend/src/app/model-profiles/page.tsx` correctly display model data without conflicts. _(verify: ui_interaction)_
