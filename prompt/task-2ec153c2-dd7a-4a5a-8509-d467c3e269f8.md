---
task_id: 2ec153c2-dd7a-4a5a-8509-d467c3e269f8
title: '[منطق] نقص در مسیر mutation برای OversightUploadSession'
type: logic_audit
priority: high
...
created_at: '2026-05-31T09:08:42.449362+00:00'
updated_at: '2026-06-01T17:09:02.752929+00:00'
---

# [منطق] نقص در مسیر mutation برای OversightUploadSession
...
سرویس OversightUploadSession state را در فایل JSON ذخیره می‌کند و temp files را مدیریت می‌کند، اما هیچ مکانیزمی برای بررسی اینکه آیا کاربر مجاز به آپلود فایل برای یک task_draft_id خاص است وجود ندارد. همچنین پاکسازی temp files به صورت خودکار انجام می‌شود اما ممکن است race condition ایجاد کند.

## 💥 پیامد (impact)
یک کاربر مخرب می‌تواند با ارسال درخواست‌های chunked به task_draft_idهای متعلق به کاربران دیگر، فایل‌های مخرب آپلود کند یا فضای دیسک را پر کند. همچنین race condition در cleanup می‌تواند باعث از دست رفتن فایل‌های معتبر شود.

## 🛠 پیشنهاد رفع اولیه
قبل از ایجاد UploadSession، ownership task_draft_id را بررسی کنید. از lock file یا transaction برای جلوگیری از race condition در cleanup استفاده کنید. محدودیت rate limiting برای آپلودهای همزمان اعمال کنید.
...
### Step 1: بررسی ownership task_draft_id قبل از ایجاد UploadSession
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اضافه کردن مکانیزم بررسی مالکیت task_draft_id در سرویس OversightUploadSession است. قبل از ایجاد هر UploadSession جدید، باید بررسی شود که کاربر درخواست‌دهنده (از طریق توکن احراز هویت) مجاز به آپلود فایل برای task_draft_id مشخص شده است. این بررسی باید با کوئری به دیتابیس یا سرویس مرتبط انجام شود. خارج از این مرحله: پیاده‌سازی rate limiting، مکانیزم lock file برای cleanup، یا تغییر در نحوه ذخیره‌سازی state JSON. نکته حیاتی: این بررسی باید در لایه سرویس (نه middleware) انجام شود تا با منطق business هماهنگ باشد.
**Excerpt:**
```
سرویس OversightUploadSession state را در فایل JSON ذخیره می‌کند و temp files را مدیریت می‌کند، اما هیچ مکانیزمی برای بررسی اینکه آیا کاربر مجاز به آپلود فایل برای یک task_draft_id خاص است وجود ندارد. ... قبل از ایجاد UploadSession، ownership task_draft_id را بررسی کنید.
```

### Step 2: پیاده‌سازی مکانیزم lock file برای جلوگیری از race condition در cleanup
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اضافه کردن مکانیزم lock file (یا transaction) برای عملیات پاکسازی temp files است. هنگام پاکسازی فایل‌های موقت مرتبط با یک UploadSession، باید از یک lock file استفاده شود تا دو فرآیند همزمان نتوانند همزمان cleanup کنند و باعث race condition شوند. مکانیزم باید شامل ایجاد یک فایل lock (مثلاً .lock) قبل از شروع cleanup و حذف آن پس از اتمام باشد. خارج از این مرحله: بررسی ownership، rate limiting، یا تغییر در منطق ایجاد UploadSession. نکته حیاتی: lock باید timeout داشته باشد تا در صورت crash فرآیند، سیستم قفل نماند.
**Excerpt:**
```
همچنین پاکسازی temp files به صورت خودکار انجام می‌شود اما ممکن است race condition ایجاد کند. ... از lock file یا transaction برای جلوگیری از race condition در cleanup استفاده کنید.
```

### Step 3: اعمال محدودیت rate limiting برای آپلودهای همزمان
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اضافه کردن محدودیت rate limiting برای درخواست‌های آپلود همزمان به سرویس OversightUploadSession است. باید محدودیتی برای تعداد آپلودهای همزمان (مثلاً ۵ آپلود همزمان) برای هر کاربر یا هر task_draft_id اعمال شود. این محدودیت باید در سطح سرویس یا با استفاده از middleware پیاده‌سازی شود. خارج از این مرحله: بررسی ownership، مکانیزم lock file برای cleanup، یا تغییر در ذخیره‌سازی state JSON. نکته حیاتی: rate limiting باید بر اساس IP کاربر یا توکن احراز هویت باشد و پیام خطای مناسب (429 Too Many Requests) برگرداند.
**Excerpt:**
```
محدودیت rate limiting برای آپلودهای همزمان اعمال کنید.
```
