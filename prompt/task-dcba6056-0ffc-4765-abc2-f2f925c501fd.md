---
task_id: dcba6056-0ffc-4765-abc2-f2f925c501fd
title: duplicate project creation endpoint در simple_projects.py و unified_api.py
type: refactor
priority: high
...
updated_at: '2026-06-03T18:38:09.887047+00:00'
target_files:
- backend/app/api/routes/simple_projects.py
- backend/app/api/routes/unified_api.py
---

# duplicate project creation endpoint در simple_projects.py و unified_api.py

## Raw Idea

هر دو فایل simple_projects.py و unified_api.py یک endpoint برای ایجاد پروژه با نام /api/simple/projects/create و /api/unified/projects/create دارند. هر دو از یک مفهوم 'ایجاد پروژه' استفاده می‌کنند اما مسیرها و منطق متفاوتی دارند. simple_projects.py از simple_creator استفاده می‌کند در حالی که unified_api.py از unified_storage. این دوگانگی باعث سردرگمی در frontend و نگهداری دشوار می‌شود.
...
- `backend/app/api/routes/simple_projects.py:333-401` — `create_project` — endpoint ایجاد پروژه در simple_projects
  ```python
  ...
  ```
- `backend/app/api/routes/unified_api.py:49-71` — `create_project` — endpoint ایجاد پروژه در unified_api
  ```python
  @router.post("/projects/create")
  async def create_project(request: CreateProjectRequest):
      """ایجاد پروژه جدید"""
...
- `backend/app/services/simple_creator.py` (سطر 1) — سرویس backend برای simple_projects
- `backend/app/services/unified_storage.py` (سطر 1) — سرویس backend برای unified_api
- `frontend/src/app/creator/page.tsx` (سطر 373) — فرانت‌اند که از simple_projects استفاده می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این دوگانگی باعث می‌شود هر زمان که منطق ایجاد پروژه تغییر کند، هر دو فایل باید به‌روزرسانی شوند.
...
- [ ] endpoint /api/simple/projects/create و /api/unified/projects/create حذف شوند
- [ ] endpoint جدید /api/projects/create ایجاد شود
- [ ] frontend از endpoint جدید استفاده کند
...
1. ادغام دو endpoint در یک endpoint واحد تحت /api/projects/create با استفاده از simple_creator به عنوان backend اصلی و حذف مسیر /api/unified/projects/create. همچنین باید مطمئن شوید که frontend فقط از یک مسیر استفاده می‌کند.
...
_قبل:_
```
دو endpoint جداگانه برای ایجاد پروژه
```

_بعد:_
```
یک endpoint واحد /api/projects/create
```
...
## 🧪 دستورات اعتبارسنجی
- `curl -X POST http://localhost:8000/api/projects/create -H 'Content-Type: application/json' -d '{"name":"test","description":"test"}'`
- `pytest backend/tests/`

## ⚠️ ریسک‌ها و موارد احتیاط
ممکن است برخی از ویژگی‌های unified_api (مانند GitHub sync) در simple_creator موجود نباشد.
