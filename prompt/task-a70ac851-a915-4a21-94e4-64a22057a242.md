---
task_id: a70ac851-a915-4a21-94e4-64a22057a242
title: Duplication of Idea-to-Prompt API Logic
type: refactor
priority: high
...
created_at: '2026-06-07T09:29:00.155226+00:00'
updated_at: '2026-06-07T09:32:50.624930+00:00'
target_files:
- backend/app/api/routes/simple_projects.py
- backend/app/api/routes/oversight.py
---

# Duplication of Idea-to-Prompt API Logic

## Raw Idea

The logic for converting a raw idea into a structured prompt, including handling file attachments and reference projects, is duplicated across the `idea_to_prompt_preview` endpoint in `simple_projects.py` and implicitly in `oversight.py` (as indicated by the comment 'استفاده مجدد از همان منطق oversight'). This redundancy complicates feature development and maintenance for prompt generation.
...
- `backend/app/api/routes/simple_projects.py:226-379` — `idea_to_prompt_preview`
  ```python
  ...
  async def idea_to_prompt_preview(request: IdeaToPromptRequest):
      ...
      # ... (truncated for brevity, includes attachment and reference project logic) ...
      try:
  ...
  ```
- `backend/app/api/routes/oversight.py:77-105` — `IdeaToPromptRequest`
  ```python
...
FastAPI backend, AI prompt engineering, file processing, database interactions.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/creator_idea_to_prompt.py` — Contains the core logic for generating strong prompts.
- `backend/app/services/oversight_service.py` — Handles attachment resolution and is referenced in the duplicated logic.
- `backend/app/services/reference_project_service.py` — Used for building context from reference projects.
- `frontend/src/app/creator/page.tsx` — Consumes the idea-to-prompt API for project creation.
- `frontend/src/app/oversight/page.tsx` — Likely uses idea-to-prompt for task creation from user input.
...
## 🌐 نقشهٔ وابستگی‌ها
This logic is crucial for both new project creation and task generation within the Oversight system. Centralizing it will streamline prompt engineering and ensure consistent behavior across different user flows.
...
- [ ] A single, authoritative service function exists for processing raw ideas into structured prompts, including handling attachments and reference projects.
- [ ] Both `/api/simple/projects/idea-to-prompt` and the relevant endpoint in `/oversight` (if it exists) call this centralized service.
- [ ] Error handling for missing vision models or failed extractions is consistent across all callers.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Centralize the API-level logic for `idea_to_prompt` (including attachment resolution, reference project context building, and error handling for vision models) into a dedicated service function, e.g., in `backend/app/services/creator_idea_to_prompt.py`. Both `simple_projects.py` and `oversight.py` should then call this single service function, passing their specific request parameters.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `Test project creation from idea with attachments and reference projects.`
- `Test task creation in Oversight from idea with attachments and reference projects.`

## ⚠️ ریسک‌ها و موارد احتیاط
Requires careful extraction and re-integration to avoid breaking existing prompt generation workflows.
...
1. A single, authoritative service function exists for processing raw ideas into structured prompts, including handling attachments and reference projects. _(verify: static)_
2. Both `/api/simple/projects/idea-to-prompt` and the relevant endpoint in `/oversight` (if it exists) call this centralized service. _(verify: static)_
3. Error handling for missing vision models or failed extractions is consistent across all callers. _(verify: api_response)_
