---
task_id: 992fd697-c7be-4f1d-a479-9a660af889a4
title: عدم وجود rate-limiting و throttle در API endpoints
type: security
priority: high
...
created_at: '2026-05-31T09:07:10.247329+00:00'
updated_at: '2026-06-01T17:07:09.362718+00:00'
target_files:
- backend/app/main.py
---

# عدم وجود rate-limiting و throttle در API endpoints

## Raw Idea

هیچ‌یک از routeهای FastAPI از middleware یا dependency rate-limiting استفاده نمی‌کنند. این موضوع امکان brute-force، DoS و abuse از طریق فراخوانی مکرر endpoints (مانند /api/analysis/run-stream، /api/debate/create، /api/oversight/scan) را فراهم می‌کند. با توجه به مصرف بالای tokenهای AI و هزینه‌های API، این یک آسیب‌پذیری جدی است.
...
- `backend/app/main.py:6-11` — `app = FastAPI()` — محل افزودن middleware rate-limiting
  ```python
...
FastAPI + Python + slowapi (پکیج پیشنهادی)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/analysis.py` (سطر 83) — endpoint /analysis/run-stream که تحلیل کامل پروژه را با AI اجرا می‌کند
- `backend/app/api/routes/debate.py` (سطر 1) — endpoint /debate/create که مناظره AI را شروع می‌کند
- `backend/app/api/routes/oversight.py` (سطر 205) — endpoint /oversight/scan که اسکن عمیق پروژه را اجرا می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این مشکل تمام routeهای API را تحت تأثیر قرار می‌دهد. افزودن middleware در main.py روی همه endpoints اعمال می‌شود.
...
- [ ] درخواست بیش از حد مجاز به یک endpoint (مثلاً 100 در دقیقه) با status 429 رد شود
- [ ] endpoint /analysis/run-stream محدودیت 5 درخواست در دقیقه داشته باشد
- [ ] endpoint /debate/create محدودیت 10 درخواست در دقیقه داشته باشد
- [ ] endpoint /oversight/scan محدودیت 3 درخواست در دقیقه داشته باشد
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. افزودن middleware rate-limiting (مثلاً slowapi یا middleware سفارشی) به app FastAPI در main.py. همچنین پیاده‌سازی throttle per-user/per-IP برای endpoints حساس مانند تحلیل، مناظره و اسکن.

## 💡 نمونه‌های قبل/بعد
**افزودن middleware rate-limit**
...
## 🧪 دستورات اعتبارسنجی
- `ab -n 200 -c 10 http://localhost:8000/api/models/`
- `curl -v http://localhost:8000/api/analysis/run-stream -X POST -H 'Content-Type: application/json' -d '{"project_id":"test","project_path":"/tmp"}'`

## ⚠️ ریسک‌ها و موارد احتیاط
تنظیم threshold خیلی پایین ممکن است کاربران عادی را محدود کند. نیاز به تنظیم دقیق بر اساس الگوی استفاده.
...
1. درخواست بیش از حد مجاز به یک endpoint (مثلاً 100 در دقیقه) با status 429 رد شود _(verify: api_response)_
2. endpoint /analysis/run-stream محدودیت 5 درخواست در دقیقه داشته باشد _(verify: api_response)_
3. endpoint /debate/create محدودیت 10 درخواست در دقیقه داشته باشد _(verify: api_response)_
4. endpoint /oversight/scan محدودیت 3 درخواست در دقیقه داشته باشد _(verify: api_response)_
