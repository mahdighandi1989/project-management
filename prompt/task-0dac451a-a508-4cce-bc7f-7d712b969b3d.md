---
task_id: 0dac451a-a508-4cce-bc7f-7d712b969b3d
title: افزودن تست‌های واحد برای عملیات اصلی دیتابیس در `backend/app/core/database.py`
type: refactor
priority: critical
...
created_at: '2026-06-07T09:28:41.033166+00:00'
updated_at: '2026-06-07T09:31:49.363182+00:00'
target_files:
- backend/app/core/database.py
---

# افزودن تست‌های واحد برای عملیات اصلی دیتابیس در `backend/app/core/database.py`

## Raw Idea

فایل `backend/app/core/database.py` مسئول مقداردهی اولیه دیتابیس، مدیریت سشن‌ها و ارائه توابع کمکی برای تعامل با دیتابیس است. این فایل یک کامپوننت حیاتی و مرکزی در معماری بک‌اند است که توسط ۶۰ فایل دیگر import می‌شود. عدم وجود تست‌های واحد اختصاصی برای این ماژول، ریسک بالایی از رگرسیون و خطاهای داده‌ای را در پی دارد، به‌ویژه با توجه به اینکه عملیات CRUD و مدیریت سشن‌ها در آن تعریف شده‌اند.
...
- `backend/app/core/database.py:1-60` — `init_db` — این فایل هستهٔ تعامل با دیتابیس است و نیاز به پوشش تست جامع دارد.
  ```python
  def init_db():
...
SQLAlchemy, SQLite, FastAPI

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` — نقطه ورود برنامه که `init_db` را فراخوانی می‌کند.
- `backend/app/api/routes/analysis.py` (سطر 117) — از `SessionLocal` برای عملیات دیتابیس استفاده می‌کند.
- `backend/app/api/routes/github_import.py` (سطر 10) — از `get_db` برای عملیات دیتابیس استفاده می‌کند.
- `backend/app/models/__init__.py` — `database.py` این فایل را import می‌کند
- `backend/app/models/project.py` — `database.py` این فایل را import می‌کند
- `backend/app/models/debate.py` — `database.py` این فایل را import می‌کند
- `backend/app/models/setting.py` — `database.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
تغییرات در این فایل یا خطاهای آن می‌تواند کل برنامه را از کار بیندازد. تست‌های واحد برای اطمینان از پایداری دیتابیس ضروری است.
...
- [ ] فایل `backend/tests/test_database.py` ایجاد شود.
- [ ] تست‌های واحد برای `init_db` اطمینان حاصل کنند که جداول به درستی ایجاد می‌شوند.
- [ ] تست‌های واحد برای `get_db` اطمینان حاصل کنند که سشن دیتابیس به درستی فراهم می‌شود.
- [ ] تست‌ها از یک دیتابیس موقت (in-memory SQLite) برای ایزوله‌سازی استفاده کنند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست جدید `backend/tests/test_database.py` و افزودن تست‌های واحد برای توابع اصلی مانند `init_db`, `get_db`, `SessionLocal`, و اطمینان از صحت اتصال، ایجاد جداول و مدیریت سشن‌ها.
...
احتمال ایجاد رگرسیون در عملیات دیتابیس در صورت عدم دقت در نوشتن تست‌ها.
...
1. فایل `backend/tests/test_database.py` ایجاد شود. _(verify: static)_
2. تست‌های واحد برای `init_db` اطمینان حاصل کنند که جداول به درستی ایجاد می‌شوند. _(verify: backend_test)_
3. تست‌های واحد برای `get_db` اطمینان حاصل کنند که سشن دیتابیس به درستی فراهم می‌شود. _(verify: backend_test)_
4. تست‌ها از یک دیتابیس موقت (in-memory SQLite) برای ایزوله‌سازی استفاده کنند. _(verify: static)_
