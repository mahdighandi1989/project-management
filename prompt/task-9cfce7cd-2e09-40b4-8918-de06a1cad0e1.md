---
task_id: 9cfce7cd-2e09-40b4-8918-de06a1cad0e1
title: Suboptimal AI model fallback in ai_generate_with_meta
type: refactor
priority: medium
...
# Suboptimal AI model fallback in ai_generate_with_meta

## Raw Idea

The `ai_generate_with_meta` function, which handles AI model generation, implements a fallback mechanism where if all user-selected models fail or are unavailable, it defaults to `available_models[0].id`. This 'first available' fallback might not be optimal. The first available model might not be suitable for the specific task, leading to lower quality responses, increased costs, or irrelevant output, especially if the user had specific models in mind that failed for transient reasons.
...
- `backend/app/api/routes/simple_projects.py:137` — Fallback to the first available model
  ```python
...
FastAPI, AI Model Management, Fallback Logic

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/ai_manager.py` — Manages available AI models.
- `backend/app/services/model_profiler.py` — Provides model performance scores that could inform smarter fallback.
- `frontend/src/app/creator/page.tsx` — Frontend page where users select models for project creation.
...
- `backend/app/services/knowledge_center_service.py` — این فایل `simple_projects.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This impacts the quality and reliability of AI-generated content across various features, including project creation and prompt generation, potentially leading to user dissatisfaction and wasted AI resources.
...
- [ ] The AI model fallback mechanism prioritizes models based on suitability (e.g., capabilities, performance scores).
- [ ] Users receive higher quality or more relevant AI responses even if their primary model choices fail.
- [ ] The system provides clear error messages if no suitable model can be found after all fallback attempts.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Enhance the fallback logic to be more intelligent. Instead of simply picking `available_models[0].id`, consider a strategy that prioritizes models based on: 1) a predefined 'most robust/general purpose' model, 2) models with similar capabilities to the originally requested ones, or 3) models with high scores from the `model_profiler`. If no suitable fallback can be determined, raise an error to inform the user that no appropriate model could be found.

## 💡 نمونه‌های قبل/بعد
**Smarter fallback logic (conceptual)**
...
_بعد:_
```
# Example: Fallback to a known robust model or use model profiler
        if not target_ids:
            robust_fallback = ai_manager.get_best_general_purpose_model()
            if robust_fallback:
                target_ids = [robust_fallback.id]
            else:
                raise HTTPException(status_code=500, detail="No suitable AI models available after fallback.")
```
...
## 🧪 دستورات اعتبارسنجی
- `Configure AI models such that user-selected models fail, and observe the fallback model's behavior and output quality.`
- `Implement unit tests for the fallback logic in `ai_generate_with_meta`.`

## ⚠️ ریسک‌ها و موارد احتیاط
Requires careful design to avoid introducing new complexities in model selection.
