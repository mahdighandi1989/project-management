---
task_id: d70990e9-ea2a-415d-b7ba-3ff076ea762a
title: Race condition and silent refresh failure in _merge_with_existing_issues
type: bug
priority: high
...
created_at: '2026-06-07T09:29:19.856225+00:00'
updated_at: '2026-06-07T09:34:09.760759+00:00'
target_files:
- backend/app/api/routes/project_journal.py
---

# Race condition and silent refresh failure in _merge_with_existing_issues

## Raw Idea

The `_merge_with_existing_issues` function, which merges new issues with existing ones in a project, attempts to refresh the `project` object from the database to prevent stale data. However, this `db.refresh(project)` call is wrapped in a bare `except Exception as e: logger.warning(...)` block. This means any critical errors during the database refresh (e.g., connection issues, object not found) are silently swallowed, potentially leading to the function operating on stale data. Furthermore, the read-modify-write pattern for the `issues_found` JSON field (which is a string column) is susceptible to race conditions if multiple concurrent requests attempt to update it, leading to data loss or inconsistencies.
...
- `backend/app/api/routes/project_journal.py:223-225` — Silent swallowing of database refresh errors
  ```python
  try:
              db.refresh(project)
...
SQLAlchemy ORM, Database Concurrency, JSON field manipulation

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/models/project.py` — Defines the `Project` model and the `issues_found` field.
- `backend/app/api/routes/project_health.py` — Likely calls `_merge_with_existing_issues` to update project health issues.
- `backend/app/core/database.py` — Database session management.
- `backend/app/services/prompt_helper.py` — `project_journal.py` این فایل را import می‌کند
- `backend/app/models/ai_profile.py` — `project_journal.py` این فایل را import می‌کند
- `backend/app/api/routes/github_import.py` — این فایل `project_journal.py` را import می‌کند (caller)
- `backend/app/api/routes/model_profiles.py` — این فایل `project_journal.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This function is a critical component of the project's issue tracking and health analysis. Data inconsistencies in `issues_found` directly impact the accuracy of project health reports and AI recommendations.
...
- [ ] Database refresh failures are logged as errors and prevent further processing with stale data.
- [ ] The system's behavior under concurrent updates to `project.issues_found` is defined and tested (e.g., using a version column or explicit locking).
- [ ] No `issues_found` data is lost or corrupted due to race conditions.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Replace the bare `except Exception` with more specific database-related exceptions for `db.refresh` and consider re-raising critical errors or implementing a more robust retry mechanism. To mitigate the race condition, explore using optimistic locking (e.g., a version column on the `Project` model) or database-specific atomic JSON update operations if the underlying database supports them. For immediate improvement, ensure the `db.refresh` failure is not silently ignored.
...
```
            ...
            logger.error(f"[Merge Issues] CRITICAL: Database refresh failed for project {project.id}: {e}", exc_info=True)
            raise # Re-raise to prevent operating on potentially stale data
```
...
## 🧪 دستورات اعتبارسنجی
- `Simulate concurrent API calls that update `project.issues_found` for the same project.`
- `Verify data consistency after concurrent updates.`
- `Introduce database connection errors during `db.refresh` and check backend logs.`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential for complex refactoring if a full optimistic locking or atomic JSON update strategy is adopted.
...
1. Database refresh failures are logged as errors and prevent further processing with stale data. _(verify: backend_test)_
2. The system's behavior under concurrent updates to `project.issues_found` is defined and tested (e.g., using a version column or explicit locking). _(verify: backend_test)_
3. No `issues_found` data is lost or corrupted due to race conditions. _(verify: backend_test)_
