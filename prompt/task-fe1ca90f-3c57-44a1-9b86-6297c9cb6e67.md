---
task_id: fe1ca90f-3c57-44a1-9b86-6297c9cb6e67
title: API Key سخت‌کد شده در فایل settings.py برای ذخیره‌سازی در .env
type: security
priority: critical
...
created_at: '2026-05-31T09:05:47.675405+00:00'
updated_at: '2026-06-03T18:37:57.817956+00:00'
---

# API Key سخت‌کد شده در فایل settings.py برای ذخیره‌سازی در .env

## Raw Idea

در فایل `backend/app/api/routes/settings.py`، تابع `update_api_keys` (خط 166) و `update_deploy_keys` (خط 294) پس از ذخیره API Key در دیتابیس و environment، آن را در فایل `.env` نیز می‌نویسند. این عمل باعث می‌شود کلیدهای حساس (مانند `OPENAI_API_KEY`, `GITHUB_TOKEN`) به صورت plaintext در فایل‌های سیستمی ذخیره شوند که در صورت دسترسی غیرمجاز به سرور، می‌توانند به سرقت بروند. همچنین، این فایل `.env` ممکن است به اشتباه در version control commit شود.
...
1. ذخیره API Key در فایل `.env` را به طور کامل حذف کنید. دیتابیس (با رمزنگاری) و environment variables برای اجرا کافی هستند. اگر نیاز به persistence است، از یک سرویس مدیریت راز (مانند HashiCorp Vault یا GitHub Secrets) استفاده کنید.
