---
task_id: 67afb2d8-a46c-495e-944f-066d8da166a9
title: افزودن تست‌های یکپارچه‌سازی برای APIهای تحلیل امنیتی در `backend/app/api/routes/security_analysis.py`
type: security
priority: critical
...
created_at: '2026-06-07T09:28:58.500985+00:00'
updated_at: '2026-06-07T09:31:57.436678+00:00'
target_files:
...
# افزودن تست‌های یکپارچه‌سازی برای APIهای تحلیل امنیتی در `backend/app/api/routes/security_analysis.py`

## Raw Idea

فایل `backend/app/api/routes/security_analysis.py` شامل endpointهایی برای اسکن رازها، تحلیل مجوزها، و تولید گزارش امنیتی جامع است. این قابلیت‌ها مستقیماً با امنیت پروژه سروکار دارند و هرگونه نقص در آن‌ها می‌تواند منجر به آسیب‌پذیری‌های جدی شود. عدم وجود تست‌های یکپارچه‌سازی برای این مسیرها به این معنی است که عملکرد صحیح آن‌ها در سناریوهای مختلف (مثلاً با فایل‌های حاوی راز یا بدون مجوز) تضمین نشده است.
...
- `backend/app/api/routes/security_analysis.py:39-104` — `scan_for_secrets` — endpoint اصلی برای اسکن رازها.
  ```python
  @router.post("/scan-secrets")
  async def scan_for_secrets(...):
  ```
- `backend/app/api/routes/security_analysis.py:138-195` — `analyze_licenses` — endpoint اصلی برای تحلیل مجوزها.
  ```python
  @router.post("/analyze-licenses")
  async def analyze_licenses(...):
  ```
- `backend/app/api/routes/security_analysis.py:201-260` — `generate_full_security_report` — endpoint اصلی برای گزارش امنیتی جامع.
  ```python
...
FastAPI, SQLAlchemy, `security_analyzer` service

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/security_analyzer.py` — سرویس اصلی که منطق تحلیل امنیتی را پیاده‌سازی می‌کند.
- `backend/app/models/project.py` — مدل پروژه که نتایج اسکن امنیتی را ذخیره می‌کند.
- `backend/app/core/database.py` — `security_analysis.py` این فایل را import می‌کند
- `backend/app/services/code_quality_analyzer.py` — `security_analysis.py` این فایل را import می‌کند
- `backend/app/main.py` — این فایل `security_analysis.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این ماژول به سرویس‌های تحلیل امنیتی و مدل پروژه وابسته است. تست‌های یکپارچه‌سازی باید تعاملات بین این کامپوننت‌ها را تأیید کنند.
...
- [ ] فایل `backend/tests/test_security_analysis.py` ایجاد شود.
- [ ] تست‌های یکپارچه‌سازی برای هر سه endpoint اصلی (secrets, licenses, full-report) اضافه شوند.
- [ ] تست‌ها شامل موارد لبه (edge cases) مانند پروژه‌های خالی یا فایل‌های خاص باشند.
- [ ] تست‌ها تأیید کنند که نتایج اسکن به درستی در دیتابیس ذخیره می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست جدید `backend/tests/test_security_analysis.py` و افزودن تست‌های یکپارچه‌سازی برای endpointهای `/scan-secrets`, `/analyze-licenses`, و `/full-report`. این تست‌ها باید سناریوهای موفقیت‌آمیز و ناموفق (مثل عدم وجود پروژه، فایل‌های خاص) را پوشش دهند.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_security_analysis.py`

## ⚠️ ریسک‌ها و موارد احتیاط
ممکن است نیاز به mock کردن سرویس‌های خارجی یا دیتابیس باشد که پیچیدگی تست را افزایش می‌دهد.
...
1. فایل `backend/tests/test_security_analysis.py` ایجاد شود. _(verify: static)_
2. تست‌های یکپارچه‌سازی برای هر سه endpoint اصلی (secrets, licenses, full-report) اضافه شوند. _(verify: backend_test)_
3. تست‌ها شامل موارد لبه (edge cases) مانند پروژه‌های خالی یا فایل‌های خاص باشند. _(verify: backend_test)_
4. تست‌ها تأیید کنند که نتایج اسکن به درستی در دیتابیس ذخیره می‌شوند. _(verify: backend_test)_

## Task Steps

### Step 1: افزودن تست‌های یکپارچه‌سازی برای Endpointهای تحلیل امنیتی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های یکپارچه‌سازی جامع برای endpointهای API تعریف‌شده در فایل `backend/app/api/routes/security_analysis.py` است. تست‌ها باید قابلیت‌های اسکن رازها، تحلیل مجوزها، و تولید گزارش امنیتی جامع را پوشش دهند. این کار شامل آماده‌سازی داده‌های آزمایشی، ارسال درخواست به endpointها، و اعتبارسنجی صحت پاسخ‌ها در سناریوهای مختلف (مانند فایل‌های حاوی راز، فایل‌های بدون مجوز خاص، و تولید گزارش کلی) می‌شود. این مرحله شامل تغییر منطق اصلی endpointهای موجود نیست، بلکه صرفاً تست رفتار خارجی آن‌ها را هدف قرار می‌دهد.
**Excerpt:**
```
فایل `backend/app/api/routes/security_analysis.py` شامل endpointهایی برای اسکن رازها، تحلیل مجوزها، و تولید گزارش امنیتی جامع است. این قابلیت‌ها مستقیماً با امنیت پروژه سروکار دارند و هرگونه نقص در آن‌ها می‌تواند منجر به آسیب‌پذیری‌های جدی شود. عدم وجود تست‌های یکپارچه‌سازی برای این مسیرها به این معنی است که عملکرد صحیح آن‌ها در سناریوهای مختلف (مثلاً با فایل‌های حاوی راز یا بدون مجوز) تضمین نشده است.
```
