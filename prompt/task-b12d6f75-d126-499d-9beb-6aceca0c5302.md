---
task_id: b12d6f75-d126-499d-9beb-6aceca0c5302
title: افزودن تست‌های واحد برای کامپوننت جریان ساخت پروژه در `frontend/src/app/creator/page.tsx`
type: bug
priority: high
...
created_at: '2026-06-07T09:28:59.811168+00:00'
updated_at: '2026-06-07T09:28:59.811171+00:00'
target_files:
- frontend/src/app/creator/page.tsx
---

# افزودن تست‌های واحد برای کامپوننت جریان ساخت پروژه در `frontend/src/app/creator/page.tsx`

## Raw Idea

صفحه `frontend/src/app/creator/page.tsx` هستهٔ قابلیت "AI Creator Engine" است که به کاربران اجازه می‌دهد پروژه‌های جدید را با استفاده از AI ایجاد کنند. این صفحه شامل منطق پیچیده‌ای برای جمع‌آوری ورودی کاربر (نام، توضیحات، نوع پروژه، مدل‌های AI، فایل‌های پیوست، پروژه‌های مرجع)، پیش‌نمایش پرامپت، و فراخوانی API بک‌اند برای ساخت پروژه است. عدم وجود تست‌های واحد برای این جریان کاری حیاتی، ریسک بالایی از خطاهای منطقی، مشکلات اعتبارسنجی ورودی، و شکست در فرایند ایجاد پروژه را به همراه دارد.
...
- `frontend/src/app/creator/page.tsx:1-1344` — `CreatorPage` — این کامپوننت مرکزی برای تولید پروژه است و نیاز به تست جامع برای اطمینان از صحت جریان کاری دارد.
  ```tsx
...
Next.js, React, React Testing Library

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/components/TaskFilePicker.tsx` — کامپوننت برای مدیریت آپلود فایل‌های پیوست.
- `backend/app/api/routes/simple_projects.py` — API بک‌اند برای `idea-to-prompt` و `create_project`.
- `backend/app/services/oversight_service.py` — برای `_resolve_attachments_for_idea` و `auto_register_watched`.

## 🌐 نقشهٔ وابستگی‌ها
این کامپوننت به چندین API بک‌اند و کامپوننت‌های فرانت‌اند دیگر وابسته است. تست‌ها باید این تعاملات را شبیه‌سازی کنند.
...
- [ ] فایل `frontend/src/app/creator/__tests__/page.test.tsx` ایجاد شود.
- [ ] تست‌هایی برای اعتبارسنجی فرم و نمایش خطاها.
- [ ] تست‌هایی برای فرایند `previewPromptFromIdea` و `createProject`.
- [ ] تست‌هایی برای مدیریت `uploadedSessions` و `referenceProjectIds`.
- [ ] تست‌ها باید از mock کردن فراخوانی‌های API و کامپوننت‌های فرزند استفاده کنند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست جدید `frontend/src/app/creator/__tests__/page.test.tsx` با استفاده از React Testing Library. تست‌ها باید شامل: رندر شدن صحیح فرم و عناصر ورودی، اعتبارسنجی ورودی کاربر (مثلاً طول ایده، انتخاب مدل)، تعاملات با `TaskFilePicker` و `Reference Projects` selector، فراخوانی صحیح `idea-to-prompt` و `create` API بک‌اند، نمایش وضعیت پیشرفت و پیام‌های بازخورد، و سناریوهای مختلف (مثلاً با فایل پیوست، با پروژه‌های مرجع).

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `npm test -- frontend/src/app/creator/page.test.tsx`

## ⚠️ ریسک‌ها و موارد احتیاط
خطاهای در این جریان می‌تواند مانع از ایجاد پروژه توسط کاربران شود و تجربه کاربری را به شدت تحت تأثیر قرار دهد.
...
1. فایل `frontend/src/app/creator/__tests__/page.test.tsx` ایجاد شود. _(verify: static)_
2. تست‌هایی برای اعتبارسنجی فرم و نمایش خطاها. _(verify: static)_
3. تست‌هایی برای فرایند `previewPromptFromIdea` و `createProject`. _(verify: static)_
4. تست‌هایی برای مدیریت `uploadedSessions` و `referenceProjectIds`. _(verify: static)_
5. تست‌ها باید از mock کردن فراخوانی‌های API و کامپوننت‌های فرزند استفاده کنند. _(verify: static)_
