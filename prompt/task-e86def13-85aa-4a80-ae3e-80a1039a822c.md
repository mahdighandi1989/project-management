---
task_id: e86def13-85aa-4a80-ae3e-80a1039a822c
title: اعتبارسنجی URL برای جلوگیری از SSRF در GitHub Import
type: security
priority: high
...
created_at: '2026-06-07T09:28:59.471885+00:00'
updated_at: '2026-06-07T09:32:25.044630+00:00'
target_files:
- backend/app/api/routes/github_import.py
---

# اعتبارسنجی URL برای جلوگیری از SSRF در GitHub Import

## Raw Idea

مسیر `backend/app/api/routes/github_import.py` برای `import_repository`، URLهای GitHub را از ورودی کاربر می‌پذیرد. اگر سرویس `github_import` (که در فایل‌های ارائه شده نیست) از یک کلاینت HTTP بدون محافظت کافی در برابر Server-Side Request Forgery (SSRF) استفاده کند، یک مهاجم می‌تواند URLهای مخرب (مانند آدرس‌های IP داخلی یا سرویس‌های ابری حساس) را ارسال کند. این امر می‌تواند منجر به افشای زیرساخت داخلی یا داده‌های حساس شود.
...
- `backend/app/api/routes/github_import.py:95-116` — `request.url` به سرویس `import_repository` ارسال می‌شود و نیاز به اعتبارسنجی دارد.
  ```python
  @router.post("/import")
...
FastAPI, `httpx`/`aiohttp`, URL Parsing

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/requirements.txt` — `httpx` و `aiohttp` کلاینت‌های HTTP هستند که می‌توانند برای SSRF استفاده شوند.
- `backend/app/services/github_import.py` — این سرویس باید منطق اعتبارسنجی URL را پیاده‌سازی کند.
...
## 🌐 نقشهٔ وابستگی‌ها
این اصلاح نیاز به افزودن اعتبارسنجی URL قوی در سرویس `github_import` و احتمالاً پیکربندی کلاینت HTTP برای جلوگیری از SSRF دارد.
...
- [ ] یک تابع `validate_external_url` جدید ایجاد می‌شود که URLها را در برابر دامنه‌های مجاز و محدوده‌های IP خصوصی بررسی می‌کند.
- [ ] سرویس `github_import` از این تابع قبل از انجام درخواست‌های HTTP خارجی استفاده می‌کند.
- [ ] تلاش برای وارد کردن از URLهای غیرمجاز یا URLهایی که به IPهای خصوصی اشاره دارند، منجر به خطای 400 Bad Request می‌شود.
- [ ] تست‌های واحد برای جلوگیری از SSRF اضافه می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک تابع اعتبارسنجی URL سخت‌گیرانه در سرویس `github_import` پیاده‌سازی کنید. این تابع باید فقط URLها را از دامنه‌های مورد اعتماد (مانند `github.com`) مجاز بداند، از تغییر مسیر به دامنه‌های غیرمجاز جلوگیری کند و درخواست‌ها به محدوده‌های IP خصوصی (مانند `127.0.0.1`، `10.0.0.0/8`) را مسدود کند.

## 💡 نمونه‌های قبل/بعد
**قبل از اعتبارسنجی URL (مفهومی در سرویس)**
...
_بعد:_
```
from ...core.security_utils import validate_external_url

if not validate_external_url(url, allowed_domains=["github.com"]):
    raise ValueError("Untrusted URL provided")
response = await client.get(url, headers=headers)
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/security/test_ssrf_prevention.py`
- `curl -X POST http://localhost:8000/github/import -H 'Content-Type: application/json' -d '{"url": "http://127.0.0.1/admin"}' # باید 400 برگرداند`

## ⚠️ ریسک‌ها و موارد احتیاط
اعتبارسنجی بیش از حد سخت‌گیرانه ممکن است دسترسی به GitHub Enterprise یا دامنه‌های سفارشی قانونی را مسدود کند.
...
1. یک تابع `validate_external_url` جدید ایجاد می‌شود که URLها را در برابر دامنه‌های مجاز و محدوده‌های IP خصوصی بررسی می‌کند. _(verify: static)_
2. سرویس `github_import` از این تابع قبل از انجام درخواست‌های HTTP خارجی استفاده می‌کند. _(verify: static)_
3. تلاش برای وارد کردن از URLهای غیرمجاز یا URLهایی که به IPهای خصوصی اشاره دارند، منجر به خطای 400 Bad Request می‌شود. _(verify: api_response)_
4. تست‌های واحد برای جلوگیری از SSRF اضافه می‌شوند. _(verify: backend_test)_
