---
task_id: 0242c3a9-7bde-4988-b1ab-c04cb21ec684
title: استفاده از SessionLocal مستقیم در endpointهای API بدون مدیریت صحیح lifecycle
type: bug
priority: high
...
created_at: '2026-05-31T09:07:31.576959+00:00'
updated_at: '2026-06-01T17:07:18.520620+00:00'
target_files:
...
# استفاده از SessionLocal مستقیم در endpointهای API بدون مدیریت صحیح lifecycle

## Raw Idea

در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است. این الگو باعث ایجاد connection leak در صورت بروز exception می‌شود، زیرا بستن session در finally تضمین نمی‌شود. همچنین در analysis.py خط 117، یک session برای deep_analyzer ساخته می‌شود که در صورت خطا در run_analysis_task، بسته نمی‌شود. این مشکل در oversight.py نیز با الگوی مشابه دیده می‌شود.
...
- `backend/app/api/routes/analysis.py:117-124` — `run_analysis_task` — SessionLocal() بدون مدیریت lifecycle - در صورت exception بسته نمی‌شود
  ```python
  ...
  ```
- `backend/app/api/routes/analysis.py:280-290` — `get_analysis_reports` — الگوی دستی - باید با Depends(get_db) جایگزین شود
  ```python
  db = SessionLocal()
  try:
      ...
      db.close()
  ```
- `backend/app/api/routes/github_import.py:137-155` — `import_repository` — الگوی دستی در middle of function - می‌تواند با get_db یکپارچه شود
  ```python
...
- `backend/app/core/database.py` (سطر 1) — تعریف get_db و SessionLocal
- `backend/app/api/routes/project_health.py` (سطر 137) — الگوی مشابه در خطوط 137-138
- `backend/app/api/routes/oversight.py` (سطر 314) — الگوی مشابه در خطوط 314-325
...
## 🌐 نقشهٔ وابستگی‌ها
این الگو در ۵+ فایل route تکرار شده است. اصلاح نیازمند تغییر در تمام endpointهایی است که مستقیماً SessionLocal() می‌سازند.
...
- [ ] هیچ endpointای مستقیماً SessionLocal() نمی‌سازد
- [ ] تمام endpointها از Depends(get_db) استفاده می‌کنند
- [ ] background tasks از context manager برای session استفاده می‌کنند
- [ ] هیچ connection leak در لاگ
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. جایگزینی تمام استفاده‌های مستقیم از SessionLocal() با وابستگی get_db که FastAPI به صورت خودکار session را مدیریت می‌کند. برای مواردی که نیاز به session در background task یا callback داریم، از context manager یا try/finally مناسب استفاده شود.
...
async with SessionLocal() as session:
    deep_analyzer = DeepAnalysisService(db_session=session)
```
...
1. هیچ endpointای مستقیماً SessionLocal() نمی‌سازد _(verify: static)_
2. تمام endpointها از Depends(get_db) استفاده می‌کنند _(verify: static)_
3. background tasks از context manager برای session استفاده می‌کنند _(verify: static)_
4. هیچ connection leak در لاگ _(verify: manual_only)_

## Task Steps

### Step 1: رفع connection leak در analysis.py خطوط 117-124: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های analysis.py از خط 117 تا 124 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد تا در صورت بروز exception، session بسته شود. خارج از این مرحله: سایر خطوط analysis.py، فایل‌های دیگر، تغییر logic business. نکته حیاتی: session ساخته شده برای deep_analyzer در خط 117 باید حتماً در finally بسته شود.
**Excerpt:**
```
در analysis.py خط 117، یک session برای deep_analyzer ساخته می‌شود که در صورت خطا در run_analysis_task، بسته نمی‌شود.
```

### Step 2: رفع connection leak در analysis.py خطوط 280-290: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های analysis.py از خط 280 تا 290 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد. خارج از این مرحله: سایر خطوط analysis.py، فایل‌های دیگر. نکته حیاتی: اطمینان از بسته شدن session حتی در صورت exception.
**Excerpt:**
```
در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است.
```

### Step 3: رفع connection leak در analysis.py خطوط 296-306: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های analysis.py از خط 296 تا 306 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد. خارج از این مرحله: سایر خطوط analysis.py، فایل‌های دیگر. نکته حیاتی: اطمینان از بسته شدن session حتی در صورت exception.
**Excerpt:**
```
در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است.
```

### Step 4: رفع connection leak در analysis.py خطوط 312-325: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های analysis.py از خط 312 تا 325 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد. خارج از این مرحله: سایر خطوط analysis.py، فایل‌های دیگر. نکته حیاتی: اطمینان از بسته شدن session حتی در صورت exception.
**Excerpt:**
```
در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است.
```

### Step 5: رفع connection leak در analysis.py خطوط 349-352: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های analysis.py از خط 349 تا 352 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد. خارج از این مرحله: سایر خطوط analysis.py، فایل‌های دیگر. نکته حیاتی: اطمینان از بسته شدن session حتی در صورت exception.
**Excerpt:**
```
در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است.
```

### Step 6: رفع connection leak در project_health.py خطوط 137-138: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های project_health.py از خط 137 تا 138 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد. خارج از این مرحله: سایر خطوط project_health.py، فایل‌های دیگر. نکته حیاتی: اطمینان از بسته شدن session حتی در صورت exception.
**Excerpt:**
```
در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است.
```

### Step 7: رفع connection leak در project_health.py خطوط 203-226: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های project_health.py از خط 203 تا 226 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد. خارج از این مرحله: سایر خطوط project_health.py، فایل‌های دیگر. نکته حیاتی: اطمینان از بسته شدن session حتی در صورت exception.
**Excerpt:**
```
در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است.
```

### Step 8: رفع connection leak در github_import.py خطوط 137-155: جایگزینی SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازنویسی بخش‌های github_import.py از خط 137 تا 155 است که در آن SessionLocal() به صورت مستقیم ساخته می‌شود. باید session را با استفاده از context manager (with statement) یا try/finally مدیریت کرد. خارج از این مرحله: سایر خطوط github_import.py، فایل‌های دیگر. نکته حیاتی: اطمینان از بسته شدن session حتی در صورت exception.
**Excerpt:**
```
در چندین endpoint (analysis.py خطوط 117-124, 280-290, 296-306, 312-325, 349-352, project_health.py خطوط 137-138, 203-226, github_import.py خطوط 137-155) از SessionLocal() به صورت مستقیم و دستی استفاده شده است.
```

### Step 9: رفع connection leak در oversight.py: جایگزینی الگوی مشابه SessionLocal() مستقیم با context manager
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و بازنویسی فایل oversight.py است که کاربر اشاره کرده الگوی مشابهی با analysis.py دارد. باید تمام موارد استفاده از SessionLocal() مستقیم در oversight.py را شناسایی و با context manager یا try/finally جایگزین کرد. خارج از این مرحله: فایل‌های دیگر. نکته حیاتی: کاربر صراحتاً گفته 'این مشکل در oversight.py نیز با الگوی مشابه دیده می‌شود'، بنابراین باید کل فایل بررسی شود.
**Excerpt:**
```
این مشکل در oversight.py نیز با الگوی مشابه دیده می‌شود.
```
