---
task_id: c83d5e16-bcc0-4830-be4f-c9a40a8a300d
title: Silent swallowing of file reading exceptions in analysis stream
type: bug
priority: high
...
created_at: '2026-06-07T09:29:18.775952+00:00'
updated_at: '2026-06-07T09:33:28.439817+00:00'
target_files:
...
# Silent swallowing of file reading exceptions in analysis stream

## Raw Idea

In the `run_analysis_stream` endpoint, when attempting to read `README.md`, `roadmap_path`, or other project files, the `try...except` blocks use a bare `except: pass`. This silently swallows all exceptions, including `FileNotFoundError`, `IOError`, or `UnicodeDecodeError`. This can lead to the analysis proceeding with incomplete or incorrect file content without any indication to the user or logs, making debugging extremely difficult and potentially yielding inaccurate analysis reports.
...
- `backend/app/api/routes/analysis.py:143` — Exception handler for README.md reading
  ```python
  except:
                              pass
  ```
- `backend/app/api/routes/analysis.py:151` — Exception handler for roadmap_path reading
  ```python
  except:
                          pass
  ```
- `backend/app/api/routes/analysis.py:179` — Exception handler for general file collection
  ```python
  except:
                                  pass
  ```
...
FastAPI, Python file I/O, Server-Sent Events (SSE)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/logging_utils.py` — Logging utility for structured error reporting.
- `frontend/src/app/analysis/page.tsx` — Frontend component that consumes the analysis stream and could display warnings.
...
## 🌐 نقشهٔ وابستگی‌ها
This issue affects the core project analysis streaming functionality, which is a primary feature for users to get real-time feedback on project health.
...
- [ ] File reading errors are logged with full traceback on the backend.
- [ ] The `run_analysis_stream` endpoint does not crash due to file reading errors.
- [ ] Frontend can potentially display a warning if a file cannot be read (future enhancement).
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Replace bare `except: pass` with specific exception handling (e.g., `except (IOError, UnicodeDecodeError) as e:`) and log the specific error using `logger.error` with `exc_info=True`. Consider sending a non-critical progress event to the frontend to indicate a file read warning.

## 💡 نمونه‌های قبل/بعد
**Improved exception handling for file read**
...
```
                            ...
                            logger.warning(f"Could not read {readme_path}: {e}", exc_info=True)
                            # Optionally, send a non-critical progress event to frontend
```
...
## 🧪 دستورات اعتبارسنجی
- `Create a test project with unreadable files (e.g., permission denied, invalid encoding) and run `/api/analysis/run-stream`.`
- `Verify error logs on the backend.`

## ⚠️ ریسک‌ها و موارد احتیاط
Minimal, primarily involves adding more specific exception types and logging.
...
1. File reading errors are logged with full traceback on the backend. _(verify: backend_test)_
2. The `run_analysis_stream` endpoint does not crash due to file reading errors. _(verify: api_response)_
3. Frontend can potentially display a warning if a file cannot be read (future enhancement). _(verify: manual_only)_
