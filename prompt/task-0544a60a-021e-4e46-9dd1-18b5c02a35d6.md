---
task_id: 0544a60a-021e-4e46-9dd1-18b5c02a35d6
title: اعمال احراز هویت مدیر برای مسیرهای به‌روزرسانی کلید API
type: security
priority: high
...
created_at: '2026-06-07T09:28:59.135496+00:00'
updated_at: '2026-06-07T09:32:14.390165+00:00'
target_files:
- backend/app/api/routes/settings.py
- backend/app/api/routes/settings.py
---

# اعمال احراز هویت مدیر برای مسیرهای به‌روزرسانی کلید API

## Raw Idea

مسیرهای `PUT /settings/api-keys` و `PUT /settings/deploy-keys` در `backend/app/api/routes/settings.py` به کاربران اجازه می‌دهند تا کلیدهای API حساس (مانند OpenAI، Claude، Render، GitHub) را به‌روزرسانی کنند. این مسیرها در حال حاضر فاقد بررسی احراز هویت صریح هستند. یک مهاجم با دسترسی به بک‌اند می‌تواند این کلیدها را تغییر دهد و کنترل کامل بر قابلیت‌های AI و یکپارچه‌سازی‌های خارجی سیستم را به دست آورد.
...
- `backend/app/api/routes/settings.py:165-166` — مسیر `update_api_keys` فاقد Dependency احراز هویت است.
  ```python
  @router.put("/api-keys")
  async def update_api_keys(request: UpdateApiKeysRequest, db: Session = Depends(get_db)):
  ```
- `backend/app/api/routes/settings.py:293-294` — مسیر `update_deploy_keys` فاقد Dependency احراز هویت است.
  ```python
...
FastAPI Dependencies, Environment Variables

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/external_prompts.py` — تعریف Dependency `require_admin_token` در این فایل قرار دارد.
- `backend/app/core/config.py` — متغیر محیطی `ADMIN_TOKEN` در اینجا پیکربندی می‌شود.
...
## 🌐 نقشهٔ وابستگی‌ها
این یک اصلاح امنیتی حیاتی برای محافظت از اعتبارنامه‌های حساس است و به پیکربندی امن `ADMIN_TOKEN` متکی است.
...
- [ ] مسیرهای `update_api_keys` و `update_deploy_keys` توسط `require_admin_token` محافظت می‌شوند.
- [ ] درخواست‌های بدون `X-Admin-Token` معتبر به این مسیرها، خطای 401 Unauthorized برمی‌گردانند.
- [ ] مستندات مربوط به استقرار شامل دستورالعمل‌هایی برای پیکربندی `ADMIN_TOKEN` می‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. وابستگی `require_admin_token` (که در `backend/app/api/routes/external_prompts.py` تعریف شده) را به مسیرهای `update_api_keys` و `update_deploy_keys` اعمال کنید. این امر تضمین می‌کند که فقط درخواست‌هایی با `X-Admin-Token` معتبر می‌توانند این تنظیمات حیاتی را تغییر دهند.

## 💡 نمونه‌های قبل/بعد
**قبل از اعمال احراز هویت**
...
_بعد:_
```
from ...api.routes.external_prompts import require_admin_token

@router.put("/api-keys", dependencies=[Depends(require_admin_token)])
async def update_api_keys(request: UpdateApiKeysRequest, db: Session = Depends(get_db)):
```
...
## 🧪 دستورات اعتبارسنجی
- `curl -X PUT http://localhost:8000/settings/api-keys -H 'Content-Type: application/json' -d '{"openai": "new_key"}' # باید 401 برگرداند`
- `curl -X PUT http://localhost:8000/settings/api-keys -H 'Content-Type: application/json' -H 'X-Admin-Token: {valid_admin_token}' -d '{"openai": "new_key"}' # باید موفق باشد`

## ⚠️ ریسک‌ها و موارد احتیاط
در صورت عدم مدیریت صحیح `ADMIN_TOKEN`، این آسیب‌پذیری می‌تواند باقی بماند.
...
1. مسیرهای `update_api_keys` و `update_deploy_keys` توسط `require_admin_token` محافظت می‌شوند. _(verify: static)_
2. درخواست‌های بدون `X-Admin-Token` معتبر به این مسیرها، خطای 401 Unauthorized برمی‌گردانند. _(verify: api_response)_
3. مستندات مربوط به استقرار شامل دستورالعمل‌هایی برای پیکربندی `ADMIN_TOKEN` می‌شود. _(verify: manual_only)_
