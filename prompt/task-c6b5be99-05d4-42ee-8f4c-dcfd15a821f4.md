---
task_id: c6b5be99-05d4-42ee-8f4c-dcfd15a821f4
title: claim/complete/progress endpointهای External Prompts تحت قفل service._lock هستند اما _emit_runner_notification با asyncio.create_task خارج از lock اجرا می‌شود و خطای آن swallowed است
type: bug
priority: medium
...
created_at: '2026-06-14T09:41:08.126250+00:00'
updated_at: '2026-06-14T09:42:56.715713+00:00'
target_files:
- backend/app/api/routes/external_prompts.py
- backend/app/api/routes/external_prompts.py
---

# claim/complete/progress endpointهای External Prompts تحت قفل service._lock هستند اما _emit_runner_notification با asyncio.create_task خارج از lock اجرا می‌شود و خطای آن swallowed است

## Raw Idea

در external_prompts.py، تابع _emit_runner_notification با asyncio.create_task(notification_service.notify_event(...)) یک fire-and-forget task می‌سازد. اگر این task شکست بخورد (مثلاً Telegram unreachable)، هیچ‌جا await نمی‌شود و exception آن به‌صورت 'Task exception was never retrieved' در event loop گم می‌شود — نه در logger پروژه ثبت می‌شود. همچنین در complete_prompt، پس از آزاد شدن service._lock و فراخوانی notification، منطق verify-then-chain (خط 394 به بعد) شروع می‌شود؛ اگر بین آزاد شدن lock و شروع verify، یک workflow دیگر همان watched را trigger کند، lock مورد ادعا در کامنت (claude_runner_verifying_task_id) هنوز ست نشده و امکان double-trigger وجود دارد.
...
- `backend/app/api/routes/external_prompts.py:73-84` — `_emit_runner_notification` — create_task بدون done_callback — exception داخل task گم می‌شود
  ```python
  asyncio.create_task(
      notification_service.notify_event(
          event, msg, subject=head, priority="low", ...
      )
  )
  except Exception as _e:
      logger.debug(f"_emit_runner_notification skipped: {_e}")
  ```
- `backend/app/api/routes/external_prompts.py:387-400`
...
- `backend/app/services/oversight_service.py` — `external_prompts.py` این فایل را import می‌کند
- `backend/app/services/prompt_github_sync.py` — `external_prompts.py` این فایل را import می‌کند
- `backend/app/services/notification_service.py` — `external_prompts.py` این فایل را import می‌کند
- `backend/app/services/oversight_verifier.py` — `external_prompts.py` این فایل را import می‌کند
- `backend/app/api/routes/oversight.py` — این فایل `external_prompts.py` را import می‌کند (caller)
- `backend/app/main.py` — این فایل `external_prompts.py` را import می‌کند (caller)
...
1. task ساخته‌شده در _emit_runner_notification را در یک wrapper با try/except قرار بده که خطا را logger.warning کند (یا از add_done_callback برای retrieve exception استفاده کن). برای double-trigger، ست کردن claude_runner_verifying_task_id باید داخل همان service._lock بلاک complete_prompt انجام شود نه بعد از آزاد شدن lock.
...
1. اعمال تغییر بدون شکستن تست‌های موجود _(verify: static)_
