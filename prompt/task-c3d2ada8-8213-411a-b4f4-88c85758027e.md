---
task_id: c3d2ada8-8213-411a-b4f4-88c85758027e
title: دو سیستم تحلیل پروژه (analysis.py و project_health.py) موازی و تکراری هستند که project_health خود deprecated اعلام شده
type: refactor
priority: high
...
created_at: '2026-06-14T09:39:52.583698+00:00'
updated_at: '2026-06-14T09:42:20.604333+00:00'
target_files:
...
# دو سیستم تحلیل پروژه (analysis.py و project_health.py) موازی و تکراری هستند که project_health خود deprecated اعلام شده

## Raw Idea

دو مسیر کاملاً موازی برای تحلیل پروژه وجود دارد: (۱) analysis.py با prefix `/analysis` که از DeepAnalysisService و project_analyzer استفاده می‌کند و AnalysisReport می‌سازد، و (۲) project_health.py با prefix `/api/projects` که از project_health_analyzer استفاده می‌کند. خود project_health.py در dependency `deprecated_health_endpoint` رسماً deprecated اعلام شده و کاربران به `/oversight` هدایت می‌شوند (خط 64-81). اما analysis.py هیچ نشانه‌ای از deprecation ندارد و همچنان مدل‌های profile را پر می‌کند. در عمل سه سیستم تحلیل cross-cutting وجود دارد (analysis، project_health، oversight/scan_v5) که هر کدام منطق جمع‌آوری فایل، فراخوانی AI و امتیازدهی را تکرار می‌کنند. این duplicated logic باعث drift در منطق امتیازدهی و سردرگمی کاربر می‌شود.
...
- `backend/app/api/routes/analysis.py:23-23` — `router` — این router هیچ deprecation marker ندارد در حالی که project_health که همان کار را می‌کند deprecated است
  ```python
  router = APIRouter(prefix="/analysis", tags=["Analysis"])
  ```
- `backend/app/api/routes/project_health.py:64-81` — `deprecated_health_endpoint` — الگوی deprecation که باید روی analysis.py هم اعمال شود
  ```python
  ...
  ```
- `backend/app/api/routes/analysis.py:152-181` — `generate_events.run_analysis_task` — منطق جمع‌آوری فایل که در scan_v5 و project_health هم تکرار شده
  ```python
...
FastAPI + SQLAlchemy + SSE streaming + multi-provider AI

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/deep_analysis_service.py` — موتور analysis.py که موازی project_health_analyzer است
- `backend/app/services/project_health_analyzer.py` — موتور project_health که deprecated شده
- `backend/app/services/scan_sections.py` — محل مناسب برای helper جمع‌آوری فایل مشترک
- `docs/ARCHITECTURE.md` — باید مسیر canonical تحلیل را مستند کند
...
## 🌐 نقشهٔ وابستگی‌ها
analysis.py از project_analyzer، model_profiler و DeepAnalysisService استفاده می‌کند؛ project_health.py از project_health_analyzer و همان model_profiler. هر دو AnalysisReport/health_scores را در DB می‌نویسند. تثبیت یک مسیر روی پروفایل مدل‌ها (model_profiler که 10 فایل آن را import می‌کنند) اثر می‌گذارد.
...
- [ ] endpoint های /analysis/* هدر Deprecation:true و Link به successor برمی‌گردانند
- [ ] ARCHITECTURE.md یک بخش 'Project Analysis Pipelines' دارد که سه سیستم را توضیح و canonical را مشخص می‌کند
- [ ] منطق جمع‌آوری فایل (supported_extensions + excluded dirs) در یک helper مشترک متمرکز شده و analysis.py از آن استفاده می‌کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک plan مهاجرت رسمی تعریف شود: analysis.py هم مثل project_health.py با dependency deprecation علامت‌گذاری شود و در ARCHITECTURE.md مسیر canonical (oversight/scan_v5) به‌عنوان single source of truth برای تحلیل ثبت شود. منطق جمع‌آوری فایل (os.walk + supported_extensions در analysis.py خط 152-181) که در چند جا تکرار شده، به یک helper مشترک در scan_sections.py منتقل شود.

## 💡 نمونه‌های قبل/بعد
**افزودن deprecation به analysis router**
...
_بعد:_
```
router = APIRouter(prefix="/analysis", tags=["Analysis"], dependencies=[Depends(deprecated_analysis_endpoint)])
```
...
## 🧪 دستورات اعتبارسنجی
- `curl -I http://localhost:8000/api/analysis/reports | grep -i deprecation`
- `grep -rn 'supported_extensions' backend/app/services/ backend/app/api/routes/`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر فرانت‌اند analysis/page.tsx همچنان به endpoint های قدیمی متکی باشد، deprecation header نباید رفتار را بشکند فقط هشدار بدهد
...
1. endpoint های /analysis/* هدر Deprecation:true و Link به successor برمی‌گردانند _(verify: static)_
2. ARCHITECTURE.md یک بخش 'Project Analysis Pipelines' دارد که سه سیستم را توضیح و canonical را مشخص می‌کند _(verify: static)_
3. منطق جمع‌آوری فایل (supported_extensions + excluded dirs) در یک helper مشترک متمرکز شده و analysis.py از آن استفاده می‌کند _(verify: static)_
