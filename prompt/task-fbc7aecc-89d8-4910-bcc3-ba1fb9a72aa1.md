---
task_id: fbc7aecc-89d8-4910-bcc3-ba1fb9a72aa1
title: حلقهٔ SSE تحلیل، خطای cancel شدن task را در‌حالت timeout مدیریت نمی‌کند و ممکن است event 'done' را از دست بدهد
type: bug
priority: high
...
created_at: '2026-06-14T09:41:04.451063+00:00'
updated_at: '2026-06-14T09:42:38.973613+00:00'
target_files:
- backend/app/api/routes/analysis.py
- backend/app/api/routes/analysis.py
---

# حلقهٔ SSE تحلیل، خطای cancel شدن task را در‌حالت timeout مدیریت نمی‌کند و ممکن است event 'done' را از دست بدهد

## Raw Idea

در run_analysis_stream، حلقهٔ تولید رویداد SSE با timeout=2.0 ثانیه روی progress_queue.get() کار می‌کند. در شاخهٔ TimeoutError، اگر analysis_task.done() باشد، حلقه با break تمام می‌شود — اما هیچ تلاشی برای drain کردن باقی‌ماندهٔ صف انجام نمی‌شود. این یعنی اگر run_analysis_task در finally خود event 'done' را در صف گذاشته باشد ولی قبل از آن task به وضعیت done رسیده باشد، حلقه قبل از خواندن 'done' خارج می‌شود و کلاینت هرگز سیگنال اتمام به‌همراه نتیجه را دریافت نمی‌کند. این یک race condition بین done شدن task و پردازش صف است. علاوه‌بر این، خطای داخل run_analysis_task فقط در صف ثبت می‌شود اما اگر cancel شود (در finally بیرونی) ممکن است CancelledError بدون log رد شود.
...
- `backend/app/api/routes/analysis.py:243-257` — `generate_events` — حلقهٔ اصلی SSE — قبل از break باید صف drain شود
  ```python
  ...
  ```
- `backend/app/api/routes/analysis.py:207-212` — `run_analysis_task.finally` — event done که ممکن است هرگز خوانده نشود
  ```python
  finally:
...
FastAPI StreamingResponse (SSE) + asyncio.Queue + Next.js fetch ReadableStream reader

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/app/analysis/page.tsx` (سطر 242) — کلاینت SSE که منتظر event 'done' یا 'analysis_completed' است
- `backend/app/services/deep_analysis_service.py` — سرویس تحلیل که progress_callback را call می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
این endpoint تنها مسیر استریم تحلیل است و توسط صفحهٔ analysis فرانت‌اند مصرف می‌شود؛ از دست رفتن event done باعث می‌شود UI تا ابد در حالت running باقی بماند یا setTimeout 3 ثانیه‌ای آن را پاک کند بدون نمایش نتیجه.
...
- [ ] وقتی تحلیل سریع تمام می‌شود (قبل از اولین timeout)، کلاینت همیشه event 'done' را دریافت می‌کند
- [ ] صف progress_queue قبل از خروج از حلقه به‌طور کامل drain می‌شود
- [ ] تست واحد با mock که task سریع done می‌شود تأیید کند آخرین event با result ارسال شده
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. در شاخهٔ TimeoutError پیش از break، صف را به‌صورت non-blocking drain کن (while not progress_queue.empty(): yield) و سپس به دنبال آن اگر task done است، یک‌بار دیگر با get_nowait تلاش کن. همچنین در finally بیرونی پس از cancel، با try/except CancelledError منتظر اتمام task بمان و log کن.

## 💡 نمونه‌های قبل/بعد
**drain صف قبل از break**
...
```
    ...
    if analysis_task.done():
        while not progress_queue.empty():
            pd = progress_queue.get_nowait()
            et = pd.get('event', 'progress')
            yield f"event: {et}\ndata: {json.dumps(pd, ensure_ascii=False, default=str)}\n\n"
        break
```
...
drain اشتباه ممکن است event دوبار ارسال شود؛ باید پس از drain حلقه قطعاً break کند
...
1. وقتی تحلیل سریع تمام می‌شود (قبل از اولین timeout)، کلاینت همیشه event 'done' را دریافت می‌کند _(verify: static)_
2. صف progress_queue قبل از خروج از حلقه به‌طور کامل drain می‌شود _(verify: static)_
3. تست واحد با mock که task سریع done می‌شود تأیید کند آخرین event با result ارسال شده _(verify: static)_
