---
task_id: 9d952b2f-671e-40aa-a6ec-de67ad5ed381
title: '[منطق] عدم تطابق بین expected_inputs و interacts_with در ai_log'
type: logic_audit
priority: high
...
در ai_log، expected_inputs شامل فیلدهایی مانند 'response', 'tokens', 'cost', 'latency' است اما در interacts_with به هیچ سرویس AI یا مدلی اشاره نشده که این داده‌ها را تأمین کند. فقط به database و کتابخانه‌های عمومی اشاره شده.

## 💥 پیامد (impact)
لاگ‌ها ممکن است ناقص یا با داده‌های پیش‌فرض ذخیره شوند چون مشخص نیست چه کسی باید این مقادیر را محاسبه و ارسال کند.

## 🛠 پیشنهاد رفع اولیه
interacts_with را به‌روز کنید تا شامل ai_manager یا سرویس‌های خاص (openai_service, claude_service) شود که مسئول محاسبه tokens, cost و latency هستند.
...
### Step 1: بررسی و به‌روزرسانی `interacts_with` در `ai_log` برای افزودن `ai_manager` و سرویس‌های AI
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی فایل‌های مرتبط با `ai_log` و `ai_llm` pipeline برای شناسایی ساختار فعلی `interacts_with` و به‌روزرسانی آن است. باید `interacts_with` را طوری اصلاح کرد که شامل `ai_manager`، `openai_service` و `claude_service` به عنوان تأمین‌کنندگان داده‌های `response`, `tokens`, `cost`, `latency` باشد. خارج از این مرحله: تغییر در منطق محاسبه tokens/cost/latency (فقط ارجاع به سرویس‌ها)، تغییر در ساختار `expected_inputs`، یا تغییر در database schema. نکته حیاتی: باید اطمینان حاصل شود که `interacts_with` با ساختار واقعی سرویس‌ها همخوانی دارد و coherence issue برطرف می‌شود.
**Excerpt:**
```
در ai_log، expected_inputs شامل فیلدهایی مانند 'response', 'tokens', 'cost', 'latency' است اما در interacts_with به هیچ سرویس AI یا مدلی اشاره نشده که این داده‌ها را تأمین کند. فقط به database و کتابخانه‌های عمومی اشاره شده.
