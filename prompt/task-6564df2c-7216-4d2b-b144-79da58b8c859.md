---
task_id: 6564df2c-7216-4d2b-b144-79da58b8c859
title: پیاده‌سازی HTML Sanitization برای محتوای تولید شده توسط کاربر/AI در فرانت‌اند
type: security
priority: medium
...
created_at: '2026-06-07T09:29:38.107570+00:00'
updated_at: '2026-06-07T09:34:20.964681+00:00'
target_files:
...
# پیاده‌سازی HTML Sanitization برای محتوای تولید شده توسط کاربر/AI در فرانت‌اند

## Raw Idea

چندین مسیر بک‌اند، محتوای تولید شده توسط کاربر یا AI (مانند پرامپت‌ها، پاسخ‌ها، خلاصه‌ها، جزئیات) را به صورت متن ذخیره می‌کنند. اگر این محتوا در فرانت‌اند بدون پاکسازی مناسب HTML رندر شود، یک آسیب‌پذیری Cross-Site Scripting (XSS) ایجاد می‌کند. مهاجم می‌تواند اسکریپت‌های مخرب را تزریق کند که هنگام مشاهده محتوای آسیب‌دیده (مثلاً در ژورنال پروژه یا گزارش‌های تحلیل) در مرورگر سایر کاربران اجرا می‌شوند.
...
- `frontend/src/app/debate/page.tsx:30-31` — `prompt` و `content` در مناظرات و سایر بخش‌ها بدون پاکسازی رندر می‌شوند.
  ```tsx
  interface DebateSummary {
    id: string;
    prompt: string;
  ```
- `frontend/src/app/archive/page.tsx:8-11` — `prompt` و `description` در آرشیو مناظرات و پروژه‌ها بدون پاکسازی رندر می‌شوند.
  ```tsx
  interface DebateItem {
    ...
    mode: string;
  ```
- `frontend/src/app/knowledge-center/page.tsx:7-19` — `title` و `summary` در مرکز دانش بدون پاکسازی رندر می‌شوند.
  ```tsx
  interface Entry {
    id: string;
...
Next.js, React, HTML Rendering, XSS Prevention

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/package.json` — نیاز به افزودن یک کتابخانه پاکسازی HTML مانند `dompurify`.
- `frontend/src/components/MarkdownRenderer.tsx` — یک مکان مرکزی برای اعمال پاکسازی HTML در صورت وجود.

## 🌐 نقشهٔ وابستگی‌ها
این یک اصلاح سمت فرانت‌اند است که نیاز به یکپارچه‌سازی کتابخانه پاکسازی HTML و اعمال مداوم آن در تمام نقاط نمایش محتوای تولید شده دارد.
...
- [ ] یک کتابخانه پاکسازی HTML سمت کلاینت (مانند `dompurify`) در فرانت‌اند یکپارچه شده است.
- [ ] تمام کامپوننت‌هایی که متن تولید شده توسط کاربر/AI را رندر می‌کنند، این پاکسازی را اعمال می‌کنند.
- [ ] Payloadهای XSS تزریق شده در فیلدهای بک‌اند در مرورگر اجرا نمی‌شوند.
- [ ] تست‌های واحد/E2E جدید برای تأیید جلوگیری از XSS اضافه می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. در فرانت‌اند، از یک کتابخانه پاکسازی HTML (مانند `dompurify`) استفاده کنید یا یک کامپوننت React ایجاد کنید که به طور خودکار HTML را قبل از رندر کردن هر متن کنترل شده توسط کاربر یا AI پاکسازی می‌کند. اطمینان حاصل کنید که تمام کامپوننت‌هایی که چنین محتوایی را نمایش می‌دهند (به ویژه رندرکننده‌های Markdown) این پاکسازی را اعمال می‌کنند.

## 💡 نمونه‌های قبل/بعد
**قبل از پاکسازی HTML (در React)**

_قبل:_
```
<ReactMarkdown>{debate.prompt}</ReactMarkdown>
```
...
<div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(debate.prompt) }} />
```
...
- `درج `<script>alert('XSS')</script>` در یک پرامپت یا توضیحات و مشاهده اینکه آیا در UI اجرا می‌شود یا خیر.`

## ⚠️ ریسک‌ها و موارد احتیاط
پاکسازی بیش از حد ممکن است قالب‌بندی HTML قانونی را حذف کند.
...
1. یک کتابخانه پاکسازی HTML سمت کلاینت (مانند `dompurify`) در فرانت‌اند یکپارچه شده است. _(verify: static)_
2. تمام کامپوننت‌هایی که متن تولید شده توسط کاربر/AI را رندر می‌کنند، این پاکسازی را اعمال می‌کنند. _(verify: static)_
3. Payloadهای XSS تزریق شده در فیلدهای بک‌اند در مرورگر اجرا نمی‌شوند. _(verify: ui_interaction)_
4. تست‌های واحد/E2E جدید برای تأیید جلوگیری از XSS اضافه می‌شوند. _(verify: backend_test)_

## Task Steps

### Step 1: پیاده‌سازی پاکسازی HTML برای محتوای تولید شده توسط کاربر/AI در فرانت‌اند
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی تمام نقاط رندرینگ فرانت‌اند است که در آن‌ها محتوای متنی تولید شده توسط کاربر یا AI (مانند پرامپت‌ها، پاسخ‌ها، خلاصه‌ها، یا جزئیات) نمایش داده می‌شود. وظیفه اصلی، ادغام یک کتابخانه قوی پاکسازی HTML یا منطق پاکسازی سفارشی است تا اطمینان حاصل شود که هرگونه تگ HTML یا اسکریپت بالقوه مخرب قبل از رندر شدن محتوا در مرورگر، خنثی شود. این شامل محتوایی است که از مسیرهای بک‌اند ذخیره‌کننده این داده‌ها بازیابی می‌شود. این مرحله به اعتبارسنجی ورودی در بک‌اند یا پاکسازی در سطح پایگاه داده نمی‌پردازد و صرفاً بر جنبه رندرینگ فرانت‌اند برای جلوگیری از XSS تمرکز دارد.
**Excerpt:**
```
چندین مسیر بک‌اند، محتوای تولید شده توسط کاربر یا AI (مانند پرامپت‌ها، پاسخ‌ها، خلاصه‌ها، جزئیات) را به صورت متن ذخیره می‌کنند. اگر این محتوا در فرانت‌اند بدون پاکسازی مناسب HTML رندر شود، یک آسیب‌پذیری Cross-Site Scripting (XSS) ایجاد می‌کند. مهاجم می‌تواند اسکریپت‌های مخرب را تزریق کند که هنگام مشاهده محتوای آسیب‌دیده (مثلاً در ژورنال پروژه یا گزارش‌های تحلیل) در مرورگر سایر کاربران اجرا می‌شوند.
```
