---
task_id: da313e05-0600-495e-88be-1306dacef29e
title: ناهماهنگی fallback پروفایل مدل‌ها بین دو endpoint مختلف (/api/models/profiles و /api/models/profiles از model_profiles.py)
type: bug
priority: high
...
created_at: '2026-06-14T09:39:53.126759+00:00'
updated_at: '2026-06-14T09:42:25.771877+00:00'
target_files:
...
# ناهماهنگی fallback پروفایل مدل‌ها بین دو endpoint مختلف (/api/models/profiles و /api/models/profiles از model_profiles.py)

## Raw Idea

دو endpoint با مسیر یکسان `/api/models/profiles` در دو فایل مختلف تعریف شده‌اند که رفتار fallback متفاوت دارند. در models.py (خط 321-341) `get_model_profiles` از `profiler.get_all_profiles()` می‌خواند و فرمت `{success, profiles, count, is_fallback}` برمی‌گرداند. در model_profiles.py (خط 142-245) `get_all_profiles` از مستقیماً DB (AIProfile) می‌خواند با query param های `sort_by/order/use_fallback` و فرمت متفاوت `{success, profiles, total, is_fallback}` (total به‌جای count). فرانت‌اند model-profiles/page.tsx (خط 116) به `?use_fallback=false` و فیلد `total` متکی است که فقط در model_profiles.py وجود دارد. اما router model_profiles با prefix `/api/models` mount می‌شود و models.py هم همان prefix را دارد — بسته به ترتیب include_router در main، ممکن است route اشتباه match شود و فرانت‌اند داده‌های fallback یا فرمت اشتباه بگیرد.
...
- `backend/app/api/routes/models.py:321-341` — `get_model_profiles` — نسخهٔ profiler-based با فیلد count
  ```python
  @router.get("/profiles")
  async def get_model_profiles():
      profiler = get_model_profiler()
      profiles = profiler.get_all_profiles()
      return {"success": True, "profiles": profiles_data, "count": len(profiles_data), "is_fallback": len(profiles_data) == 0}
  ```
- `backend/app/api/routes/model_profiles.py:142-149` — `get_all_profiles` — نسخهٔ DB-based با فیلد total و query params — این همان است که فرانت می‌خواهد
  ```python
  @router.get("/profiles")
  async def get_all_profiles(
      ...
      db=Depends(get_db)):
  ```
- `backend/app/api/routes/model_profiles.py:50-50` — `router` — prefix یکسان با models.py که tدر main mount می‌شود
  ```python
...
FastAPI APIRouter با prefix مشترک + SQLAlchemy + Next.js client fetch

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/models.py` (سطر 15) — prefix /models — تعارض احتمالی mount
- `backend/app/main.py` (سطر 66) — ترتیب include_router تعیین‌کنندهٔ route برنده
- `frontend/src/app/model-profiles/page.tsx` (سطر 116) — به use_fallback=false و فیلد total متکی است
...
## 🌐 نقشهٔ وابستگی‌ها
هر دو endpoint از model_profiler (10 importer) استفاده می‌کنند. model_profiles.py با defensive import mount می‌شود (MODEL_PROFILES_AVAILABLE). تعارض روی صفحهٔ model-profiles که برای ادمین مهم است اثر مستقیم دارد.
...
- [ ] فقط یک handler برای GET /api/models/profiles به‌صورت قطعی match می‌شود (با تست route)
- [ ] پاسخ شامل هم total و هم count است تا فرانت‌اند هر دو نسخه را پشتیبانی کند
- [ ] فرانت‌اند model-profiles/page.tsx با use_fallback=false داده‌های واقعی DB می‌گیرد نه fallback نمایشی
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. تعارض route را حل کن: یا endpoint `/profiles` را از models.py حذف کن (چون model_profiles.py نسخهٔ کامل‌تر با DB دارد)، یا هر دو را به یک مسیر واحد ادغام کن. ترتیب include_router در main.py را بررسی کن تا مشخص شود کدام route برنده می‌شود.

## 💡 نمونه‌های قبل/بعد
**یکسان‌سازی فرمت پاسخ**

_قبل:_
```
return {"success": True, "profiles": ..., "count": len(...)}
```

_بعد:_
```
return {"success": True, "profiles": ..., "total": len(...), "count": len(...)}  # هر دو فیلد برای backward compat
```
...
## 🧪 دستورات اعتبارسنجی
- `curl 'http://localhost:8000/api/models/profiles?use_fallback=false' | python -m json.tool`
- `grep -rn 'def get_all_profiles\|def get_model_profiles' backend/app/api/routes/`

## ⚠️ ریسک‌ها و موارد احتیاط
حذف route ممکن است caller های قدیمی را بشکند؛ باید همهٔ مصرف‌کنندگان frontend بررسی شوند
...
1. فقط یک handler برای GET /api/models/profiles به‌صورت قطعی match می‌شود (با تست route) _(verify: static)_
2. پاسخ شامل هم total و هم count است تا فرانت‌اند هر دو نسخه را پشتیبانی کند _(verify: static)_
3. فرانت‌اند model-profiles/page.tsx با use_fallback=false داده‌های واقعی DB می‌گیرد نه fallback نمایشی _(verify: static)_
