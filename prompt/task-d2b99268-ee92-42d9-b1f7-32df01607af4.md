---
task_id: d2b99268-ee92-42d9-b1f7-32df01607af4
title: 'فایل‌های route عظیم (>۱۰۰۰ خط) بدون تفکیک: oversight.py ۱۵۳۶، simple_projects.py ۱۵۲۴، project_memory.py ۱۷۶۱، project_health.py ۱۷۱۸، project_journal.py ۱۶۷۸'
type: refactor
priority: medium
...
created_at: '2026-06-14T09:41:07.761002+00:00'
updated_at: '2026-06-14T09:42:51.741930+00:00'
target_files:
- backend/app/api/routes/project_journal.py
- backend/app/api/routes/project_memory.py
---

# فایل‌های route عظیم (>۱۰۰۰ خط) بدون تفکیک: oversight.py ۱۵۳۶، simple_projects.py ۱۵۲۴، project_memory.py ۱۷۶۱، project_health.py ۱۷۱۸، project_journal.py ۱۶۷۸

## Raw Idea

حداقل ۶ فایل route از مرز ۱۰۰۰ خط عبور کرده‌اند که نگهداری، code review و تست را دشوار می‌کند. project_memory.py با ۱۷۶۱ خط شامل تعریف مدل‌های دیتابیس inline (ActivityLog، DetailedOperation، Report، ReportTrigger در project_journal.py)، توابع کمکی GitHub (commit_to_github، extract_code_blocks)، منطق dependency-sorting و endpointها به‌صورت یکجاست. این نقض جداسازی لایه‌هاست: مدل‌ها باید در models/، منطق GitHub در services/ و فقط endpoint handlers در routes/ باشند. همچنین `Base.metadata.create_all(bind=engine)` در سطح ماژول project_journal.py خط ۲۶۵ side-effect در import-time ایجاد می‌کند.
...
- `backend/app/api/routes/project_journal.py:143-265` — `ActivityLog, DetailedOperation, Report, ReportTrigger + create_all` — تعریف ۴ مدل دیتابیس + create_all side-effect در یک route file
  ```python
  ...
  ```
- `backend/app/api/routes/project_memory.py:304-401` — `extract_code_blocks, commit_to_github` — منطق GitHub در route file به‌جای service
  ```python
  async def commit_to_github(owner, repo, token, file_path, content, ...):
...
- `backend/app/models/project.py` — `project_journal.py` این فایل را import می‌کند
...
1. مدل‌های دیتابیس تعریف‌شده در routes (ActivityLog و …) را به فایل‌های متناظر در `backend/app/models/` منتقل کن. توابع GitHub helper (commit_to_github، extract_code_blocks) را به یک service مثل `github_storage.py` یا یک ماژول جدید منتقل کن. create_all را از import-time خارج کن و در lifespan/init_db قرار بده. هر route را به زیرماژول‌های منطقی (مثلاً oversight_tasks، oversight_watched) تقسیم کن.
