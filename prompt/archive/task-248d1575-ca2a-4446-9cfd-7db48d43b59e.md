---
task_id: 248d1575-ca2a-4446-9cfd-7db48d43b59e
title: 'endpoint بک‌اند بلااستفاده: POST /pr/from-project/{project_id}'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:56.562110+00:00'
updated_at: '2026-06-02T17:44:04.296678+00:00'
archived: true
...
---
[scan #2 at 2026-05-15T06:05:56.640461+00:00]
## 📋 شرح
endpoint `POST /issues/from-project/{project_id}` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/issues/from-project/{project_id}`
- فایل: `backend/app/api/routes/gi
---
[scan #3 at 2026-05-15T06:39:19.701375+00:00]
## 📋 شرح
endpoint `POST /pr/from-project/{project_id}` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- method: `POST`
- path: `/pr/from-project/{project_id}`
- فایل: `backend/app/api/routes/github_imp
---
[scan #4 at 2026-05-15T06:39:19.746284+00:00]
## 📋 شرح
endpoint `POST /issues/from-project/{project_id}` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- فایل: `backend/app/api/routes/gi
---
[scan #5 at 2026-05-15T07:39:31.433682+00:00]
## 📋 شرح
endpoint `POST /pr/from-project/{project_id}` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `POST /pr/from-project/{project_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
