---
task_id: fef99b8c-5d16-44b0-ab0c-a718d0603e57
title: 'endpoint بک‌اند بلااستفاده: PUT /schedule'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:56.171805+00:00'
updated_at: '2026-06-02T17:43:52.962514+00:00'
archived: true
archived_at: '2026-05-17T10:33:24.859430+00:00'
target_files:
- backend/app/api/routes/analysis.py
---

# endpoint بک‌اند بلااستفاده: PUT /schedule

## Raw Idea

## 📋 شرح
endpoint `PUT /schedule` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #2 at 2026-05-15T06:05:56.240400+00:00]
## 📋 شرح
endpoint `DELETE /schedule/{project_id}` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no fr
---
[scan #3 at 2026-05-15T06:39:19.230235+00:00]
## 📋 شرح
endpoint `PUT /schedule` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- path: `/schedule`
- فایل: `backend/app/api/routes/analysis.py`
- علت: no frontend fetch + no recent call in
---
[scan #4 at 2026-05-15T06:39:19.273002+00:00]
## 📋 شرح
endpoint `DELETE /schedule/{project_id}` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no fr
---
[scan #5 at 2026-05-15T07:39:30.952312+00:00]
## 📋 شرح
endpoint `PUT /schedule` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend fetch + no recent call in
---
[scan #6 at 2026-05-15T07:39:30.995276+00:00]
## 📋 شرح
endpoint `DELETE /schedule/{project_id}` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- [ ] مشخص شد endpoint `PUT /schedule` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/schedule` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
1. مشخص شد endpoint `PUT /schedule` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
