---
task_id: df88b572-69f7-4265-92ac-2a870f3b6d2d
title: 'endpoint بک‌اند بلااستفاده: POST /parse-url'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:56.413932+00:00'
updated_at: '2026-05-29T20:11:11.573321+00:00'
archived: true
...
---
[scan #2 at 2026-05-15T06:05:56.486533+00:00]
## 📋 شرح
endpoint `POST /pr/create` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #3 at 2026-05-15T06:05:56.524634+00:00]
## 📋 شرح
endpoint `POST /issues/push` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #4 at 2026-05-15T06:39:19.524986+00:00]
## 📋 شرح
endpoint `POST /parse-url` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend fetch + no 
---
[scan #5 at 2026-05-15T06:39:19.569819+00:00]
## 📋 شرح
endpoint `POST /pr/create` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend fetch + no 
---
[scan #6 at 2026-05-15T06:39:19.615270+00:00]
## 📋 شرح
endpoint `POST /issues/push` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `POST /parse-url` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
