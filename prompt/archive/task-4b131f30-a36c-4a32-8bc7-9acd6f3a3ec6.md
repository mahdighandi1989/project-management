---
task_id: 4b131f30-a36c-4a32-8bc7-9acd6f3a3ec6
title: 'endpoint بک‌اند بلااستفاده: POST /auto-build'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:57.073572+00:00'
updated_at: '2026-06-02T17:44:19.396702+00:00'
archived: true
...
---
[scan #2 at 2026-05-15T06:05:57.159015+00:00]
## 📋 شرح
...
- علت: no frontend fet
---
[scan #3 at 2026-05-15T06:05:57.372208+00:00]
## 📋 شرح
endpoint `POST /save-file` در `backend/app/api/routes/orchestrator.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #4 at 2026-05-15T06:05:57.514753+00:00]
## 📋 شرح
endpoint `POST /execute-task` در `backend/app/api/routes/orchestrator.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #5 at 2026-05-15T06:05:57.580465+00:00]
## 📋 شرح
endpoint `POST /evaluate` در `backend/app/api/routes/orchestrator.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #6 at 2026-05-15T06:39:20.552623+00:00]
## 📋 شرح
endpoint `POST /auto-build` در `backend/app/api/routes/orchestrator.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `POST /auto-build` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
