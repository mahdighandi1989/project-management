---
task_id: afb8aac1-c3ba-4a0d-8621-887e0e9c4e42
title: 'فیلد بلااستفاده: Deployment.auto_deploy'
type: cleanup
priority: low
...
created_at: '2026-05-15T06:41:03.519988+00:00'
updated_at: '2026-05-29T20:12:25.734182+00:00'
archived: true
archived_at: '2026-05-17T10:43:00.591637+00:00'
target_files:
- backend/app/services/deploy_service.py
---

# فیلد بلااستفاده: Deployment.auto_deploy

## Raw Idea

## 📋 شرح
فیلد `auto_deploy` در dataclass/model `Deployment` (فایل `backend/app/services/deploy_service.py`) تعریف شده ولی هرگز خوانده نمی‌شود.

## 🤔 چرا مهم است
یا (الف) ذخیره می‌شود برای آینده ولی هیچ‌کس مصرف نمی‌کند (waste)، یا (ب) reader gone است (regression — قبلاً مصرف می‌شد).

## 🔍 جزئیات
- علت: field defined in Deployment but not read/written anywhere
---
[scan #2 at 2026-05-15T06:41:03.665495+00:00]
## 📋 شرح
فیلد `publish_path` در dataclass/model `Deployment` (فایل `backend/app/services/deploy_service.py`) تعریف شده ولی هرگز خوانده نمی‌شود.
...
- [ ] تأیید شد `auto_deploy` در هیچ reader مصرف نمی‌شود
...
1. گام ۱: grep روی `\.auto_deploy` + `['auto_deploy']` در کل کدبیس.
...
1. تأیید شد `auto_deploy` در هیچ reader مصرف نمی‌شود _(verify: static)_
