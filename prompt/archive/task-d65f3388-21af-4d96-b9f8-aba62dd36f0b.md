---
task_id: d65f3388-21af-4d96-b9f8-aba62dd36f0b
title: 'endpoint بک‌اند بلااستفاده: GET /profiles/{model_id}/history'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:56.004008+00:00'
updated_at: '2026-06-02T17:43:49.039840+00:00'
archived: true
archived_at: '2026-05-17T18:13:57.869045+00:00'
target_files:
- backend/app/api/routes/analysis.py
---

# endpoint بک‌اند بلااستفاده: GET /profiles/{model_id}/history

## Raw Idea

## 📋 شرح
endpoint `GET /profiles/{model_id}/history` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `GET`
- path: `/profiles/{model_id}/history`
...
---
[scan #2 at 2026-05-15T06:39:19.015880+00:00]
## 📋 شرح
endpoint `GET /profiles/{model_id}/history` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت:
---
[scan #3 at 2026-05-15T07:39:30.742513+00:00]
## 📋 شرح
endpoint `GET /profiles/{model_id}/history` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت:
---
[scan #4 at 2026-05-15T10:32:33.965369+00:00]
## 📋 شرح
endpoint `GET /profiles/{model_id}/history` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت:
---
[scan #5 at 2026-05-15T16:45:11.318732+00:00]
## 📋 شرح
endpoint `GET /profiles/{model_id}/history` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- [ ] مشخص شد endpoint `GET /profiles/{model_id}/history` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/profiles/{model_id}/history` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
1. مشخص شد endpoint `GET /profiles/{model_id}/history` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
