---
task_id: 633abd7a-c942-43cc-91fd-60e35eadd353
title: 'endpoint بک‌اند بلااستفاده: POST /profiles/compare'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:56.038208+00:00'
updated_at: '2026-05-29T20:11:04.567905+00:00'
archived: true
...
---
[scan #2 at 2026-05-15T06:05:56.104745+00:00]
## 📋 شرح
endpoint `POST /profiles/refresh-rankings` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/profiles/refresh-rankings`
- فایل: `backend/app/api/routes/analysis.py`
- علت: n
---
[scan #3 at 2026-05-15T06:05:56.138521+00:00]
## 📋 شرح
endpoint `POST /profiles/initialize` در `backend/app/api/routes/analysis.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/profiles/initialize`
- فایل: `backend/app/api/routes/analysis.py`
- علت: no frontend f
---
[scan #4 at 2026-05-15T06:05:57.617930+00:00]
## 📋 شرح
endpoint `POST /compare-models` در `backend/app/api/routes/orchestrator.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend fet
---
[scan #5 at 2026-05-15T06:05:57.744514+00:00]
## 📋 شرح
endpoint `POST /upload-for-analysis` در `backend/app/api/routes/orchestrator.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/upload-for-analysis`
- فایل: `backend/app/api/routes/orchestrator.py`
- علت: no fr
---
[scan #6 at 2026-05-15T06:39:18.793475+00:00]
## 📋 شرح
...
1. مشخص شد endpoint `POST /profiles/compare` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
