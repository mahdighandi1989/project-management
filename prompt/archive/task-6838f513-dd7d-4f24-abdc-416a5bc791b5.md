---
task_id: 6838f513-dd7d-4f24-abdc-416a5bc791b5
title: 'endpoint بک‌اند بلااستفاده: POST /imported/{project_id}/refresh'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:56.343080+00:00'
updated_at: '2026-06-03T17:16:14.931557+00:00'
archived: true
...
---
[scan #2 at 2026-05-15T06:39:19.437738+00:00]
## 📋 شرح
endpoint `POST /imported/{project_id}/refresh` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #3 at 2026-05-15T07:39:31.130007+00:00]
## 📋 شرح
endpoint `POST /imported/{project_id}/refresh` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- فایل: `backend/app/api/routes/github_i
---
[scan #4 at 2026-05-15T10:32:34.370446+00:00]
## 📋 شرح
endpoint `POST /imported/{project_id}/refresh` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- فایل: `backend/app/api/routes/github_i
---
[scan #5 at 2026-05-15T16:45:11.879991+00:00]
## 📋 شرح
endpoint `POST /imported/{project_id}/refresh` در `backend/app/api/routes/github_import.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
1. مشخص شد endpoint `POST /imported/{project_id}/refresh` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
