---
task_id: 87243eda-ef1c-41c5-9cca-74455e9ffcb5
title: 'endpoint بک‌اند بلااستفاده: PUT /settings/{model_id}'
type: audit
priority: medium
...
created_at: '2026-05-15T06:39:20.183620+00:00'
updated_at: '2026-06-02T17:45:32.589437+00:00'
archived: true
archived_at: '2026-05-17T10:40:47.052178+00:00'
target_files:
- backend/app/api/routes/models.py
---

# endpoint بک‌اند بلااستفاده: PUT /settings/{model_id}

## Raw Idea

## 📋 شرح
endpoint `PUT /settings/{model_id}` در `backend/app/api/routes/models.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #2 at 2026-05-15T06:39:20.411033+00:00]
## 📋 شرح
endpoint `DELETE /settings/{model_id}` در `backend/app/api/routes/models.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend f
---
[scan #3 at 2026-05-15T07:39:32.011664+00:00]
## 📋 شرح
endpoint `PUT /settings/{model_id}` در `backend/app/api/routes/models.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #4 at 2026-05-15T07:39:32.223443+00:00]
## 📋 شرح
endpoint `DELETE /settings/{model_id}` در `backend/app/api/routes/models.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend f
---
[scan #5 at 2026-05-15T10:32:35.215066+00:00]
## 📋 شرح
endpoint `PUT /settings/{model_id}` در `backend/app/api/routes/models.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no frontend fetch +
---
[scan #6 at 2026-05-15T10:32:35.405122+00:00]
## 📋 شرح
endpoint `DELETE /settings/{model_id}` در `backend/app/api/routes/models.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- [ ] مشخص شد endpoint `PUT /settings/{model_id}` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/settings/{model_id}` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
1. مشخص شد endpoint `PUT /settings/{model_id}` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
