---
task_id: 3fbc9874-cbcf-4e62-b6af-d8ed50a03acb
title: 'endpoint بک‌اند بلااستفاده: GET /model-categories'
type: audit
priority: medium
...
created_at: '2026-05-15T06:05:57.660230+00:00'
updated_at: '2026-06-02T17:44:26.832997+00:00'
archived: true
archived_at: '2026-05-17T18:17:25.349547+00:00'
target_files:
- backend/app/api/routes/orchestrator.py
---

# endpoint بک‌اند بلااستفاده: GET /model-categories

## Raw Idea

## 📋 شرح
endpoint `GET /model-categories` در `backend/app/api/routes/orchestrator.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- [ ] مشخص شد endpoint `GET /model-categories` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/model-categories` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
1. مشخص شد endpoint `GET /model-categories` در کدام دسته است (orphan/internal/deprecated) _(verify: static)_
