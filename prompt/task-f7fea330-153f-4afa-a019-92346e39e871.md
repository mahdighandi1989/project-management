---
task_id: f7fea330-153f-4afa-a019-92346e39e871
...
created_at: '2026-05-31T09:07:46.119220+00:00'
updated_at: '2026-06-14T09:41:40.934782+00:00'
target_files:
...
در EVENT_REGISTRY، event 'task_created' ناقص است (بدون بسته شدن dict و کاما). این باعث SyntaxError در runtime می‌شود و کل سرویس از کار می‌افتد. همچنین event 'scan_started' با default_enabled=False تعریف شده که با منطق 'همیشه notify' در تضاد است.

📁 file: backend/app/services/notification_service.py (line 150)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-05-31T09:07:47.029755+00:00]
مدیریت state ایندکس (telegram_index_state.json) از فایل JSON استفاده می‌کند که برای concurrent access (مثلاً چند webhook همزمان) مناسب نیست. در محیط production با ترافیک بالا، این باعث data corruption می‌شود.

📁 file: backend/app/services/notification_service.py (line 65)

🎯 پیشنهاد: این الگو معمولا
---
[scan #3 at 2026-05-31T09:09:16.045944+00:00]
استفاده از ABC (Abstract Base Class) برای NotificationService در حالی که فقط یک implementation وجود دارد و هیچ extension point واقعی در کد دیده نمی‌شود. این پیچیدگی غیرضروری است و maintainability را کاهش می‌دهد.

📁 file: backend/app/services/notification_service.py (line 30)

🎯 پیشنهاد: این الگو معم
---
[scan #4 at 2026-05-31T09:09:16.354104+00:00]
در تابع _resolve_storage_dir()، از try-except برای تست write استفاده شده، اما اگر دایرکتوری قابل نوشتن نباشد، به Path('/tmp') fallback می‌کند بدون validation که /tmp هم قابل نوشتن است یا خیر. این می‌تواند منجر به خطاهای runtime شود.

📁 file: backend/app/services/notification_service.py (line 45)

🎯 
---
[scan #5 at 2026-06-14T09:41:40.599488+00:00]
_resolve_storage_dir در صورت شکست همهٔ candidate ها، خاموش (silent) به Path('/tmp') برمی‌گردد بدون هیچ log/warning. یعنی اگر storage اصلی غیرقابل‌نوشتن شود، prefs و index_state در /tmp ذخیره می‌شوند و کاربر هیچ‌وقت متوجه از دست رفتن persistence نمی‌شود — outcome (degradation) نه لاگ می‌شود نه به سطح
---
[scan #6 at 2026-06-14T09:41:40.934776+00:00]
fallback نهایی Path('/tmp') بدون mkdir/write_test برمی‌گردد، در حالی که خود '/tmp/notifications' (candidate قبلی) همان write_test را داشت. اگر /tmp هم writable نباشد (container read-only fs)، نوشتن بعدی بدون validation می‌شکند. فرض «‎/tmp همیشه نوشتنی است» در محیط‌های containerized دیگر صادق نیست.
...
- `backend/app/services/notification_service.py:150`
...
- `backend/app/services/oversight_telegram_compose.py` — `notification_service.py` این فایل را import می‌کند
- `backend/app/api/routes/oversight.py` — `notification_service.py` این فایل را import می‌کند
- `backend/app/services/oversight_service.py` — `notification_service.py` این فایل را import می‌کند
- `backend/app/services/oversight_upload_session.py` — `notification_service.py` این فایل را import می‌کند
- `backend/app/api/routes/simple_projects.py` — این فایل `notification_service.py` را import می‌کند (caller)
- `backend/app/api/routes/notifications.py` — این فایل `notification_service.py` را import می‌کند (caller)
