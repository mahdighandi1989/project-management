---
task_id: 808701dd-014f-452a-addc-c8de58dc77d8
title: ⬆️ render_logs.py ممکن است نیاز به update داشته باشد به‌خاطر تغییرات در test_creator_deploy_render_ai_parity.py
...
created_at: '2026-06-14T09:41:07.032248+00:00'
updated_at: '2026-06-14T09:42:43.928053+00:00'
target_files:
- backend/app/api/routes/render_logs.py
---

# ⬆️ render_logs.py ممکن است نیاز به update داشته باشد به‌خاطر تغییرات در test_creator_deploy_render_ai_parity.py
...
## 🔍 جزئیات
- 📂 فایل تغییریافته (consumer): `backend/tests/test_creator_deploy_render_ai_parity.py`
- 📂 فایل upstream (producer — نیازمند update): `backend/app/api/routes/render_logs.py`
- 🔥 risk: **high**
- 📝 reason: تست جدید قراردادی روی این فایل تحمیل می‌کند: باید تابع جدید `_ai_analyze_repo_only` با پارامترهای (owner, repo, branch, github_token, github_url, model_id) و بدون db/project_id وجود داشته باشد، و `_ai_analyze_project` موجود باید refactor شود تا به `_ai_analyze_repo_only` delegate کند (تست test_ai_analyze_project_body_now_uses_helper چک می‌کند که رشته '_ai_analyze_repo_only' در بدنه آن باشد). اگر ا
...
- `backend/tests/test_creator_deploy_render_ai_parity.py` — consumer که تغییر کرده
- `backend/app/api/routes/simple_projects.py` — این فایل `render_logs.py` را import می‌کند (caller)
- `backend/app/main.py` — این فایل `render_logs.py` را import می‌کند (caller)
- `backend/tests/test_inspector_oversight_bridge.py` — این فایل `render_logs.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
`backend/tests/test_creator_deploy_render_ai_parity.py` به `backend/app/api/routes/render_logs.py` وابسته است.
...
- [ ] بررسی شد `backend/tests/test_creator_deploy_render_ai_parity.py` چه API ای از `backend/app/api/routes/render_logs.py` می‌خواهد
- [ ] اگر API در `backend/app/api/routes/render_logs.py` نیست، اضافه شد
...
1. گام ۱: `backend/tests/test_creator_deploy_render_ai_parity.py` را بخوان و usages جدید از `backend/app/api/routes/render_logs.py` را پیدا کن.
گام ۲: API contract در `backend/app/api/routes/render_logs.py` چک کن.
گام ۳: اگر contract ناسازگار است، `backend/app/api/routes/render_logs.py` را extend کن.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/api/routes/render_logs.py`
- `ruff check backend/app/api/routes/render_logs.py`
- `pytest -x`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر `backend/app/api/routes/render_logs.py` ممکن است بقیه consumers را تحت تأثیر قرار دهد — قبل از تغییر، همهٔ مصرف‌کنندگانش را check کن.
...
1. بررسی شد `backend/tests/test_creator_deploy_render_ai_parity.py` چه API ای از `backend/app/api/routes/render_logs.py` می‌خواهد _(verify: static)_
2. اگر API در `backend/app/api/routes/render_logs.py` نیست، اضافه شد _(verify: static)_
3. تست integration که هر دو سمت را پوشش می‌دهد عبور می‌کند _(verify: static)_
