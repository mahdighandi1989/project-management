---
task_id: af78e389-786d-4b2a-952e-28907aeac6ab
title: 'Anti-pattern: stale_assumption'
type: bug_fix
priority: high
...
created_at: '2026-05-31T09:07:47.330777+00:00'
updated_at: '2026-06-03T18:38:17.908605+00:00'
target_files:
- backend/app/core/database.py
---

# Anti-pattern: stale_assumption

## Raw Idea

کد فرض می‌کند که دیتابیس SQLite همیشه در مسیر /app/storage در production وجود دارد، اما این مسیر ممکن است در محیط‌های مختلف (مثل Docker بدون volume) وجود نداشته باشد یا قابل نوشتن نباشد. همچنین فرض می‌کند که ENVIRONMENT همیشه 'production' یا 'development' است، در حالی که ممکن است مقادیر دیگری مثل 'staging' یا 'test' داشته باشد که باعث رفتار غیرمنتظره می‌شود.

📁 file: backend/app/core/database.py (line 18)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-05-31T09:07:48.241497+00:00]
تنظیمات poolclass=StaticPool برای SQLite در محیط‌های multi-threaded (مثل FastAPI با workers متعدد) ممکن است باعث race condition شود. StaticPool برای single-thread مناسب است، در حالی که connect_args={'check_same_thread': False} نشان می‌دهد که قصد استفاده multi-thread دارید. این دو تنظیم با هم در تضاد
...
- `backend/app/core/database.py:18`
...
- `backend/app/api/routes/analysis.py` — این فایل `database.py` را import می‌کند (caller)
- `backend/app/api/routes/github_import.py` — این فایل `database.py` را import می‌کند (caller)
