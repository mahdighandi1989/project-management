---
task_id: 412c0cff-c894-419e-8022-eb89fc6fe433
title: Decentralized API Key and Token Management
type: refactor
priority: high
...
created_at: '2026-06-07T09:29:18.455923+00:00'
updated_at: '2026-06-07T09:33:16.674474+00:00'
target_files:
...
# Decentralized API Key and Token Management

## Raw Idea

The logic for loading, saving, and validating API keys and tokens (for AI providers, GitHub, Render, and external tools) is scattered across multiple files (`settings.py`, `github_import.py`, `external_prompts.py`, `main.py`). This decentralization leads to inconsistent handling of sensitive information, potential for subtle bugs in loading/persisting, and makes security auditing and management more complex.
...
- `backend/app/api/routes/settings.py:165-220` — `update_api_keys`
  ```python
  @router.put("/api-keys")
  async def update_api_keys(request: UpdateApiKeysRequest, db: Session = Depends(get_db)):
      ...
      try:
          updated = []
          env_updates = {}
          # ... (truncated for brevity, contains logic for saving keys) ...
          return {
              "success": True,
              "updated": updated,
...
- `backend/app/api/routes/github_import.py:45-52` — `get_effective_token`
  ...
  def get_effective_token(provided_token: str = None, use_global: bool = False) -> str:
      ...
      return ""
  ```
- `backend/app/main.py:384-400` — `load_api_keys_from_database`
  ```python
  async def load_api_keys_from_database():
      ...
      try:
          ...
          key_mapping = [
              ...
              ("external_tool_token", "EXTERNAL_TOOL_TOKEN"),
          ...
          logging.getLogger(__name__).warning(f"Failed to load API keys from DB: {e}")
  ```
...
FastAPI, SQLAlchemy, environment variables, `.env` file management, security.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/config.py` — Relies on environment variables for configuration.
- `backend/app/models/setting.py` — Database model for storing settings, including encrypted secrets.
- `backend/app/services/ai_manager.py` — Needs API keys to initialize AI providers.
- `backend/app/services/notification_service.py` — May require Telegram bot tokens.
- `backend/app/services/cloud_code_service.py` — Requires OAuth tokens.
...
- `backend/app/api/routes/project_memory.py` — این فایل `settings.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
API key and token management is a cross-cutting concern impacting security, AI service availability, and external integrations. Centralizing this will improve consistency, security, and maintainability.
...
- [ ] A new service `backend/app/services/secret_management_service.py` is created.
- [ ] All logic for loading, saving, and validating API keys/tokens is moved to this service.
- [ ] All existing callers (e.g., in `settings.py`, `github_import.py`, `external_prompts.py`, `main.py`) are updated to use the new service.
- [ ] The `.env` file is correctly updated and environment variables are set consistently.
- [ ] All AI providers and external integrations function correctly after the refactor.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Create a dedicated `backend/app/services/secret_management_service.py` (or similar) to encapsulate all operations related to API keys and tokens: loading from environment/database, saving to database/`.env` file, and validation. All modules requiring access to these secrets should interact with this centralized service. The `load_api_keys_from_database` in `main.py` should also delegate to this service.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `Test updating API keys via `/settings/api-keys` and deploy keys via `/settings/deploy-keys`.`
- `Verify that AI models are available and functional.`
- `Test GitHub import and external prompt claiming to ensure token access is correct.`
- `Restart the application and verify that API keys are loaded correctly from the database.`

## ⚠️ ریسک‌ها و موارد احتیاط
High risk of breaking critical functionality (AI services, GitHub integration, deployments) if not implemented carefully. Requires extensive testing.
...
1. A new service `backend/app/services/secret_management_service.py` is created. _(verify: static)_
2. All logic for loading, saving, and validating API keys/tokens is moved to this service. _(verify: static)_
3. All existing callers (e.g., in `settings.py`, `github_import.py`, `external_prompts.py`, `main.py`) are updated to use the new service. _(verify: static)_
4. The `.env` file is correctly updated and environment variables are set consistently. _(verify: static)_
5. All AI providers and external integrations function correctly after the refactor. _(verify: ui_interaction)_

## Task Steps

### Step 1: Centralize API Key and Token Management Logic
**Status:** `pending` (0%)
**Scope:** This foundational step involves identifying all existing logic related to loading, saving, and validating API keys and tokens for various providers (AI providers, GitHub, Render, and external tools) currently scattered across `settings.py`, `github_import.py`, `external_prompts.py`, and `main.py`. The primary goal is to refactor and consolidate this scattered logic into a single, dedicated module or service responsible for all key and token management. This centralization aims to ensure consistent handling of sensitive information, reduce potential for subtle bugs in loading/persisting, and simplify security auditing and management. This step focuses solely on the refactoring and consolidation of existing logic, not on introducing new key types or changing the underlying storage mechanism.
**Excerpt:**
```
The logic for loading, saving, and validating API keys and tokens (for AI providers, GitHub, Render, and external tools) is scattered across multiple files (`settings.py`, `github_import.py`, `external_prompts.py`, `main.py`). This decentralization leads to inconsistent handling of sensitive information, potential for subtle bugs in loading/persisting, and makes security auditing and management more complex.
```
