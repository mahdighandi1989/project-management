---
task_id: 83f8756c-0899-4ae3-8e14-f0d3f42f6825
title: 'endpoint بک‌اند بلااستفاده: GET /next'
type: audit
priority: medium
...
created_at: '2026-05-31T09:08:59.002052+00:00'
updated_at: '2026-06-03T18:38:29.980328+00:00'
target_files:
- backend/app/api/routes/external_prompts.py
---

# endpoint بک‌اند بلااستفاده: GET /next

## Raw Idea

## 📋 شرح
endpoint `GET /next` در `backend/app/api/routes/external_prompts.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
---
[scan #2 at 2026-05-31T09:08:59.997207+00:00]
## 📋 شرح
endpoint `POST /{task_id}/claim` در `backend/app/api/routes/external_prompts.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no fr
---
[scan #3 at 2026-05-31T09:09:00.328353+00:00]
## 📋 شرح
endpoint `POST /{task_id}/progress` در `backend/app/api/routes/external_prompts.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/{task_id}/progress`
- فایل: `backend/app/api/routes/external_prompts.py`
- علت:
---
[scan #4 at 2026-05-31T09:09:00.635463+00:00]
## 📋 شرح
endpoint `POST /{task_id}/complete` در `backend/app/api/routes/external_prompts.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت:
---
[scan #5 at 2026-05-31T09:09:00.946453+00:00]
## 📋 شرح
endpoint `POST /{task_id}/fail` در `backend/app/api/routes/external_prompts.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.
...
- علت: no fron
---
[scan #6 at 2026-05-31T09:09:01.259235+00:00]
## 📋 شرح
endpoint `POST /_admin/backfill` در `backend/app/api/routes/external_prompts.py` تعریف شده ولی هیچ `fetch`، `axios`، `apiClient.*`، یا frontend call به آن دیده نمی‌شود.

## 🔍 جزئیات
- method: `POST`
- path: `/_admin/backfill`
- فایل: `backend/app/api/routes/external_prompts.py`
- علت: no fr
...
- [ ] مشخص شد endpoint `GET /next` در کدام دسته است (orphan/internal/deprecated)
...
1. گام ۱: grep روی `/next` در frontend + scripts/ + docs/ — اگر caller هست، اتصال را drop شده اصلاح کن.
...
1. مشخص شد endpoint `GET /next` در کدام دسته است (orphan/internal/deprecated) _(verify: manual_only)_
