---
task_id: e5d7aae3-26e7-4645-931c-3f846ef2ee61
title: 'CORS با Access-Control-Allow-Origin: ''*'' روی SSE endpoint تحلیل تنظیم شده'
type: security
priority: high
...
created_at: '2026-06-14T09:39:51.356459+00:00'
updated_at: '2026-06-14T09:42:10.332940+00:00'
target_files:
- backend/app/api/routes/analysis.py
---

# CORS با Access-Control-Allow-Origin: '*' روی SSE endpoint تحلیل تنظیم شده

## Raw Idea

در endpoint استریم تحلیل (run-analysis-stream)، هدر `Access-Control-Allow-Origin: *` به‌صورت hardcoded روی پاسخ StreamingResponse ست شده است. این یعنی هر origin خارجی می‌تواند این endpoint را فراخوانی کند. چون این endpoint محتوای کامل فایل‌های پروژه (شامل کدهای منبع و احتمالاً secretهای داخل فایل‌ها) را می‌خواند و در رویدادهای SSE ارسال می‌کند، باز بودن CORS روی آن یک نشت اطلاعات بالقوه است. علاوه بر این، خود FastAPI CORSMiddleware در main.py از settings.CORS_ORIGINS استفاده می‌کند، اما این هدر دستی آن پیکربندی را override و دور می‌زند. هیچ authentication روی این endpoint وجود ندارد.
...
- `backend/app/api/routes/analysis.py:259-268` — `run_analysis_stream` — هدر CORS باز که باید حذف یا محدود شود
  ```python
...
FastAPI StreamingResponse + SSE + CORSMiddleware + Next.js 14 App Router

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/main.py` (سطر 7) — CORSMiddleware مرکزی اینجا با settings.CORS_ORIGINS پیکربندی می‌شود و توسط این هدر دستی override می‌گردد
- `backend/app/core/config.py` — settings.CORS_ORIGINS که باید منبع واحد origin مجاز باشد
...
## 🌐 نقشهٔ وابستگی‌ها
این endpoint تنها مصرف‌کنندهٔ project_analyzer + DeepAnalysisService برای استریم است و frontend/src/app/analysis/page.tsx آن را fetch می‌کند. تغییر هدر CORS فقط روی فراخوانی cross-origin اثر دارد و نباید same-origin frontend را بشکند.
...
- [ ] هدر Access-Control-Allow-Origin: * از پاسخ run-analysis-stream حذف شده باشد
- [ ] درخواست cross-origin از origin خارج از whitelist با CORS مسدود شود
- [ ] frontend same-origin همچنان بتواند استریم را دریافت کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. هدر دستی `Access-Control-Allow-Origin: *` را از StreamingResponse حذف کن و اجازه بده CORSMiddleware مرکزی (با لیست محدود allowed origins از settings.CORS_ORIGINS) مدیریت CORS را برعهده بگیرد. اگر SSE نیاز به CORS خاص دارد، origin را به‌صورت داینامیک از request.headers بخوان و فقط در صورت تطبیق با whitelist بازتاب بده. همچنین یک لایه auth (مثل require token) روی این endpoint اضافه کن چون محتوای کامل فایل‌ها را افشا می‌کند.

## 💡 نمونه‌های قبل/بعد
**حذف هدر باز و واگذاری به middleware**
...
headers={
    ...
    "Connection": "keep-alive",
    "X-Accel-Buffering": "no"
}  # CORS توسط CORSMiddleware مرکزی مدیریت می‌شود
```
...
## 🧪 دستورات اعتبارسنجی
- `curl -i -H 'Origin: https://evil.example' -X POST http://localhost:8000/api/analysis/run-stream | grep -i access-control-allow-origin`
- `pytest backend/tests -k cors`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر frontend روی origin متفاوت از backend مستقر باشد، باید آن origin به CORS_ORIGINS اضافه شود تا استریم نشکند
...
1. هدر Access-Control-Allow-Origin: * از پاسخ run-analysis-stream حذف شده باشد _(verify: static)_
2. درخواست cross-origin از origin خارج از whitelist با CORS مسدود شود _(verify: static)_
3. frontend same-origin همچنان بتواند استریم را دریافت کند _(verify: static)_
