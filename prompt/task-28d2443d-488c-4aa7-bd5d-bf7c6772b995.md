---
task_id: 28d2443d-488c-4aa7-bd5d-bf7c6772b995
title: تابع run_analysis_stream در analysis.py بیش از ۱۸۰ خط با منطق تودرتو و file-walk تکراری دارد
type: refactor
priority: high
...
created_at: '2026-06-14T09:39:53.681760+00:00'
updated_at: '2026-06-14T09:42:30.807909+00:00'
target_files:
- backend/app/api/routes/analysis.py
- backend/app/api/routes/analysis.py
---

# تابع run_analysis_stream در analysis.py بیش از ۱۸۰ خط با منطق تودرتو و file-walk تکراری دارد

## Raw Idea

endpoint `run_analysis_stream` در فایل analysis.py (که خود ۷۰۷ خط است) شامل توابع تودرتوی عمیق (generate_events → run_analysis_task) با منطق file-collection است که عملاً همان منطق file-walk موجود در `analysis.py` خط ۱۶۰-۱۸۰ و نیز در `deep_analysis_service` تکرار شده. extension whitelist، فیلتر دایرکتوری‌های node_modules/.git/__pycache__ و خواندن README به‌صورت inline در یک تابع نوشته شده‌اند. این over-engineering یک endpoint را تست‌ناپذیر و نگهداری‌ناپذیر می‌کند. همچنین DB session (`analysis_db = SessionLocal()`) در خط ۱۱۷ باز می‌شود اما هرگز در finally بسته نمی‌شود → connection leak.
...
- `backend/app/api/routes/analysis.py:105-212` — `run_analysis_stream.generate_events.run_analysis_task` — تابع تودرتوی غول‌پیکر + DB leak + duplicate file-walk
  ```python
  from ...core.database import SessionLocal
  analysis_db = SessionLocal()
  # ... هیچ analysis_db.close() در finally نیست
  for root, dirs, filenames in os.walk(project_path):
      dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', ...}]
  ```
- `backend/app/api/routes/analysis.py:152-180` — `supported_extensions` — extension whitelist تکراری که باید مرکزی شود
  ```python
...
FastAPI + SSE (StreamingResponse) + SQLAlchemy SessionLocal + os.walk

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/deep_analysis_service.py` — همین منطق file-collection احتمالاً اینجا هم تکرار شده
- `backend/app/services/project_analyzer.py` — analyze_project نیز فایل‌ها را جمع می‌کند
- `backend/app/core/database.py` — SessionLocal که leak می‌شود
...
## 🌐 نقشهٔ وابستگی‌ها
core/database.py توسط ۶۰ فایل import می‌شود؛ یک helper مرکزی برای session management و file-collection می‌تواند leak‌های مشابه را در سراسر routes کاهش دهد.
...
- [ ] تابع جدید collect_project_files با امضای (project_path, roadmap_path) در یک ماژول utility وجود دارد و توسط run_analysis_stream استفاده می‌شود
- [ ] analysis_db.close() در یک finally block فراخوانی می‌شود
- [ ] extension whitelist و directory-ignore-set به‌صورت ثابت ماژول‌سطح تعریف شده (نه inline داخل تابع)
- [ ] تست واحد برای collect_project_files که فیلتر node_modules و خواندن README را پوشش دهد اضافه شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. منطق جمع‌آوری فایل‌ها (file-walk + extension whitelist + README/roadmap reading) را به یک helper مستقل مثل `collect_project_files(project_path, roadmap_path)` در یک ماژول utility (مثلاً `app/services/file_collector.py`) استخراج کن. تابع `run_analysis_task` را flatten کن. DB session را در یک try/finally یا context manager قرار بده تا حتماً close شود.
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/ -k file_collector`
- `python -c "from app.services.file_collector import collect_project_files"`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر منطق SSE ممکن است رفتار stream را تغییر دهد؛ نیاز به تست end-to-end تحلیل
...
1. تابع جدید collect_project_files با امضای (project_path, roadmap_path) در یک ماژول utility وجود دارد و توسط run_analysis_stream استفاده می‌شود _(verify: static)_
2. analysis_db.close() در یک finally block فراخوانی می‌شود _(verify: static)_
3. extension whitelist و directory-ignore-set به‌صورت ثابت ماژول‌سطح تعریف شده (نه inline داخل تابع) _(verify: static)_
4. تست واحد برای collect_project_files که فیلتر node_modules و خواندن README را پوشش دهد اضافه شود _(verify: static)_
