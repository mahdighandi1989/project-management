---
task_id: e3991819-2406-41bd-887d-9e7d8fb00f24
title: Duplication of AI Generation Core Logic
type: refactor
priority: critical
...
created_at: '2026-06-07T09:28:58.822563+00:00'
updated_at: '2026-06-07T09:32:08.932801+00:00'
target_files:
- backend/app/api/routes/project_memory.py
- backend/app/api/routes/simple_projects.py
---

# Duplication of AI Generation Core Logic

## Raw Idea

The core AI generation functions, `ai_generate_with_meta` and `ai_generate`, are duplicated across `project_memory.py` and `simple_projects.py`. This leads to redundant code, increased maintenance burden, and potential for inconsistent behavior or bugs if one version is updated without the other. This fundamental AI interaction logic should reside in a single, shared utility module.
...
- `backend/app/api/routes/project_memory.py:93-180` — `ai_generate_with_meta`
  ```python
  ...
  async def ai_generate(
      ...
      *,
      ...
      return content
  ```
- `backend/app/api/routes/simple_projects.py:93-180` — `ai_generate_with_meta`
  ```python
  async def ai_generate_with_meta(
      prompt: str,
      model_ids: Optional[List[str]] = None,
...
FastAPI backend, Python AI services, Pydantic models.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/ai_manager.py` — Manages AI model interactions and is imported by the duplicated functions.
- `backend/app/services/ai_base.py` — Defines base AI message structures used by the duplicated functions.
- `backend/app/core/models_registry.py` — Provides model information used for selection and fallback.
...
## 🌐 نقشهٔ وابستگی‌ها
These functions are fundamental for AI interaction, used by various project creation and management flows. Centralizing them will ensure consistent model selection, error handling, and future enhancements.
...
- [ ] The `ai_generate_with_meta` and `ai_generate` functions are removed from `project_memory.py` and `simple_projects.py`.
- [ ] A new module `backend/app/services/ai_core_utils.py` contains the consolidated logic for these functions.
- [ ] All callers in `project_memory.py` and `simple_projects.py` correctly import and use the functions from `ai_core_utils.py`.
- [ ] Existing functionality that relies on these functions (e.g., project creation, memory field execution) works as expected.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Extract `ai_generate_with_meta` and `ai_generate` into a new shared module, e.g., `backend/app/services/ai_core_utils.py`. Update both `project_memory.py` and `simple_projects.py` to import and use these centralized functions. Ensure all necessary imports (like `get_ai_manager` and `Message`) are handled correctly in the new module.
...
async def ai_generate_with_meta(...): # ... duplicated logic ...
```

_بعد:_
```
from ...services.ai_core_utils import ai_generate_with_meta, ai_generate
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/test_simple_creator.py`
- `pytest backend/tests/test_project_memory.py`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential for breaking AI generation flows if imports or logic are not correctly refactored; requires thorough testing of all AI-driven features.
...
1. The `ai_generate_with_meta` and `ai_generate` functions are removed from `project_memory.py` and `simple_projects.py`. _(verify: static)_
2. A new module `backend/app/services/ai_core_utils.py` contains the consolidated logic for these functions. _(verify: static)_
3. All callers in `project_memory.py` and `simple_projects.py` correctly import and use the functions from `ai_core_utils.py`. _(verify: static)_
4. Existing functionality that relies on these functions (e.g., project creation, memory field execution) works as expected. _(verify: backend_test)_
