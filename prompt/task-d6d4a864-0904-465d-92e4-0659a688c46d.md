---
task_id: d6d4a864-0904-465d-92e4-0659a688c46d
title: 'Technical Debt: Deprecated Project Health Analysis with Overlapping Functionality'
type: refactor
priority: medium
...
created_at: '2026-06-07T09:30:21.015127+00:00'
updated_at: '2026-06-07T09:34:52.613115+00:00'
target_files:
...
# Technical Debt: Deprecated Project Health Analysis with Overlapping Functionality

## Raw Idea

The `backend/app/api/routes/project_health.py` module is explicitly marked as deprecated, with a clear migration path suggested to `/api/oversight/scan` and `/api/oversight/codex`. However, `backend/app/api/routes/analysis.py` also provides project analysis functionality, including streaming progress, which overlaps with the deprecated module. This creates confusion about the intended primary analysis endpoint and leaves redundant code paths active.
...
- `backend/app/api/routes/project_health.py:63-82` — `deprecated_health_endpoint`
  ```python
...
- `backend/app/api/routes/analysis.py:83-268` — `run_analysis_stream`
  ...
  async def run_analysis_stream(request: AnalysisRequest):
      ...
      - زمان سپری شده
      """
      # ... (truncated for brevity, contains logic for streaming analysis) ...
  ```
- `backend/app/api/routes/oversight.py:57-80` — `WatchedUpdate`
  ```python
  class WatchedUpdate(BaseModel):
      ...
      # ... (truncated for brevity, includes other scan-related fields) ...
  ```
...
FastAPI, project analysis, background tasks, SSE, database interactions.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/app/analysis/page.tsx` — Currently uses `/api/analysis/run-stream` for analysis.
- `frontend/src/app/oversight/page.tsx` — The intended target for comprehensive project scanning and health features.
- `backend/app/services/project_health_analyzer.py` — Service used by the deprecated health analysis.
- `backend/app/services/deep_analysis_service.py` — Service used by the active analysis endpoint.
...
## 🌐 نقشهٔ وابستگی‌ها
The deprecation of `project_health.py` is a clear signal for refactoring. The overlapping functionality with `analysis.py` and the designated successor `oversight.py` requires a consolidated strategy to avoid technical debt and confusion.
...
- [ ] All unique and valuable features from `project_health.py` are migrated to `analysis.py` or `oversight.py`.
- [ ] `project_health.py` is completely removed from the codebase.
- [ ] The `analysis` and `oversight` modules have clearly defined responsibilities for project analysis.
- [ ] Frontend components are updated to use the correct, non-deprecated endpoints.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Complete the migration of all relevant functionality from `project_health.py` to `oversight.py` and/or `analysis.py`. Once fully migrated, `project_health.py` should be removed. Clarify the roles of `analysis.py` (e.g., for ad-hoc, streaming analysis) and `oversight.py` (for scheduled, comprehensive scans and task generation) to avoid future overlaps.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `Verify that all project health analysis features are available and functional through `analysis.py` or `oversight.py`.`
- `Ensure no broken references or endpoints after removing `project_health.py`.`

## ⚠️ ریسک‌ها و موارد احتیاط
High risk of losing functionality or breaking existing analysis workflows if not all features are correctly migrated and tested.
...
1. All unique and valuable features from `project_health.py` are migrated to `analysis.py` or `oversight.py`. _(verify: api_response)_
2. `project_health.py` is completely removed from the codebase. _(verify: static)_
3. The `analysis` and `oversight` modules have clearly defined responsibilities for project analysis. _(verify: manual_only)_
4. Frontend components are updated to use the correct, non-deprecated endpoints. _(verify: ui_interaction)_

## Task Steps

### Step 1: تحلیل و شناسایی دقیق همپوشانی‌های عملکردی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق و موشکافانه کدهای موجود در `backend/app/api/routes/project_health.py` و `backend/app/api/routes/analysis.py` است. هدف، شناسایی دقیق تمام عملکردهای 'تحلیل پروژه' و 'پیشرفت استریمینگ' است که بین این دو ماژول همپوشانی دارند یا قرار است از ماژول منسوخ‌شده به مسیرهای جدید منتقل شوند. این مرحله صرفاً تحلیلی است و هیچ تغییری در کد اعمال نمی‌شود.
**Excerpt:**
```
The `backend/app/api/routes/project_health.py` module is explicitly marked as deprecated, with a clear migration path suggested to `/api/oversight/scan` and `/api/oversight/codex`. However, `backend/app/api/routes/analysis.py` also provides project analysis functionality, including streaming progress, which overlaps with the deprecated module. This creates confusion about the intended primary analysis endpoint and leaves redundant code paths active.
```

### Step 2: انتقال عملکردهای `project_health.py` به مسیرهای جدید `/api/oversight/scan` و `/api/oversight/codex`
**Status:** `pending` (0%)
**Scope:** در این مرحله، عملکردهای شناسایی شده از `backend/app/api/routes/project_health.py` که برای مهاجرت در نظر گرفته شده‌اند، به صورت کامل در مسیرهای جدید `/api/oversight/scan` و `/api/oversight/codex` پیاده‌سازی یا ادغام می‌شوند. این شامل ایجاد توابع جدید، کپی کردن منطق کسب‌وکار، و اطمینان از عملکرد صحیح در مسیرهای جدید است. ماژول `project_health.py` در این مرحله هنوز حذف نمی‌شود.
**Excerpt:**
```
The `backend/app/api/routes/project_health.py` module is explicitly marked as deprecated, with a clear migration path suggested to `/api/oversight/scan` and `/api/oversight/codex`.
```

### Step 3: بازسازی `backend/app/api/routes/analysis.py` برای حذف عملکردهای تکراری
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی `backend/app/api/routes/analysis.py` متمرکز است. هرگونه عملکرد 'تحلیل پروژه' یا 'پیشرفت استریمینگ' که اکنون توسط `/api/oversight/scan` و `/api/oversight/codex` پوشش داده می‌شود، از `backend/app/api/routes/analysis.py` حذف یا بازسازی می‌شود. هدف این است که `analysis.py` فقط عملکردهای منحصر به فرد خود را حفظ کند و از همپوشانی با ماژول‌های جدید جلوگیری شود.
**Excerpt:**
```
However, `backend/app/api/routes/analysis.py` also provides project analysis functionality, including streaming progress, which overlaps with the deprecated module. This creates confusion about the intended primary analysis endpoint and leaves redundant code paths active.
```

### Step 4: حذف ماژول منسوخ‌شده `backend/app/api/routes/project_health.py`
**Status:** `pending` (0%)
**Scope:** پس از اطمینان از انتقال کامل عملکردهای `backend/app/api/routes/project_health.py` و بازسازی `backend/app/api/routes/analysis.py`، این مرحله شامل حذف فیزیکی فایل `backend/app/api/routes/project_health.py` و هرگونه ارجاع مستقیم به آن در codebase است. این اقدام باید با دقت انجام شود تا از شکستن وابستگی‌های پنهان جلوگیری شود.
**Excerpt:**
```
The `backend/app/api/routes/project_health.py` module is explicitly marked as deprecated, with a clear migration path suggested to `/api/oversight/scan` and `/api/oversight/codex`.
```

### Step 5: به‌روزرسانی تمام فراخوان‌های داخلی به ماژول `project_health`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و به‌روزرسانی تمام نقاط در codebase است که قبلاً به `backend/app/api/routes/project_health.py` یا توابع/endpointهای آن ارجاع می‌دادند. این ارجاعات باید به مسیرهای جدید `/api/oversight/scan` یا `/api/oversight/codex` هدایت شوند. این کار شامل تغییر importها، فراخوانی توابع، و URLهای داخلی است.
**Excerpt:**
```
The `backend/app/api/routes/project_health.py` module is explicitly marked as deprecated, with a clear migration path suggested to `/api/oversight/scan` and `/api/oversight/codex`.
```

### Step 6: پیاده‌سازی و به‌روزرسانی تست‌های جامع
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های واحد (unit tests) و تست‌های یکپارچه‌سازی (integration tests) جدید برای مسیرهای `/api/oversight/scan` و `/api/oversight/codex` است. همچنین، تست‌های موجود برای `backend/app/api/routes/analysis.py` باید به‌روزرسانی شوند تا تغییرات اعمال شده در بازسازی آن را منعکس کنند و اطمینان حاصل شود که هیچ رگرسیونی رخ نداده است. این تست‌ها باید تمام سناریوهای مهم را پوشش دهند.
**Excerpt:**
```
This creates confusion about the intended primary analysis endpoint and leaves redundant code paths active.
```

### Step 7: به‌روزرسانی مستندات API و اطلاع‌رسانی به مصرف‌کنندگان خارجی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی تمام مستندات مرتبط با API (مانند OpenAPI specification، راهنماهای توسعه‌دهندگان) برای منعکس کردن منسوخ شدن `backend/app/api/routes/project_health.py` و معرفی مسیرهای جدید `/api/oversight/scan` و `/api/oversight/codex` به عنوان نقاط پایانی اصلی تحلیل است. همچنین، باید اطلاع‌رسانی‌های لازم به مصرف‌کنندگان خارجی API انجام شود تا از این تغییرات آگاه شوند.
**Excerpt:**
```
The `backend/app/api/routes/project_health.py` module is explicitly marked as deprecated, with a clear migration path suggested to `/api/oversight/scan` and `/api/oversight/codex`. However, `backend/app/api/routes/analysis.py` also provides project analysis functionality, including streaming progress, which overlaps with the deprecated module. This creates confusion about the intended primary analysis endpoint and leaves redundant code paths active.
```
