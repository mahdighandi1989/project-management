---
task_id: ad61c739-a9cc-455b-bb1f-69c894a2993a
title: Endpoint های GitHub Import بدون authentication و با امکان SSRF/توکن سراسری
type: security
priority: high
...
created_at: '2026-06-14T09:39:51.717016+00:00'
updated_at: '2026-06-14T09:42:12.859649+00:00'
target_files:
- backend/app/api/routes/github_import.py
- backend/app/api/routes/github_import.py
---

# Endpoint های GitHub Import بدون authentication و با امکان SSRF/توکن سراسری

## Raw Idea

تمام endpoint های روتر github_import (check, import, parse-url, refresh) هیچ لایه authentication ندارند. هر کاربری که به سرور دسترسی دارد می‌تواند با `use_global_token=true` از توکن GitHub سراسری ذخیره‌شده در env (GITHUB_TOKEN) برای دسترسی به repoهای خصوصی استفاده کند، بدون اینکه خودش توکن داشته باشد. این یک privilege escalation است: مهاجم می‌تواند با توکن سرور به repoهای خصوصی صاحب سرور دسترسی پیدا کند. علاوه بر این، check_repository و import_repository ورودی url را بدون اعتبارسنجی کافی به سرویس می‌دهند که می‌تواند بستر SSRF باشد اگر parse_github_url هرگونه URL را بپذیرد.
...
- `backend/app/api/routes/github_import.py:45-52` — `get_effective_token` — بازگرداندن توکن سراسری بدون هیچ بررسی احراز هویت
  ```python
  def get_effective_token(provided_token: str = None, use_global: bool = False) -> str:
      ...
      return ""
  ```
- `backend/app/api/routes/github_import.py:55-91` — `check_repository` — endpoint بدون auth که می‌تواند از توکن سراسری استفاده کند
  ```python
  @router.post("/check")
...
FastAPI APIRouter + GitHub REST API + توکن env سراسری

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/api/routes/external_prompts.py` (سطر 102) — الگوی صحیح require_external_token/require_admin_token برای محافظت endpoint اینجاست
- `backend/app/services/github_import.py` — سرویسی که توکن را برای فراخوانی GitHub API استفاده می‌کند
- `backend/app/main.py` (سطر 19) — روتر github_import بدون dependency محافظتی mount می‌شود
...
## 🌐 نقشهٔ وابستگی‌ها
روتر github_import در main.py mount شده و سرویس github_import توسط oversight و auto_setup هم استفاده می‌شود؛ اضافه کردن auth فقط روی این روتر مسیرهای ورودی public را می‌بندد بدون اثر روی فراخوانی‌های داخلی سرویس.
...
- [ ] فراخوانی /github/check و /github/import بدون توکن معتبر با 401 رد شود
- [ ] use_global_token=true فقط برای کاربر/admin احرازشده توکن سراسری را برمی‌گرداند
- [ ] url با host غیر github.com در import رد شود (whitelist)
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. روی روتر github_import یک dependency authentication اضافه کن (مشابه require_external_token در external_prompts.py یا require_admin_token). به‌خصوص پارامتر use_global_token باید فقط برای کاربر احرازشده/admin مجاز باشد. ورودی url را در get_effective_token و قبل از فراخوانی import محدود کن تا فقط hostهای github.com مجاز باشند (whitelist) تا SSRF بسته شود.

## 💡 نمونه‌های قبل/بعد
**محافظت use_global_token پشت auth**
...
```
if use_global:
    # فقط برای درخواست احرازشده/admin مجاز است
    require_admin_token(...)
    return os.environ.get("GITHUB_TOKEN", "")
```
...
## 🧪 دستورات اعتبارسنجی
- `curl -i -X POST http://localhost:8000/api/github/check -d '{"url":"owner/repo","use_global_token":true}' -H 'Content-Type: application/json'`
- `pytest backend/tests -k github_import`

## ⚠️ ریسک‌ها و موارد احتیاط
ممکن است flowهای فعلی که بدون توکن به این endpoint وابسته‌اند بشکنند؛ نیاز به به‌روزرسانی frontend برای ارسال auth header
...
1. فراخوانی /github/check و /github/import بدون توکن معتبر با 401 رد شود _(verify: static)_
2. use_global_token=true فقط برای کاربر/admin احرازشده توکن سراسری را برمی‌گرداند _(verify: static)_
3. url با host غیر github.com در import رد شود (whitelist) _(verify: static)_
