---
task_id: a8dd377e-7d7a-4e65-9a67-5ab42ef71e7d
title: notification بدون event_type صریح در oversight_inspector_bridge.py
...
created_at: '2026-06-07T09:30:54.234280+00:00'
updated_at: '2026-06-07T09:30:56.614902+00:00'
target_files:
- backend/app/services/oversight_inspector_bridge.py
---

# notification بدون event_type صریح در oversight_inspector_bridge.py

## Raw Idea

## 📋 شرح
در `backend/app/services/oversight_inspector_bridge.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
---
[scan #2 at 2026-06-07T09:30:54.912519+00:00]
## 📋 شرح
در `backend/app/services/oversight_verifier.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
## 🔍
---
[scan #3 at 2026-06-07T09:30:55.236161+00:00]
## 📋 شرح
در `backend/app/services/scan_v5/_findings_to_tasks.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.

## 🤔 چرا مهم است
event_type کلید routing و filter در سیستم notification است. بدون آن، نمی‌توان آن event را به‌صورت per-event mute/customize کر
---
[scan #4 at 2026-06-07T09:30:55.583706+00:00]
## 📋 شرح
در `backend/app/services/task_merge_service.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
## 🔍
---
[scan #5 at 2026-06-07T09:30:55.935752+00:00]
## 📋 شرح
در `backend/app/services/ai_balance_service.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
## 🔍
---
[scan #6 at 2026-06-07T09:30:56.293383+00:00]
## 📋 شرح
در `backend/app/services/oversight_model_temp_activate.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.

## 🤔 چرا مهم است
event_type کلید routing و filter در سیستم notification است. بدون آن، نمی‌توان آن event را به‌صورت per-event mute/customize
...
- `backend/app/services/oversight_inspector_bridge.py:902`
...
- `backend/app/services/oversight_service.py` — `oversight_inspector_bridge.py` این فایل را import می‌کند
- `backend/app/services/ai_manager.py` — `oversight_inspector_bridge.py` این فایل را import می‌کند
- `backend/app/core/models_registry.py` — `oversight_inspector_bridge.py` این فایل را import می‌کند
- `backend/app/services/ai_base.py` — `oversight_inspector_bridge.py` این فایل را import می‌کند
- `backend/app/api/routes/oversight.py` — این فایل `oversight_inspector_bridge.py` را import می‌کند (caller)
- `backend/app/services/inspector_recording_processor.py` — این فایل `oversight_inspector_bridge.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/services/oversight_inspector_bridge.py`
- `ruff check backend/app/services/oversight_inspector_bridge.py`
- `pytest -x`
...
3. از UI tab notification settings این event قابل toggle است _(verify: static)_
