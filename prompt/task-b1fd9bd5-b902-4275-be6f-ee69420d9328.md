---
task_id: b1fd9bd5-b902-4275-be6f-ee69420d9328
title: '[منطق] عدم وجود user context در مدل‌ها و سرویس‌ها'
type: logic_audit
priority: high
...
created_at: '2026-05-31T09:08:29.863248+00:00'
updated_at: '2026-05-31T09:08:29.863252+00:00'
---

# [منطق] عدم وجود user context در مدل‌ها و سرویس‌ها
...
مدل AILog و InspectorSession فاقد فیلدهایی مانند user_id یا owner هستند. سرویس OversightUploadSession نیز هیچ ارجاعی به کاربر ایجادکننده session ندارد. این بدان معناست که امکان تفکیک داده‌ها بر اساس کاربر وجود ندارد.

## 💥 پیامد (impact)
نمی‌توان تشخیص داد کدام کاربر کدام لاگ یا session را ایجاد کرده است. این امر باعث می‌شود هرگونه permission checking در آینده غیرممکن باشد و داده‌ها به صورت global و بدون ایزولیشن ذخیره شوند.

## 🛠 پیشنهاد رفع اولیه
به مدل AILog فیلد user_id (ForeignKey به User model) اضافه کنید. به InspectorSession نیز user_id اضافه کنید. در OversightUploadSession، user_id را به عنوان بخشی از state ذخیره کنید و در تمام queryها فیلتر بر اساس user_id اعمال شود.
...
### Step 1: اضافه کردن فیلد user_id به مدل AILog
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن فیلد user_id از نوع ForeignKey به مدل User در مدل AILog است. باید migration مربوطه ایجاد شود. خارج از این مرحله: تغییر در InspectorSession، OversightUploadSession، یا هر query/سرویس دیگر. نکته حیاتی: اطمینان از اینکه فیلد nullable نیست و برای رکوردهای موجود مقدار پیش‌فرض ندارد (نیاز به data migration یا انتخاب کاربر).
**Excerpt:**
```
به مدل AILog فیلد user_id (ForeignKey به User model) اضافه کنید.
```

### Step 2: اضافه کردن فیلد user_id به مدل InspectorSession
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن فیلد user_id از نوع ForeignKey به مدل User در مدل InspectorSession است. migration مربوطه ایجاد شود. خارج از این مرحله: تغییر در AILog، OversightUploadSession، یا queryها. نکته حیاتی: اطمینان از اینکه فیلد nullable نیست و برای sessionهای موجود نیاز به data migration است.
**Excerpt:**
```
به InspectorSession نیز user_id اضافه کنید.
```

### Step 3: ذخیره user_id در OversightUploadSession state
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن user_id به عنوان بخشی از state در OversightUploadSession است. باید در زمان ایجاد session، user_id از کاربر جاری استخراج و ذخیره شود. خارج از این مرحله: تغییر در AILog، InspectorSession، یا queryهای فیلتر. نکته حیاتی: نحوه ذخیره state (مثلاً در دیکشنری state یا فیلد جداگانه) باید مشخص شود.
**Excerpt:**
```
در OversightUploadSession، user_id را به عنوان بخشی از state ذخیره کنید
```

### Step 4: اعمال فیلتر user_id در تمام queryهای مربوط به AILog
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی تمام queryهایی که از مدل AILog استفاده می‌کنند تا بر اساس user_id کاربر جاری فیلتر شوند. باید در سرویس‌ها و endpointهای مربوطه اعمال شود. خارج از این مرحله: queryهای InspectorSession و OversightUploadSession. نکته حیاتی: اطمینان از اینکه کاربران admin بتوانند همه لاگ‌ها را ببینند (اگر نیاز است).
— [merged] این مرحله شامل به‌روزرسانی تمام queryهایی که از مدل InspectorSession استفاده می‌کنند تا بر اساس user_id کاربر جاری فیلتر شوند. خارج از این مرحله: queryهای AILog و OversightUploadSession. نکته حیاتی: اطمینان از اینکه sessionهای عمومی (اگر وجود دارند) مدیریت شوند.
— [merged] این مرحله شامل به‌روزرسانی تمام queryهایی که از OversightUploadSession استفاده می‌کنند تا بر اساس user_id (که در state ذخیره شده) فیلتر شوند. خارج از این مرحله: queryهای AILog و InspectorSession. نکته حیاتی: نحوه استخراج user_id از state و اعمال فیلتر باید مشخص شود.
**Excerpt:**
```
در تمام queryها فیلتر بر اساس user_id اعمال شود.
```
