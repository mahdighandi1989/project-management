---
task_id: 52df5398-bff6-4dda-9c20-5c68f119a3b9
title: Embedded Generic Utility Functions in API Routes
type: refactor
priority: medium
...
created_at: '2026-06-07T09:30:21.355722+00:00'
updated_at: '2026-06-07T09:35:04.179287+00:00'
target_files:
...
# Embedded Generic Utility Functions in API Routes

## Raw Idea

Several API route files contain generic utility functions that are not directly related to API endpoint definition but rather to data processing, file operations, or domain-specific logic. Examples include `extract_technologies_from_project` in `orchestrator.py`, `extract_code_blocks` and `commit_to_github` in `project_memory.py`, and issue merging logic in `project_health.py`. This practice reduces modularity, makes code harder to reuse, and bloats route files.
...
- `backend/app/api/routes/orchestrator.py:22-129` — `extract_technologies_from_project`
  ```python
  def extract_technologies_from_project(project: Dict, estimated_files: List[str]) -> List[str]:
      ...
      """
      technologies = set()
      # ... (truncated for brevity, contains logic for technology extraction) ...
      return list(technologies)
  ```
- `backend/app/api/routes/project_memory.py:346-400` — `commit_to_github`
  ```python
  async def commit_to_github(
      ...
      # ... (truncated for brevity, contains GitHub API interaction logic) ...
      return {
          "success": True,
          "path": file_path,
  ...
  ```
- `backend/app/api/routes/project_health.py:203-284` — `_merge_with_existing_issues`
  ```python
  def _merge_with_existing_issues(project, new_issues: list, source: str = "analysis", db=None) -> list:
      ...
      # ... (truncated for brevity, contains issue merging logic) ...
...
Python utility functions, file system operations, GitHub API interaction, data processing.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/github_import.py` — Also interacts with GitHub repositories.
- `backend/app/services/project_analyzer.py` — Could benefit from shared analysis utilities.
- `backend/app/services/smart_orchestrator.py` — The orchestrator service could use extracted utilities.
...
## 🌐 نقشهٔ وابستگی‌ها
These utility functions are generic enough to be reused across different parts of the application. Extracting them will improve code organization, testability, and reduce duplication.
...
- [ ] All identified generic utility functions are moved out of API route files into appropriate service or utility modules.
- [ ] The original route files are updated to import and use the new utility modules.
- [ ] All functionality remains intact and passes existing tests.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Extract these generic utility functions into dedicated service or utility modules based on their domain. For instance, file-related utilities to `backend/app/utils/file_utils.py`, GitHub operations to `backend/app/services/github_operations_service.py`, and project analysis helpers to `backend/app/services/project_analysis_utils.py`. Update the original route files to import and use these new centralized utilities.
...
- `Manually test features that rely on these utilities (e.g., project creation, GitHub commit, health analysis).`

## ⚠️ ریسک‌ها و موارد احتیاط
Moderate risk of breaking functionality if imports or function signatures are not correctly updated during extraction.
...
1. All identified generic utility functions are moved out of API route files into appropriate service or utility modules. _(verify: static)_
2. The original route files are updated to import and use the new utility modules. _(verify: static)_
3. All functionality remains intact and passes existing tests. _(verify: backend_test)_
