---
task_id: 40cf9a16-b2ab-4cdc-9763-524d51ece357
title: محدود کردن CORS Origins برای محیط تولید
type: security
priority: medium
...
created_at: '2026-06-07T09:29:38.471240+00:00'
updated_at: '2026-06-07T09:34:29.424387+00:00'
target_files:
- backend/app/main.py
- backend/app/main.py
---

# محدود کردن CORS Origins برای محیط تولید

## Raw Idea

میان‌افزار `CORSMiddleware` در `backend/app/main.py` از `settings.CORS_ORIGINS` برای پیکربندی سیاست CORS استفاده می‌کند. اگر `settings.CORS_ORIGINS` در محیط تولید به `["*"]` تنظیم شده باشد، این امر به هر وب‌سایتی اجازه می‌دهد تا درخواست‌های Cross-Origin به بک‌اند ارسال کند. این می‌تواند یک خطر امنیتی باشد و حملات CSRF-مانند یا استخراج داده را تسهیل کند، به خصوص اگر کوکی‌ها یا توکن‌های احراز هویت درگیر باشند.
...
- `backend/app/main.py:6-7` — پیکربندی `CORSMiddleware` از `settings.CORS_ORIGINS` استفاده می‌کند.
  ```python
  from fastapi import FastAPI, Request, Response
  from fastapi.middleware.cors import CORSMiddleware
  ```
- `backend/app/main.py:13-17` — خطی که `allow_origins` را از `settings.CORS_ORIGINS` می‌خواند.
  ```python
  app.add_middleware(
      CORSMiddleware,
...
FastAPI, CORS, Environment Variables

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/config.py` — فایلی که `settings.CORS_ORIGINS` در آن تعریف شده است.
- `frontend/next.config.js` — پیکربندی دامنه فرانت‌اند.
...
## 🌐 نقشهٔ وابستگی‌ها
این یک تغییر پیکربندی است که برای امنیت تولید حیاتی است و نیاز به مدیریت دقیق متغیرهای محیطی دارد.
...
- [ ] `CORS_ORIGINS` در محیط تولید به دامنه‌های خاص و مورد اعتماد محدود شده است.
- [ ] در محیط توسعه، `CORS_ORIGINS` اجازه دسترسی از `localhost` و/یا `*` را می‌دهد.
- [ ] مستندات استقرار شامل دستورالعمل‌هایی برای پیکربندی `CORS_ORIGINS` است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. اطمینان حاصل کنید که `settings.CORS_ORIGINS` در محیط تولید به طور سخت‌گیرانه فقط برای دامنه‌های فرانت‌اند مجاز پیکربندی شده است. برای محیط توسعه، می‌توان `["*"]` یا `http://localhost:3000` را به صورت شرطی مجاز دانست.

## 💡 نمونه‌های قبل/بعد
**قبل از محدودیت CORS (در config.py)**
...
## 🧪 دستورات اعتبارسنجی
- `بررسی مقدار `settings.CORS_ORIGINS` در `backend/app/core/config.py`.`
- `استقرار در محیط staging و تلاش برای درخواست‌های Cross-Origin از یک دامنه غیرمجاز (باید مسدود شود).`

## ⚠️ ریسک‌ها و موارد احتیاط
پیکربندی نادرست CORS می‌تواند دسترسی قانونی فرانت‌اند را مسدود کند.
...
1. `CORS_ORIGINS` در محیط تولید به دامنه‌های خاص و مورد اعتماد محدود شده است. _(verify: backend_test)_
2. در محیط توسعه، `CORS_ORIGINS` اجازه دسترسی از `localhost` و/یا `*` را می‌دهد. _(verify: backend_test)_
3. مستندات استقرار شامل دستورالعمل‌هایی برای پیکربندی `CORS_ORIGINS` است. _(verify: manual_only)_
