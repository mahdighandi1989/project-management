---
task_id: 972d851c-af6c-410a-8e85-276f555636aa
title: Insufficient error handling for GitHub API calls in commit_to_github
type: bug
priority: high
...
created_at: '2026-06-07T09:29:19.510379+00:00'
updated_at: '2026-06-07T09:34:01.401331+00:00'
target_files:
- backend/app/api/routes/project_memory.py
---

# Insufficient error handling for GitHub API calls in commit_to_github

## Raw Idea

The `commit_to_github` function, responsible for committing files to a GitHub repository, uses a bare `except: pass` when attempting to fetch the SHA of an existing file. This silently swallows critical errors such as network issues, API rate limits, or invalid credentials during the GET request to GitHub. This can lead to incorrect behavior where files are always created (if SHA fetch fails) instead of updated, or other unexpected outcomes without any logging or user feedback, making the GitHub integration unreliable and difficult to debug.
...
- `backend/app/api/routes/project_memory.py:379` — Silent exception handling during SHA retrieval from GitHub
  ```python
  except:
              pass
  ```
...
FastAPI, aiohttp, GitHub API, Asynchronous Programming

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/github_storage.py` — Likely uses similar GitHub API interaction patterns.
- `backend/app/core/logging_utils.py` — For structured error logging.
- `backend/app/api/routes/oversight.py` — Uses GitHub for prompt synchronization and could be affected by unreliable GitHub API calls.
...
## 🌐 نقشهٔ وابستگی‌ها
This function is crucial for persisting AI-generated code changes to GitHub, a core feature of the project. Unreliable commits can lead to data loss or inconsistent repository states.
...
- [ ] GitHub API errors during SHA retrieval are logged on the backend.
- [ ] The `commit_to_github` function provides more specific error messages in its return value if the PUT request fails.
- [ ] Files are correctly updated (not duplicated) when `commit_to_github` is called with existing files.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Replace the bare `except: pass` with specific exception handling for `aiohttp.ClientError` or `asyncio.TimeoutError` and log the error with `logger.error` including `exc_info=True`. Additionally, enhance error handling for the `session.put` request to provide more informative feedback based on GitHub API response status codes.

## 💡 نمونه‌های قبل/بعد
**Specific exception handling for GitHub GET request**
...
```
            ...
            logger.error(f"GitHub API GET failed for {api_url}: {e}", exc_info=True)
        except asyncio.TimeoutError:
            logger.error(f"GitHub API GET timed out for {api_url}")
```
...
## 🧪 دستورات اعتبارسنجی
- `Configure a GitHub repository and attempt to commit a file, then commit an update to the same file, observing backend logs and GitHub repository state.`
- `Simulate network issues or invalid GitHub tokens to trigger and verify error logging.`

## ⚠️ ریسک‌ها و موارد احتیاط
Low, primarily involves improving error visibility and robustness.
...
1. GitHub API errors during SHA retrieval are logged on the backend. _(verify: backend_test)_
2. The `commit_to_github` function provides more specific error messages in its return value if the PUT request fails. _(verify: api_response)_
3. Files are correctly updated (not duplicated) when `commit_to_github` is called with existing files. _(verify: backend_test)_
