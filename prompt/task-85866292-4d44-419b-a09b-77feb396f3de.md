---
task_id: 85866292-4d44-419b-a09b-77feb396f3de
title: Information leakage in external token validation
type: security
priority: high
...
created_at: '2026-06-07T09:29:19.124890+00:00'
updated_at: '2026-06-07T09:33:49.900526+00:00'
target_files:
- backend/app/api/routes/external_prompts.py
- backend/app/api/routes/external_prompts.py
---

# Information leakage in external token validation

## Raw Idea

The `_check_external_token` and `require_admin_token` functions expose sensitive server configuration details to external callers. If the `EXTERNAL_TOOL_TOKEN` or `ADMIN_TOKEN` environment variables are not set, the API returns a `503` error with a `detail` message explicitly stating that the environment variable is not configured. This information could be used by attackers to understand the server's setup and identify potential misconfigurations.
...
- `backend/app/api/routes/external_prompts.py:95` — Error message for missing EXTERNAL_TOOL_TOKEN
  ```python
  detail="EXTERNAL_TOOL_TOKEN env var تنظیم نشده — endpoint غیرفعال است.",
  ```
- `backend/app/api/routes/external_prompts.py:115` — Error message for missing ADMIN_TOKEN
  ```python
...
FastAPI, Environment Variables, Security

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/config.py` — Configuration file where these environment variables are typically managed.
- `backend/app/core/logging_utils.py` — For logging the internal reason for the failure.
...
## 🌐 نقشهٔ وابستگی‌ها
These functions are critical for securing external API access, including integrations with Cloud Code and administrative endpoints. The information leakage affects the overall security posture.
...
- [ ] External API calls with missing `EXTERNAL_TOOL_TOKEN` or `ADMIN_TOKEN` env vars return a generic error message.
- [ ] The specific reason for the token configuration issue is logged on the backend, not exposed to the client.
- [ ] No sensitive environment variable names are exposed in API responses.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Modify `_check_external_token` and `require_admin_token` to return a generic error message (e.g., 'Authentication failed' or 'Service unavailable') when the expected token is not configured, without revealing the specific internal reason. The detailed reason should only be logged internally.

## 💡 نمونه‌های قبل/بعد
**Generic error message for missing token**
...
_بعد:_
```
detail="Service unavailable due to misconfiguration.", # Log the specific reason internally
```
...
## 🧪 دستورات اعتبارسنجی
- `Unset `EXTERNAL_TOOL_TOKEN` and `ADMIN_TOKEN` environment variables.`
- `Make a `GET /external/prompts/next` request and verify the error message.`
- `Make a `POST /oversight/backfill-all-prompts` request and verify the error message.`

## ⚠️ ریسک‌ها و موارد احتیاط
Low, involves changing error message strings.
...
1. External API calls with missing `EXTERNAL_TOOL_TOKEN` or `ADMIN_TOKEN` env vars return a generic error message. _(verify: api_response)_
2. The specific reason for the token configuration issue is logged on the backend, not exposed to the client. _(verify: backend_test)_
3. No sensitive environment variable names are exposed in API responses. _(verify: api_response)_
