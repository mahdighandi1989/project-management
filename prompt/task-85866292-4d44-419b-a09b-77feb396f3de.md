...
updated_at: '2026-06-07T09:29:19.124892+00:00'
target_files:
...
1. External API calls with missing `EXTERNAL_TOOL_TOKEN` or `ADMIN_TOKEN` env vars return a generic error message. _(verify: static)_
2. The specific reason for the token configuration issue is logged on the backend, not exposed to the client. _(verify: static)_
3. No sensitive environment variable names are exposed in API responses. _(verify: static)_
