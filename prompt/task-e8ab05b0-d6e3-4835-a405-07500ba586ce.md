...
updated_at: '2026-06-07T09:28:40.713504+00:00'
target_files:
...
1. یک تابع `sanitize_path(path: str, base_dir: str)` ایجاد می‌شود که مسیرهای امن را برمی‌گرداند.
2. تمام مسیرهای API که عملیات نوشتن فایل را انجام می‌دهند، از `sanitize_path` استفاده می‌کنند.
3. تلاش برای استفاده از دنباله‌های Path Traversal (مانند `../`) یا مسیرهای مطلق منجر به خطای 400 یا 403 می‌شود.
4. تست‌های واحد برای اعتبارسنجی مسیر اضافه می‌شوند.
