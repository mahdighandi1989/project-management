---
task_id: e8ab05b0-d6e3-4835-a405-07500ba586ce
title: اعتبارسنجی دقیق مسیر فایل برای جلوگیری از Path Traversal
type: security
priority: critical
...
created_at: '2026-06-07T09:28:40.713501+00:00'
updated_at: '2026-06-07T09:31:41.838832+00:00'
target_files:
...
# اعتبارسنجی دقیق مسیر فایل برای جلوگیری از Path Traversal

## Raw Idea

مسیرهای API که به کاربران یا مدل‌های AI اجازه می‌دهند مسیر فایل‌ها را برای عملیات ذخیره، آپلود یا Commit به GitHub مشخص کنند (مانند `project_memory.py` و `unified_api.py`)، فاقد اعتبارسنجی کافی برای جلوگیری از حملات Path Traversal هستند. یک مهاجم می‌تواند از دنباله‌های `../` برای نوشتن فایل در مکان‌های دلخواه خارج از دایرکتوری پروژه استفاده کند که می‌تواند منجر به اجرای کد از راه دور (RCE) یا خرابی داده شود.
...
- `backend/app/api/routes/project_memory.py:40-51` — `target_path` در `DynamicFieldRequest` توسط کاربر/AI کنترل می‌شود و بدون اعتبارسنجی استفاده می‌شود.
  ```python
  ...
  ```
- `backend/app/api/routes/unified_api.py:153-164` — `file_path` و `folder` در `save_file` و `upload_file` توسط کاربر کنترل می‌شوند.
  ```python
  ...
  ```
- `backend/app/api/routes/orchestrator.py:346-351` — `file_path` در `commit_to_github` (که توسط `project_memory` استفاده می‌شود) باید اعتبارسنجی شود.
  ```python
...
FastAPI, Python `os.path`, GitHub API

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/services/github_storage.py` — سرویس ذخیره‌سازی GitHub که عملیات Commit را انجام می‌دهد.
- `backend/app/services/unified_storage.py` — سرویس ذخیره‌سازی یکپارچه که عملیات فایل را انجام می‌دهد.
- `backend/app/api/routes/simple_projects.py` — مسیرهای ایجاد پروژه نیز ممکن است به این آسیب‌پذیری مرتبط باشند.
...
## 🌐 نقشهٔ وابستگی‌ها
این آسیب‌پذیری بسیار حیاتی است و نیاز به پیاده‌سازی دقیق پاکسازی مسیر در لایه API و سرویس‌های فایل دارد تا از دسترسی غیرمجاز به فایل‌های سیستمی جلوگیری شود.
...
- [ ] یک تابع `sanitize_path(path: str, base_dir: str)` ایجاد می‌شود که مسیرهای امن را برمی‌گرداند.
- [ ] تمام مسیرهای API که عملیات نوشتن فایل را انجام می‌دهند، از `sanitize_path` استفاده می‌کنند.
- [ ] تلاش برای استفاده از دنباله‌های Path Traversal (مانند `../`) یا مسیرهای مطلق منجر به خطای 400 یا 403 می‌شود.
- [ ] تست‌های واحد برای اعتبارسنجی مسیر اضافه می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک تابع کمکی برای پاکسازی مسیر (path sanitization) ایجاد کنید که تمام دنباله‌های `../` و مسیرهای مطلق را حذف یا رد کند. این تابع باید اطمینان حاصل کند که مسیر نهایی همیشه در داخل دایرکتوری مجاز پروژه قرار دارد. این تابع باید قبل از هرگونه عملیات فایل سیستمی یا فراخوانی GitHub API روی مسیرهای ارسالی توسط کاربر/AI اعمال شود.
...
_بعد:_
```
from ...core.security_utils import sanitize_path

sanitized_path = sanitize_path(file_path)
if not sanitized_path:
    raise HTTPException(status_code=400, detail="Invalid file path")
full_path = os.path.join(project_root, sanitized_path)
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/security/test_path_sanitization.py`
- `curl -X POST http://localhost:8000/api/unified/projects/{project_id}/files -H 'Content-Type: application/json' -d '{"file_path": "../../.env", "content": "MALICIOUS_CONTENT"}' # باید 400/403 برگرداند`

## ⚠️ ریسک‌ها و موارد احتیاط
پیاده‌سازی نادرست می‌تواند عملیات فایل قانونی را مسدود کند یا همچنان آسیب‌پذیری را باقی بگذارد.
...
1. یک تابع `sanitize_path(path: str, base_dir: str)` ایجاد می‌شود که مسیرهای امن را برمی‌گرداند. _(verify: static)_
2. تمام مسیرهای API که عملیات نوشتن فایل را انجام می‌دهند، از `sanitize_path` استفاده می‌کنند. _(verify: static)_
3. تلاش برای استفاده از دنباله‌های Path Traversal (مانند `../`) یا مسیرهای مطلق منجر به خطای 400 یا 403 می‌شود. _(verify: api_response)_
4. تست‌های واحد برای اعتبارسنجی مسیر اضافه می‌شوند. _(verify: backend_test)_

## Task Steps

### Step 1: شناسایی و نقشه‌برداری Endpoints/توابع API آسیب‌پذیر
**Status:** `pending` (0%)
**Scope:** این مرحله اولیه شامل بررسی دقیق کدبیس، به ویژه تمرکز بر فایل‌های `project_memory.py` و `unified_api.py` است. هدف این است که تمام Endpointsهای API یا توابع داخلی که مسیرهای فایل ارائه‌شده توسط کاربر را برای عملیات ذخیره، آپلود یا Commit به GitHub می‌پذیرند، به دقت شناسایی شوند. این مرحله شامل پیاده‌سازی هیچ تغییری نیست، بلکه صرفاً ایجاد یک لیست دقیق از توابع هدف و پارامترهای ورودی آن‌ها است که نیاز به اعتبارسنجی مسیر دارند.
**Excerpt:**
```
مسیرهای API که به کاربران یا مدل‌های AI اجازه می‌دهند مسیر فایل‌ها را برای عملیات ذخیره، آپلود یا Commit به GitHub مشخص کنند (مانند `project_memory.py` و `unified_api.py`)، فاقد اعتبارسنجی کافی برای جلوگیری از حملات Path Traversal هستند. یک مهاجم می‌تواند از دنباله‌های `../` برای نوشتن فایل در مکان‌های دلخواه خارج از دایرکتوری پروژه استفاده کند که می‌تواند منجر به اجرای کد از راه دور (RCE) یا خرابی داده شود.
```

### Step 2: پیاده‌سازی یک ابزار مرکزی اعتبارسنجی مسیر امن
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد یک تابع یا کلاس ابزار قابل استفاده مجدد و قوی متمرکز است که مسئول اعتبارسنجی و پاکسازی مسیرهای فایل است. این ابزار باید به طور موثر تلاش‌های Path Traversal (مانند دنباله‌های `../`، مسیرهای مطلق خارج از دایرکتوری‌های مجاز) را شناسایی و خنثی کند، با حل مسیرها به یک فرم متعارف در یک دایرکتوری پایه امن از پیش تعریف شده. این ابزار باید یا یک مسیر امن را برگرداند یا برای ورودی‌های نامعتبر خطا ایجاد کند. این ابزار برای ادغام در چندین مسیر API طراحی خواهد شد.
**Excerpt:**
```
مسیرهای API که به کاربران یا مدل‌های AI اجازه می‌دهند مسیر فایل‌ها را برای عملیات ذخیره، آپلود یا Commit به GitHub مشخص کنند (مانند `project_memory.py` و `unified_api.py`)، فاقد اعتبارسنجی کافی برای جلوگیری از حملات Path Traversal هستند. یک مهاجم می‌تواند از دنباله‌های `../` برای نوشتن فایل در مکان‌های دلخواه خارج از دایرکتوری پروژه استفاده کند که می‌تواند منجر به اجرای کد از راه دور (RCE) یا خرابی داده شود.
```

### Step 3: ادغام اعتبارسنجی مسیر در `project_memory.py`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `project_memory.py` برای گنجاندن ابزار اعتبارسنجی مسیر توسعه یافته در مرحله 2 است. برای هر Endpoint API یا تابع شناسایی شده در `project_memory.py` که مسیرهای فایل ارائه‌شده توسط کاربر را برای عملیات ذخیره، آپلود یا Commit می‌پذیرد، مسیر ورودی باید *قبل از* انجام هرگونه عملیات سیستم فایل، از طریق ابزار اعتبارسنجی عبور داده شود. این امر تضمین می‌کند که فقط مسیرهای امن و مجاز پردازش می‌شوند و از حملات Path Traversal جلوگیری می‌کند.
**Excerpt:**
```
مسیرهای API که به کاربران یا مدل‌های AI اجازه می‌دهند مسیر فایل‌ها را برای عملیات ذخیره، آپلود یا Commit به GitHub مشخص کنند (مانند `project_memory.py` و `unified_api.py`)، فاقد اعتبارسنجی کافی برای جلوگیری از حملات Path Traversal هستند. یک مهاجم می‌تواند از دنباله‌های `../` برای نوشتن فایل در مکان‌های دلخواه خارج از دایرکتوری پروژه استفاده کند که می‌تواند منجر به اجرای کد از راه دور (RCE) یا خرابی داده شود.
```

### Step 4: ادغام اعتبارسنجی مسیر در `unified_api.py`
**Status:** `pending` (0%)
**Scope:** این مرحله مشابه مرحله 3 است اما بر فایل `unified_api.py` تمرکز دارد. ابزار اعتبارسنجی مسیر از مرحله 2 باید در تمام Endpointsهای API یا توابع شناسایی شده در `unified_api.py` که مسیرهای فایل ارائه‌شده توسط کاربر را برای عملیات ذخیره، آپلود یا Commit می‌پذیرند، ادغام شود. این ادغام باید *قبل از* انجام هرگونه عملیات سیستم فایل انجام شود تا اطمینان حاصل شود که فقط مسیرهای معتبر و امن استفاده می‌شوند، در نتیجه از حملات Path Traversal ناشی از این کامپوننت جلوگیری می‌شود.
**Excerpt:**
```
مسیرهای API که به کاربران یا مدل‌های AI اجازه می‌دهند مسیر فایل‌ها را برای عملیات ذخیره، آپلود یا Commit به GitHub مشخص کنند (مانند `project_memory.py` و `unified_api.py`)، فاقد اعتبارسنجی کافی برای جلوگیری از حملات Path Traversal هستند. یک مهاجم می‌تواند از دنباله‌های `../` برای نوشتن فایل در مکان‌های دلخواه خارج از دایرکتوری پروژه استفاده کند که می‌تواند منجر به اجرای کد از راه دور (RCE) یا خرابی داده شود.
```

### Step 5: افزودن تست‌های واحد و یکپارچه‌سازی جامع
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد جدید برای ابزار اعتبارسنجی مسیر (اگر در مرحله 2 به طور کامل پوشش داده نشده باشد) و تست‌های یکپارچه‌سازی برای مسیرهای API اصلاح شده در `project_memory.py` و `unified_api.py` است. تست‌ها باید طیف وسیعی از سناریوها را پوشش دهند، از جمله مسیرهای معتبر، مسیرهایی با اشکال مختلف دنباله‌های `../`، مسیرهای مطلق، بایت‌های تهی و سایر موارد خاص طراحی شده برای دور زدن اعتبارسنجی مسیر. هدف این است که اطمینان حاصل شود اعتبارسنجی پیاده‌سازی شده به درستی تمام تلاش‌های شناخته شده Path Traversal را شناسایی و مسدود می‌کند بدون اینکه عملیات فایل قانونی را مختل کند.
**Excerpt:**
```
مسیرهای API که به کاربران یا مدل‌های AI اجازه می‌دهند مسیر فایل‌ها را برای عملیات ذخیره، آپلود یا Commit به GitHub مشخص کنند (مانند `project_memory.py` و `unified_api.py`)، فاقد اعتبارسنجی کافی برای جلوگیری از حملات Path Traversal هستند. یک مهاجم می‌تواند از دنباله‌های `../` برای نوشتن فایل در مکان‌های دلخواه خارج از دایرکتوری پروژه استفاده کند که می‌تواند منجر به اجرای کد از راه دور (RCE) یا خرابی داده شود.
```

### Step 6: انجام ممیزی امنیتی و بازبینی کد
**Status:** `pending` (0%)
**Scope:** این مرحله نهایی شامل یک ممیزی امنیتی کامل و بازبینی کد تمام تغییرات ایجاد شده در مراحل قبلی است. بازبینی باید بر اطمینان از اینکه منطق اعتبارسنجی مسیر به درستی پیاده‌سازی شده است، تمام نقاط آسیب‌پذیر شناسایی شده در `project_memory.py` و `unified_api.py` را پوشش می‌دهد و هیچ آسیب‌پذیری یا رگرسیون جدیدی را معرفی نمی‌کند، تمرکز کند. این شامل بررسی موارد خاص، دور زدن‌های احتمالی و رعایت بهترین شیوه‌های امنیتی است. ممیزی همچنین باید تأیید کند که مدیریت خطا برای مسیرهای نامعتبر مناسب است و اطلاعات حساس را فاش نمی‌کند.
**Excerpt:**
```
مسیرهای API که به کاربران یا مدل‌های AI اجازه می‌دهند مسیر فایل‌ها را برای عملیات ذخیره، آپلود یا Commit به GitHub مشخص کنند (مانند `project_memory.py` و `unified_api.py`)، فاقد اعتبارسنجی کافی برای جلوگیری از حملات Path Traversal هستند. یک مهاجم می‌تواند از دنباله‌های `../` برای نوشتن فایل در مکان‌های دلخواه خارج از دایرکتوری پروژه استفاده کند که می‌تواند منجر به اجرای کد از راه دور (RCE) یا خرابی داده شود.
```
