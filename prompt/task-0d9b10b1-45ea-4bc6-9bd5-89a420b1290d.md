---
task_id: 0d9b10b1-45ea-4bc6-9bd5-89a420b1290d
title: Bare except و except با pass در حداقل ۱۵ نقطه از routes خطاها را بی‌صدا می‌بلعند
type: bug
priority: high
...
created_at: '2026-06-14T09:41:04.066872+00:00'
updated_at: '2026-06-14T09:42:36.068814+00:00'
target_files:
...
# Bare except و except با pass در حداقل ۱۵ نقطه از routes خطاها را بی‌صدا می‌بلعند

## Raw Idea

الگوی `except:` خالی یا `except Exception: pass` به‌طور گسترده در فایل‌های route استفاده شده که خطاها را بدون log می‌بلعد و دیباگ را غیرممکن می‌کند. نمونه‌ها: analysis.py خطوط ۱۴۰-۱۴۱، ۱۴۹-۱۵۰، ۱۷۹-۱۸۰؛ github_import.py خط ۳۵۳-۳۵۴؛ project_journal.py خطوط ۴۴-۴۵، ۹۵-۹۶، ۳۸۴-۳۸۵؛ project_memory.py خط ۳۷۸-۳۷۹؛ model_profiles.py خط ۲۷۲-۲۷۳؛ security_analysis.py خطوط ۷۶-۷۷، ۱۲۴-۱۲۵. این bare except حتی KeyboardInterrupt و SystemExit را هم می‌گیرد. هنگام بروز مشکل (مثلاً JSON خراب یا fail شدن extraction) کاربر هیچ سیگنالی نمی‌بیند.
...
- `backend/app/api/routes/analysis.py:139-141` — `readme reading` — bare except حین خواندن README
  ```python
  with open(readme_path, 'r', encoding='utf-8') as f:
      ...
      pass
  ```
- `backend/app/api/routes/project_journal.py:44-45` — `get_active_models` — bare except که خطای DB را پنهان می‌کند
  ```python
  try:
      ...
      pass
  ```
- `backend/app/api/routes/security_analysis.py:73-77` — `scan_for_secrets json parse` — JSON parse error بی‌صدا
  ```python
  stored_files = json.loads(project.github_files) ...
  except:
      pass
  ```
...
FastAPI + Python exception handling + StructuredLogger

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/core/logging_utils.py` (سطر 1) — StructuredLogger که باید برای log استفاده شود
- `backend/app/api/routes/model_profiles.py` — except خالی در parsing score_history
- `backend/app/api/routes/project_memory.py` — except خالی در commit_to_github SHA check
...
- `backend/app/core/database.py` — `project_journal.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
logging_utils.py توسط ۱۹ فایل import می‌شود — الگوی StructuredLogger از قبل موجود است و فقط باید در این نقاط فراخوانی شود.
...
- [ ] هیچ `except:` خالی (بدون نوع exception) در فایل‌های routes باقی نمانده باشد — قابل بررسی با grep
- [ ] هر except که قبلاً pass داشت حداقل یک logger call یا کامنت # best-effort صریح دارد
- [ ] ruff با قانون E722 (bare-except) روی پوشهٔ routes بدون خطا اجرا شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. همهٔ `except:` را به `except Exception as e:` تبدیل کن و حداقل یک `logger.debug/warning` با پیام معنادار اضافه کن. در نقاطی که silent-ignore عمداً درست است (مثل cleanup‌های best-effort) کامنت `# best-effort` اضافه کن تا تمایز روشن شود.
...
```
    ...
    logger.debug(f"reading README failed (non-fatal): {e}")
```
...
## 🧪 دستورات اعتبارسنجی
- `ruff check backend/app/api/routes/ --select E722`
- `grep -rn 'except:' backend/app/api/routes/ | grep -v 'except Exception'`

## ⚠️ ریسک‌ها و موارد احتیاط
بسیار کم — تغییرات صرفاً افزودن log هستند و رفتار را تغییر نمی‌دهند
...
1. هیچ `except:` خالی (بدون نوع exception) در فایل‌های routes باقی نمانده باشد — قابل بررسی با grep _(verify: static)_
2. هر except که قبلاً pass داشت حداقل یک logger call یا کامنت # best-effort صریح دارد _(verify: static)_
3. ruff با قانون E722 (bare-except) روی پوشهٔ routes بدون خطا اجرا شود _(verify: static)_

## Task Steps

### Step 1: اصلاح bare except در analysis.py (خطوط ۱۴۰-۱۴۱، ۱۴۹-۱۵۰، ۱۷۹-۱۸۰)
**Status:** `pending` (0%)
**Scope:** در فایل route مربوط به analysis (همان فایلی که کاربر با نام analysis.py اشاره کرده و در آن متد security_analysis ممکن است حضور داشته باشد)، سه نقطهٔ bare except یا except Exception: pass را در خطوط تقریبی ۱۴۰-۱۴۱، ۱۴۹-۱۵۰ و ۱۷۹-۱۸۰ اصلاح کن. هر کدام باید به یک except با نوع خطای مشخص (مثلاً except (ValueError, KeyError) یا except Exception as e بسته به context) تبدیل شود و خطا با logger.exception یا logger.error به‌همراه stack trace و context کافی ثبت گردد. هرگز KeyboardInterrupt و SystemExit نباید بلعیده شوند. خارج از scope: تغییر منطق business یا refactor کلی فایل. نکته حیاتی: شماره خطوط تقریبی است؛ مدل اجراکننده باید خودش هر سه نقطه را با grep پیدا و verify کند.
**Excerpt:**
```
الگوی `except:` خالی یا `except Exception: pass` به‌طور گسترده در فایل‌های route استفاده شده که خطاها را بدون log می‌بلعد و دیباگ را غیرممکن می‌کند. نمونه‌ها: analysis.py خطوط ۱۴۰-۱۴۱، ۱۴۹-۱۵۰، ۱۷۹-۱۸۰
```

### Step 2: اصلاح bare except در github_import.py (خط ۳۵۳-۳۵۴)
**Status:** `pending` (0%)
**Scope:** در فایل route مربوط به github_import.py، نقطهٔ bare except یا except Exception: pass در خطوط تقریبی ۳۵۳-۳۵۴ را اصلاح کن. except باید به نوع خطای مشخص محدود شود و خطا با logger به‌همراه stack trace و context (مثلاً نام repo یا URL در حال import) ثبت گردد. KeyboardInterrupt و SystemExit نباید گرفته شوند. خارج از scope: تغییر منطق import یا سایر فایل‌ها. نکته حیاتی: github_import یکی از identifierهای snake_case ذکرشده است؛ مسیر فایل دقیقاً همان github_import.py باقی بماند.
**Excerpt:**
```
github_import.py خط ۳۵۳-۳۵۴؛ ... این bare except حتی KeyboardInterrupt و SystemExit را هم می‌گیرد. هنگام بروز مشکل (مثلاً JSON خراب یا fail شدن extraction) کاربر هیچ سیگنالی نمی‌بیند.
```

### Step 3: اصلاح bare except در project_journal.py (خطوط ۴۴-۴۵، ۹۵-۹۶، ۳۸۴-۳۸۵)
**Status:** `pending` (0%)
**Scope:** در فایل project_journal.py سه نقطهٔ bare except یا except Exception: pass در خطوط تقریبی ۴۴-۴۵، ۹۵-۹۶ و ۳۸۴-۳۸۵ را اصلاح کن. هر except باید نوع خطای مشخص داشته باشد و با logger.exception/error به‌همراه stack trace و context ثبت شود؛ KeyboardInterrupt و SystemExit نباید بلعیده شوند. خارج از scope: تغییر منطق journal یا سایر فایل‌ها. نکته حیاتی: project_journal یکی از identifierهای ذکرشده است؛ هر سه نقطه باید جداگانه اصلاح شوند نه ادغام.
**Excerpt:**
```
project_journal.py خطوط ۴۴-۴۵، ۹۵-۹۶، ۳۸۴-۳۸۵؛
```

### Step 4: اصلاح bare except در project_memory.py (خط ۳۷۸-۳۷۹)
**Status:** `pending` (0%)
**Scope:** در فایل project_memory.py نقطهٔ bare except یا except Exception: pass در خطوط تقریبی ۳۷۸-۳۷۹ را اصلاح کن. except باید به نوع خطای مشخص محدود شود و خطا با logger به‌همراه stack trace و context ثبت گردد؛ KeyboardInterrupt و SystemExit نباید گرفته شوند. خارج از scope: تغییر منطق memory یا سایر فایل‌ها. نکته حیاتی: project_memory یکی از identifierهای ذکرشده است؛ مسیر فایل ثابت بماند.
**Excerpt:**
```
project_memory.py خط ۳۷۸-۳۷۹؛ ... این bare except حتی KeyboardInterrupt و SystemExit را هم می‌گیرد.
```

### Step 5: اصلاح bare except در model_profiles.py (خط ۲۷۲-۲۷۳)
**Status:** `pending` (0%)
**Scope:** در فایل model_profiles.py نقطهٔ bare except یا except Exception: pass در خطوط تقریبی ۲۷۲-۲۷۳ را اصلاح کن. except باید نوع خطای مشخص داشته باشد و خطا با logger به‌همراه stack trace و context ثبت شود؛ KeyboardInterrupt و SystemExit نباید بلعیده شوند. خارج از scope: تغییر منطق model profile یا سایر فایل‌ها. نکته حیاتی: model_profiles یکی از identifierهای ذکرشده است؛ مسیر فایل دقیقاً model_profiles.py.
**Excerpt:**
```
model_profiles.py خط ۲۷۲-۲۷۳؛
```

### Step 6: اصلاح bare except در security_analysis.py (خطوط ۷۶-۷۷، ۱۲۴-۱۲۵)
**Status:** `pending` (0%)
**Scope:** در فایل security_analysis.py دو نقطهٔ bare except یا except Exception: pass در خطوط تقریبی ۷۶-۷۷ و ۱۲۴-۱۲۵ را اصلاح کن. هر except باید نوع خطای مشخص داشته باشد و خطا با logger.exception/error به‌همراه stack trace و context ثبت شود؛ KeyboardInterrupt و SystemExit نباید گرفته شوند. خارج از scope: تغییر منطق security analysis یا سایر فایل‌ها. نکته حیاتی: security_analysis یکی از identifierهای ذکرشده است؛ هر دو نقطه جداگانه اصلاح شوند.
**Excerpt:**
```
security_analysis.py خطوط ۷۶-۷۷، ۱۲۴-۱۲۵. این bare except حتی KeyboardInterrupt و SystemExit را هم می‌گیرد. هنگام بروز مشکل (مثلاً JSON خراب یا fail شدن extraction) کاربر هیچ سیگنالی نمی‌بیند.
```

### Step 7: audit نهایی: تضمین عدم وجود bare except بلعندهٔ خطا در همهٔ فایل‌های route اصلاح‌شده
**Status:** `pending` (0%)
**Scope:** یک بررسی نهایی (audit) روی هر شش فایل اصلاح‌شده (analysis.py, github_import.py, project_journal.py, project_memory.py, model_profiles.py, security_analysis.py) انجام بده تا مطمئن شوی هیچ `except:` خالی یا `except Exception: pass` بدون log باقی نمانده و هیچ‌کدام KeyboardInterrupt/SystemExit را نمی‌بلعند. الگوی استاندارد ثبت خطا (logger با stack trace + context) باید در همه نقاط یکدست اعمال شده باشد. خارج از scope: افزودن نقاط جدید except یا تغییر منطق. نکته حیاتی: این مرحله صرفاً verification و یکدست‌سازی است؛ اگر هنگام audit نقطه‌ای جا مانده بود، آن را نیز اصلاح کن.
**Excerpt:**
```
الگوی `except:` خالی یا `except Exception: pass` به‌طور گسترده در فایل‌های route استفاده شده که خطاها را بدون log می‌بلعد و دیباگ را غیرممکن می‌کند. ... این bare except حتی KeyboardInterrupt و SystemExit را هم می‌گیرد.
```
