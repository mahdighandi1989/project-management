---
task_id: 7de33d62-2413-4877-b09c-4970c4b7e98b
title: نقص در مدیریت Session دیتابیس در endpoint های Analysis و Model Profiles
type: bug
priority: high
...
created_at: '2026-05-31T09:07:09.948225+00:00'
updated_at: '2026-06-01T17:06:51.092880+00:00'
target_files:
...
# نقص در مدیریت Session دیتابیس در endpoint های Analysis و Model Profiles

## Raw Idea

در چندین endpoint (analysis.py, model_profiles.py, models.py) session دیتابیس با `SessionLocal()` ساخته می‌شود اما در مسیرهای خطا یا استثنا بسته نمی‌شود. در `analysis.py` خطوط 280-290 و 296-306 و 348-352، session در `try/finally` بسته می‌شود ولی در `run_analysis_stream` (خط 117) session درون تابع async ساخته می‌شود و در `finally` بسته نمی‌شود. در `model_profiles.py` خطوط 176-227 و 259-278 و 366-378، session با `Depends(get_db)` تزریق می‌شود که مدیریت خودکار دارد ولی برخی endpoint ها مستقیماً `SessionLocal()` می‌سازند و در `finally` نمی‌بندند. این باعث نشت connection در بار بالا می‌شود.
...
- `backend/app/api/routes/analysis.py:115-124` — `run_analysis_stream` — session ساخته شده ولی در finally بسته نمی‌شود
  ```python
  ...
  ```
- `backend/app/api/routes/analysis.py:207-212` — `run_analysis_task` — در این finally باید analysis_db.close() اضافه شود
  ```python
  finally:
  ...
  ```
- `backend/app/api/routes/model_profiles.py:176-227` — `get_all_profiles` — از Depends(get_db) استفاده می‌کند که safe است
  ```python
...
- `backend/app/core/database.py` (سطر 1) — SessionLocal و get_db در اینجا تعریف شده‌اند
- `backend/app/api/routes/project_health.py` (سطر 316) — الگوی مشابه دارد
...
## 🌐 نقشهٔ وابستگی‌ها
این issue روی ۳ فایل route تأثیر می‌گذارد و با الگوی common در database.py مرتبط است.
...
- [ ] تمامی session های ساخته شده با SessionLocal() در finally بسته شوند
- [ ] هیچ warning ای در log مربوط به unclosed session دیده نشود
- [ ] endpoint run_analysis_stream بعد از ۱۰۰ درخواست همزمان بدون خطا کار کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. رفع نشت session در `run_analysis_stream` با اضافه کردن `finally` که `analysis_db.close()` را صدا بزند. همچنین در `model_profiles.py` تمام `SessionLocal()` مستقیم را با `Depends(get_db)` جایگزین کن یا `try/finally` اضافه کن.
...
## 🧪 دستورات اعتبارسنجی
- `pytest backend/tests/ -k analysis -v --log-cli-level=WARNING 2>&`
...
1. تمامی session های ساخته شده با SessionLocal() در finally بسته شوند _(verify: static)_
2. هیچ warning ای در log مربوط به unclosed session دیده نشود _(verify: backend_test)_
3. endpoint run_analysis_stream بعد از ۱۰۰ درخواست همزمان بدون خطا کار کند _(verify: backend_test)_

## Task Steps

### Step 1: رفع نشت session در `run_analysis_stream` (analysis.py خط 117)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح تابع async `run_analysis_stream` در فایل `analysis.py` است. session ساخته‌شده با `SessionLocal()` درون این تابع باید در بلوک `try/finally` بسته شود. خارج از این مرحله: سایر endpoint های analysis.py که قبلاً session را در `finally` می‌بندند (خطوط 280-290 و 296-306 و 348-352) نیاز به تغییر ندارند. نکته حیاتی: session در تابع async ساخته می‌شود، بنابراین بستن آن باید با `await session.close()` یا همگام انجام شود و از `contextlib` یا `async with` استفاده نشود مگر اینکه کتابخانه async پشتیبانی کند.
**Excerpt:**
```
در `analysis.py` خطوط 280-290 و 296-306 و 348-352، session در `try/finally` بسته می‌شود ولی در `run_analysis_stream` (خط 117) session درون تابع async ساخته می‌شود و در `finally` بسته نمی‌شود.
```

### Step 2: رفع نشت session در endpoint‌های `model_profiles.py` که مستقیماً `SessionLocal()` می‌سازند (خطوط 176-227)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و اصلاح endpoint‌های موجود در `model_profiles.py` در محدوده خطوط 176-227 است. باید مشخص شود کدام endpoint‌ها مستقیماً `SessionLocal()` می‌سازند و session را در `finally` نمی‌بندند. سپس برای آن‌ها بلوک `try/finally` اضافه شود. خارج از این مرحله: endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند (خطوط 259-278 و 366-378) نیاز به تغییر ندارند. نکته حیاتی: نباید endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند به `SessionLocal()` تغییر داده شوند.
**Excerpt:**
```
در `model_profiles.py` خطوط 176-227 و 259-278 و 366-378، session با `Depends(get_db)` تزریق می‌شود که مدیریت خودکار دارد ولی برخی endpoint ها مستقیماً `SessionLocal()` می‌سازند و در `finally` نمی‌بندند.
```

### Step 3: رفع نشت session در endpoint‌های `model_profiles.py` که مستقیماً `SessionLocal()` می‌سازند (خطوط 259-278)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و اصلاح endpoint‌های موجود در `model_profiles.py` در محدوده خطوط 259-278 است. باید مشخص شود کدام endpoint‌ها مستقیماً `SessionLocal()` می‌سازند و session را در `finally` نمی‌بندند. سپس برای آن‌ها بلوک `try/finally` اضافه شود. خارج از این مرحله: endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند (خطوط 176-227 و 366-378) نیاز به تغییر ندارند. نکته حیاتی: نباید endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند به `SessionLocal()` تغییر داده شوند.
**Excerpt:**
```
در `model_profiles.py` خطوط 176-227 و 259-278 و 366-378، session با `Depends(get_db)` تزریق می‌شود که مدیریت خودکار دارد ولی برخی endpoint ها مستقیماً `SessionLocal()` می‌سازند و در `finally` نمی‌بندند.
```

### Step 4: رفع نشت session در endpoint‌های `model_profiles.py` که مستقیماً `SessionLocal()` می‌سازند (خطوط 366-378)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و اصلاح endpoint‌های موجود در `model_profiles.py` در محدوده خطوط 366-378 است. باید مشخص شود کدام endpoint‌ها مستقیماً `SessionLocal()` می‌سازند و session را در `finally` نمی‌بندند. سپس برای آن‌ها بلوک `try/finally` اضافه شود. خارج از این مرحله: endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند (خطوط 176-227 و 259-278) نیاز به تغییر ندارند. نکته حیاتی: نباید endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند به `SessionLocal()` تغییر داده شوند.
**Excerpt:**
```
در `model_profiles.py` خطوط 176-227 و 259-278 و 366-378، session با `Depends(get_db)` تزریق می‌شود که مدیریت خودکار دارد ولی برخی endpoint ها مستقیماً `SessionLocal()` می‌سازند و در `finally` نمی‌بندند.
```

### Step 5: رفع نشت session در endpoint‌های `models.py` که مستقیماً `SessionLocal()` می‌سازند
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و اصلاح endpoint‌های موجود در `models.py` است. باید مشخص شود کدام endpoint‌ها مستقیماً `SessionLocal()` می‌سازند و session را در `finally` نمی‌بندند. سپس برای آن‌ها بلوک `try/finally` اضافه شود. خارج از این مرحله: endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند نیاز به تغییر ندارند. نکته حیاتی: فایل `models.py` ممکن است شامل مدل‌های SQLAlchemy باشد نه endpoint‌ها؛ در این صورت باید فایل مربوط به endpoint‌های models (مثلاً `models_endpoints.py` یا `routers/models.py`) بررسی شود.
**Excerpt:**
```
در چندین endpoint (analysis.py, model_profiles.py, models.py) session دیتابیس با `SessionLocal()` ساخته می‌شود اما در مسیرهای خطا یا استثنا بسته نمی‌شود.
```

### Step 6: بررسی و رفع نشت session در سایر فایل‌های endpoint (در صورت وجود)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل جستجوی سراسری در پروژه برای یافتن تمام مواردی است که `SessionLocal()` مستقیماً ساخته می‌شود و در `finally` بسته نمی‌شود. این جستجو باید تمام فایل‌های Python را شامل شود. برای هر مورد یافت‌شده، بلوک `try/finally` اضافه شود. خارج از این مرحله: endpoint‌هایی که از `Depends(get_db)` استفاده می‌کنند نیاز به تغییر ندارند. نکته حیاتی: این مرحله باید پس از اصلاح فایل‌های ذکرشده (analysis.py, model_profiles.py, models.py) انجام شود تا موارد اضافی نیز پوشش داده شوند.
**Excerpt:**
```
این باعث نشت connection در بار بالا می‌شود.
```
